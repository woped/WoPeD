...
public class ModelTranslator {
	public static UMLModelProcessor translate(PetriNetModelProcessor petrinet) {
		...
		for (Iterator placeIter = petrinet.getElementContainer()
				.getElementsByType(AbstractPetriNetModelElement.PLACE_TYPE)
				.values().iterator(); placeIter.hasNext();) {
			PlaceModel place = (PlaceModel) placeIter.next();
			tempSourceElements = petrinet.getElementContainer()
					.getSourceElements(place.getId());
			tempTargetElements = petrinet.getElementContainer()
					.getTargetElements(place.getId());
			...
			if ("START".equals(place.getNameValue())) {
				...
				uml.createElement(tempCreateMap).setElementContext(
						place.getElementContext());
			...
			// }
				...
				// Check if split-join
				for (Iterator sourceIter = tempSourceElements.values()
					...
					if (tempSource.getType() == OperatorTransitionModel.XOR_JOIN_TYPE) {
						for (Iterator targetIter = tempTargetElements.values()
								.iterator(); targetIter.hasNext();) {
							tempTarget = (AbstractElementModel) sourceIter
									.next();
							if (tempTarget.getType() == OperatorTransitionModel.XOR_SPLIT_TYPE) {
								// referenc on each other
								Object newId = uml
										.getNewElementId(OperatorTransitionModel.XOR_SPLIT_TYPE);
		...
		for (Iterator iter = petrinet.getElementContainer().getRootElements()
			...
			if (tempElement.getType() == AbstractPetriNetModelElement.TRANS_SIMPLE_TYPE) {
				...
				if (tempElement.getElementContext().isXORType()) {
					tempCreateMap
							.setType(AbstractUMLElementModel.OPERATOR_TYPE);
					...
					tempCreateMap
							.setType(AbstractUMLElementModel.ACTIVITY_TYPE);
				}
				uml.createElement(tempCreateMap).setElementContext(
						tempElement.getElementContext());
				...
				if (tempOperator.getOperatorType() == OperatorTransitionModel.AND_JOIN_TYPE
						|| tempOperator.getOperatorType() == OperatorTransitionModel.AND_SPLIT_TYPE) {
					tempCreateMap.setOperatorType(OperatorModel.AND_TYPE);
					uml.createElement(tempCreateMap).setElementContext(
							tempElement.getElementContext());
					...
					if (idMapper.containsKey(tempOperator.getId())) {
						tempCreateMap.setId((String) idMapper.get(tempOperator
								.getId()));
					}
					uml.createElement(tempCreateMap).setElementContext(
							tempElement.getElementContext());
		...
		ArcModel tempArc;
		for (Iterator arcIter = petrinet.getElementContainer().getArcMap()
				.values().iterator(); arcIter.hasNext();) {
			...
			tempTarget = petrinet.getElementContainer().getElementById(
					tempArc.getTargetId());
			tempSource = petrinet.getElementContainer().getElementById(
					tempArc.getSourceId());
			if (uml.getElementContainer().containsElement(tempTarget.getId())) {
				if (uml.getElementContainer().containsElement(
						tempSource.getId())) {
					uml.createArc(tempSource.getId(), tempTarget.getId());
				} else {
					for (Iterator sourceIter = petrinet.getElementContainer()
							.getSourceElements(tempSource.getId()).values()
							.iterator(); sourceIter.hasNext();) {
						tempSource = (AbstractPetriNetModelElement) sourceIter
								.next();
						if (uml.getElementContainer().containsElement(
								tempSource.getId())) {
							uml.createArc(tempSource.getId(), tempTarget
									.getId());
						}
					}
		...
		LoggerManager.debug(Constants.CORE_LOGGER, "### Translation done ("
	...
	public static PetriNetModelProcessor translate(UMLModelProcessor uml) {
		...
		for (Iterator iter = uml.getElementContainer().getRootElements()
				.iterator(); iter.hasNext();) {
			...
			if (tempElement.getType() == AbstractUMLElementModel.ACTIVITY_TYPE) {
				...
				petrinet.createElement(tempMap).setElementContext(
						tempElement.getElementContext());
			} else if (tempElement.getType() == AbstractUMLElementModel.OPERATOR_TYPE) {
				Map outgoingArcs = uml.getElementContainer().getOutgoingArcs(
						tempElement.getId());
				Map incomingArcs = uml.getElementContainer().getIncomingArcs(
						tempElement.getId());
				...
				if (((OperatorModel) tempElement).getOperatorType() == OperatorModel.XOR_TYPE) {
					...
					if (outgoingArcs.size() > 1 && incomingArcs.size() > 1) {
						...
						petrinet.createElement(tempMap).setElementContext(
								tempElement.getElementContext());
						petrinet.createElement(tempMap2).setElementContext(
								tempElement.getElementContext());
						tempPlaceElement = petrinet.createElement(placeMap);
						petrinet.createArc(tempMap.getId(), tempPlaceElement
								.getId());
						petrinet.createArc(tempPlaceElement.getId(), tempMap2
								.getId());
						...
						tempPetriElement.setElementContext(tempElement
								.getElementContext());
		...
		for (Iterator iter = uml.getElementContainer().getArcMap().values()
				.iterator(); iter.hasNext();) {
			ArcModel arc = (ArcModel) iter.next();
			// memo all outgoing and incoming arcs
			tempSource = uml.getElementContainer().getElementById(
					arc.getSourceId());
			tempTarget = uml.getElementContainer().getElementById(
					arc.getTargetId());
			// Check if Target or source is listed in the idMapper... then
			// correct Ids!
			if (idMapper.containsKey(tempSource.getId())) {
				selectedSourceId = ((List) idMapper.get(tempSource.getId()))
						.get(1); // It's
				// the
				// Split!
			} else {
			...
			if (idMapper.containsKey(tempTarget.getId())) {
				selectedTargetId = ((List) idMapper.get(tempTarget.getId()))
						.get(0); // It's
				// the
				// Join!
			} else {
			...
			if (AbstractUMLElementModel.ACTIVITY_TYPE == tempSource.getType()
					|| tempSource.getType() == AbstractUMLElementModel.OPERATOR_TYPE) {
				if (AbstractUMLElementModel.ACTIVITY_TYPE == tempTarget
						.getType()
						|| tempTarget.getType() == AbstractUMLElementModel.OPERATOR_TYPE) {
					CreationMap tempPlace = CreationMap.createMap();
					tempPlace.setType(AbstractPetriNetModelElement.PLACE_TYPE);
					tempPlace.setPosition((tempSource.getX() + tempTarget
							.getX()) / 2, (tempSource.getY() + tempTarget
							.getY()) / 2);
					tempPlaceElement = petrinet.createElement(tempPlace);
					petrinet.createArc(selectedSourceId, tempPlaceElement
							.getId());
					petrinet.createArc(tempPlaceElement.getId(),
							selectedTargetId);
				} else if (AbstractUMLElementModel.STATE_TYPE == tempTarget
						.getType()) {
					petrinet.createArc(selectedSourceId, selectedTargetId);
				}
			} else if (AbstractUMLElementModel.STATE_TYPE == tempSource
					.getType()) {
				if (AbstractUMLElementModel.ACTIVITY_TYPE == tempTarget
						.getType()
						|| tempTarget.getType() == AbstractUMLElementModel.OPERATOR_TYPE) {
