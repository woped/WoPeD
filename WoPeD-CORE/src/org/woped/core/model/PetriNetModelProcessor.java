...
public class PetriNetModelProcessor implements
		Serializable {
				...
				if (map.getTriggerType() != -1) {
					...
					if (map.getTriggerPosition() != null) {
						((TransitionModel) anElement)
								.getToolSpecific()
								.getTrigger()
								.setPosition(map.getTriggerPosition().x,
				...
				if (map.getResourceOrgUnit() != null
					...
					if (map.getResourcePosition() != null) {
						((TransitionModel) anElement)
								.getToolSpecific()
								.getTransResource()
								.setPosition(map.getResourcePosition().x,
										map.getResourcePosition().y);
					}
				}
				if (map.getTransitionTime() != -1) {
					((TransitionModel) anElement).getToolSpecific().setTime(
	 ...
	 * @return
	 */
	public Vector<String> getResourceClassesResourceIsAssignedTo(
			String resourceId) {
	...
	public String getLogicalFingerprint() {
		...
		Iterator<AbstractPetriNetElementModel> aemIter = getElementContainer()
		...
		while (amIter.hasNext()) {
			 ...
			 * currArc.getTargetId();
			 */
			arcFingerprint += "*" + currArc.getSourceId()
					+ currArc.getTargetId();
		}
	 ...
	 * do
	 * 
	 * @param fingerprintToCheck
	 *            The fingerprintrepresentation which get's compared to the
	 *            current fingerprint
	...
	public boolean isLogicalFingerprintEqual(String fingerprintToCheck) {
		...
		currLeadingPart = currFingerprint.substring(0,
				currFingerprint.indexOf('*'));
		...
		if (currFingerprint.length() == fingerprintToCheck.length()
				&& currLeadingPart.equals(fingerprintToCheck.substring(0,
						fingerprintToCheck.indexOf('*')))) {
			...
			String arcsToCheck = fingerprintToCheck
					.substring(fingerprintToCheck.indexOf('*'));
			String currArc;
			boolean deleted = true;
			while (deleted && arcsToCheck != "") {
				// delete the leading '*'
				arcsToCheck = arcsToCheck
						.substring(arcsToCheck.indexOf('*') + 1);
				// Get the next arc
				// - String till the next occurance of '*'
				// - or till the end of the String if there is no more '*'
				if (arcsToCheck.indexOf('*') != -1) {
					currArc = arcsToCheck
							.substring(0, arcsToCheck.indexOf('*'));
	...
	public void setOrganizationUnits(
			Vector<ResourceClassModel> organizationUnits) {
	 ...
	 */
	public void setResourceMapping(
			LinkedHashMap<String, Vector<String>> resourceMapping) {
	...
	public void setResourceMapping(
			HashMap<String, Vector<String>> resourceMapping) {
		this.resourceMapping = resourceMapping;
	}
