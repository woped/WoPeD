 ...
 * <br>
 * 
 *         Represents a Petri-Net. It is the main access point to the Model. Contains the ModelElementFactory and the ModelElementContainer <br>
 * <br>
...
public class PetriNetModelProcessor extends AbstractModelProcessor implements Serializable {
    ...
    /**
     * Method getNewPetriNetElement. Creates a new PetriNetElement with a special type (@see PetriNetElementModel) and a Aalst type respectivley
     * 
     * @param type
     * @param aalsttype
            ...
            if (map.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE
                ...
                if (map.getTriggerType() != -1) {
                    ...
                    if (map.getTriggerPosition() != null) {
                        ((TransitionModel) anElement).getToolSpecific().getTrigger().setPosition(
                                map.getTriggerPosition().x, map.getTriggerPosition().y);
                    }
                }
                if (map.getResourceOrgUnit() != null && map.getResourceRole() != null) {
                    newTransResource(map);
                    if (map.getResourcePosition() != null) {
                        ((TransitionModel) anElement).getToolSpecific().getTransResource().setPosition(
                                map.getResourcePosition().x, map.getResourcePosition().y);
                    }
                }
    ...
    /**
     * This Method is called, when Elements get connected. If the source or the target is an Operator, the internal transformation to the classic petrinet will
     * be done.
     * 
     * @param sourceId
     * @param targetId
    ...
    /**
     * This Method is called, when Elements get connected. It only checks the connection for AalstElements and guarantees the building of a classical Petrinet
     * in The Model, if transformOperators is true.
     * 
     * @param id
     * @param sourceId
     * @param targetId
    ...
    public ArcModel createArc(String id, Object sourceId, Object targetId, Point2D[] points, boolean transformOperators) {
        ...
        ArcModel displayedArc = ModelElementFactory.createArcModel(id, (DefaultPort) sourceModel.getChildAt(0),
    ...
    public void insertArc(ArcModel arc, boolean transformOperators) {
        AbstractElementModel sourceModel = getElementContainer().getElementById(arc.getSourceId());
        AbstractElementModel targetModel = getElementContainer().getElementById(arc.getTargetId());
        ...
        if (transformOperators) {
            ...
            /* IF OPERATOR IS TARGET */
                ...
                if (targetModel.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE) {
    ...
    public void removeArc(Object id) {
        ...
        if (arcToDelete != null) {
            ...
            if (targetElement.getType() == PetriNetModelElement.SUBP_TYPE) {
                ((SubProcessModel) targetElement).getSimpleTransContainer().removeElement(sourceElement.getId());
            }

            if (sourceElement.getType() == PetriNetModelElement.SUBP_TYPE) {
    ...
    /**
     * The fingerprint is represented as a String with the following components concatenated: - the number of places - all place-IDs directly followed by their
     * individual TokenCount - the '#'-sign as an seperator - the number of arcs - for each arc a concatenation of '*', the arc's sourceID and the arc's
     * targetID Example: p11p20p30p4#6*p1t1*t1p2*p2t2*t2p3*p3t3*t3p4
     * 
    ...
    public String getLogicalFingerprint() {
        ...
        Iterator<AbstractElementModel> aemIter = getElementContainer().getElementsByType(
                PetriNetModelElement.PLACE_TYPE).values().iterator();
        int count = 0;
    ...
    /**
     * Compares an fingerprint to the fingerprint of the current net on a logical basis and not absolute like the equals-method of the String would do
     * 
    ...
    public boolean isLogicalFingerprintEqual(String fingerprintToCheck) {
        ...
        if (currFingerprint.length() == fingerprintToCheck.length()
                && currLeadingPart.equals(fingerprintToCheck.substring(0, fingerprintToCheck.indexOf('*')))) {
