 ...
 */
public class PetriNetModelProcessor extends AbstractModelProcessor implements Serializable
{
        ...
        if (transformOperators)
        {
            ...
            if (sourceModel.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE)
            {
                ...
                if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_SPLIT_TYPE)
                {
                    ...
                    // neuen Eintrag hat
                    if (getElementContainer().getTargetElements(operatorModel.getId()).size() == 1
                            && operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() == 1)
                    {
                        // get simple trans
                ...
                else if (operatorModel.getOperatorType() == OperatorTransitionModel.OR_SPLIT_TYPE)
                {
                    ...
                    {
                        LoggerManager.error(Constants.CORE_LOGGER, "The Maximum for OR Split is 5 following places.");
                    } else
                    {
                        ...
                        for (int k = trans - newTrans; k < trans; k++)
                        {
                            ...
                            for (short l = 0; l < z; l++)
                            {
                                ...
                                {
                                    operatorModel.addReference(getNexArcId(), (DefaultPort) simpleTrans.getChildAt(0), (DefaultPort) operatorModel.getSimpleTransContainer().getElementById(targets[l])
                                            .getChildAt(0));
                                }
                            }
            ...
            else if (targetModel.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE)
            {
                ...
                if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE)
                {
                     ...
                     */
                    if (getElementContainer().getSourceElements(operatorModel.getId()).size() == 1
                            && operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() == 1)
                    {
                        // get simple trans
                        TransitionModel simpleTrans = (TransitionModel) operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).values().iterator().next();
                ...
                else if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_SPLIT_TYPE || operatorModel.getOperatorType() == OperatorTransitionModel.OR_SPLIT_TYPE)
                {
                    ...
                    while (sourceIter.hasNext())
                    {
                        ...
                        while (simpleRootIter.hasNext())
                        {
                            ...
                            if (tempTarget.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE) operatorModel.addReference(getNexArcId(), (DefaultPort) tempSource.getChildAt(0),
