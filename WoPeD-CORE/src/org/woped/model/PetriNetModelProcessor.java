...
import org.woped.model.petrinet.ResourceClassModel;
import org.woped.model.petrinet.ResourceModel;
import org.woped.model.petrinet.Toolspecific;
 ...
 */
public class PetriNetModelProcessor extends AbstractModelProcessor implements WoPeDLogger, Serializable
{
    ...
    public AbstractElementModel createElement(CreationMap map)
    {
        ...
        if (map.isValid())
        {
            ...
            AbstractElementModel anElement = ModelElementFactory.createModelElement(map);
    ...
    public ArcModel createArc(String id, Object sourceId, Object targetId, Point2D[] points, boolean transformOperators)
    {
        ...
        ArcModel displayedArc = ModelElementFactory.createArcModel(getNexArcId(), (DefaultPort) sourceModel.getChildAt(0), (DefaultPort) targetModel.getChildAt(0));
        ...
        if (transformOperators)
        {
            ...
            if (sourceModel.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE)
            {
                ...
                logger.debug("Connection from Aalst Model detected... resolve Inner-Transitions ...");
                ...
                if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_SPLIT_TYPE)
                {
                    ...
                    // neuen Eintrag hat
                    if (getElementContainer().getTargetElements(operatorModel.getId()).size() == 1 && operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() == 1)
                    {
                        // get simple trans
                        TransitionModel simpleTrans = (TransitionModel) operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).values().iterator().next();
                    ...
                    } else
                    {
                        ...
                        // create simple trans
                        TransitionModel simpleTrans = operatorModel.addNewSimpleTrans();
                        ...
                        // transition
                        Iterator sourceIter = getElementContainer().getSourceElements(operatorModel.getId()).keySet().iterator();
                        while (sourceIter.hasNext())
                        {
                            operatorModel.addReference(getNexArcId(), (DefaultPort) getElementContainer().getElementById(sourceIter.next()).getChildAt(0), (DefaultPort) simpleTrans.getChildAt(0));
                        }
                    }
                ...
                else if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE)
                {
                    ...
                    Iterator simpleRootIter = operatorModel.getSimpleTransContainer().getRootElements().iterator();
                    while (simpleRootIter.hasNext())
                    {
                        PetriNetModelElement pec = (PetriNetModelElement) simpleRootIter.next();
                        if (pec.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE) operatorModel.addReference(getNexArcId(), pec.getPort(), targetModel.getPort());
                    }
                }
                ...
                else if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_SPLITJOIN_TYPE)
                {
                    ...
                    if (operatorModel.getSimpleTransContainer().getArcMap().size() > 0)
                    {
                        simpleTrans = operatorModel.addNewSimpleTrans();
                    } else
                    {
                        simpleTrans = (TransitionModel) operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).values().iterator().next();
                    }
                    PlaceModel centerPlace = operatorModel.getCenterPlace();
                    // centerplace with simpletrans
                    operatorModel.addReference(getNexArcId(), (DefaultPort) centerPlace.getChildAt(0), (DefaultPort) simpleTrans.getChildAt(0));
                ...
                {
                    int z = getElementContainer().getTargetElements(operatorModel.getId()).size();
                    // Wenn die Referenztabelle von dem Aalst Element nur diesen
                    // neuen Eintrag hat
                    if (z == 1 && operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() == 1)
                    {
                        // get simple trans
                    ...
                    {
                        logger.error("The Maximum for OR Split is 5 following places.");
                    } else
                    {

                        Iterator targetIter = getElementContainer().getTargetElements(operatorModel.getId()).keySet().iterator();
                        ...
                        for (int k = trans - newTrans; k < trans; k++)
                        {
                            ...
                            for (short l = 0; l < z; l++)
                            {
                                ...
                                {
                                    operatorModel.addReference(getNexArcId(), (DefaultPort) simpleTrans.getChildAt(0), (DefaultPort) operatorModel.getSimpleTransContainer().getElementById(targets[l]).getChildAt(0));
                                }
                            }
                            sourceIter = getElementContainer().getSourceElements(operatorModel.getId()).keySet().iterator();
                ...
                logger.debug("... Inner-Transition resolving completed");
            }
            /* IF OPERATOR IS TARGET */
            else if (targetModel.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE)
            {

                logger.debug("Connection to Aalst Model detected... resolve Inner-Transitions ...");
                ...
                if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE)
                {
                     ...
                     */
                    if (getElementContainer().getSourceElements(operatorModel.getId()).size() == 1 && operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() == 1)
                    {
                        // get simple trans
                        TransitionModel simpleTrans = (TransitionModel) operatorModel.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).values().iterator().next();
                        // dann füge nur die Reference hinzu
                        operatorModel.addReference(getNexArcId(), (DefaultPort) sourceModel.getChildAt(0), (DefaultPort) simpleTrans.getChildAt(0));
                    } else
                    {
                        ...
                        while (targetIter.hasNext())
                        {
                            operatorModel.addReference(getNexArcId(), (DefaultPort) simpleTrans.getChildAt(0), (DefaultPort) getElementContainer().getElementById(targetIter.next()).getChildAt(0));
                        }
                    }
                }
                ...
                else if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_SPLITJOIN_TYPE)
                {
                    ...
                    operatorModel.addReference(getNexArcId(), (DefaultPort) simpleTrans.getChildAt(0), (DefaultPort) centerPlace.getChildAt(0));
                    // source with simpletrans
                    operatorModel.addReference(getNexArcId(), (DefaultPort) sourceModel.getChildAt(0), (DefaultPort) simpleTrans.getChildAt(0));
                }
                /* IF target ist XOR SPLIT OR OR SPLIT */
                else if (operatorModel.getOperatorType() == OperatorTransitionModel.XOR_SPLIT_TYPE || operatorModel.getOperatorType() == OperatorTransitionModel.OR_SPLIT_TYPE)
                {
                    ...
                    {
                        Iterator simpleRootIter = operatorModel.getSimpleTransContainer().getRootElements().iterator();
                        AbstractElementModel tempSource = getElementContainer().getElementById(sourceIter.next());
                        while (simpleRootIter.hasNext())
                        {
                            // wenn simpletrans dann reference
                            PetriNetModelElement tempTarget = (PetriNetModelElement) simpleRootIter.next();
                            if (tempTarget.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE) operatorModel.addReference(getNexArcId(), (DefaultPort) tempSource.getChildAt(0), (DefaultPort) tempTarget.getChildAt(0));
                        }
                    }
                }
    ...
    public void removeArc(Object id)
    {
        ...
        if (arcToDelete != null)
        {
            if (getElementContainer().getElementById(arcToDelete.getSourceId()).getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE)
            {
                OperatorTransitionModel currentOperator = (OperatorTransitionModel) getElementContainer().getElementById(arcToDelete.getSourceId());
                ...
                {
                    if (currentOperator.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() != 1)
                    {
            ...
            {
                OperatorTransitionModel currentOperator = (OperatorTransitionModel) getElementContainer().getElementById(arcToDelete.getTargetId());
                ...
                if (currentOperator.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE)
                {
                    if (currentOperator.getSimpleTransContainer().getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE).size() != 1)
                    {
                        ...
                        logger.debug("INNER Target Elements deleted");
                    }
                }
                /* IF TARGET IS XOR SPLITJOIN */
                else if (currentOperator.getOperatorType() == OperatorTransitionModel.XOR_SPLITJOIN_TYPE)
                ...
                logger.debug("INNER ARC TO SOURCE deleted");
            }
    ...
    public String getNewElementId(int elementType)
    {
        ...
        } else if (elementType == PetriNetModelElement.TRANS_SIMPLE_TYPE || elementType == PetriNetModelElement.TRANS_OPERATOR_TYPE)
        {
    ...
    public void addResourceMapping(String resourceClass, String resourceId)
    {
        ...
        if (containsRole(resourceClass) != -1 || containsOrgunit(resourceClass) != -1)
        {
            if (containsResource(resourceId) != -1)
            {
                ...
                {
                    ((Vector) getResourceMapping().get(resourceClass)).add(resourceId);
                }
            } else
            {
                logger.warn("resource doesn't exist");
            }

        } else
        {
            logger.warn("role or orgUnit doesn't exist");
        }
    ...
    public Vector getResourceClassesResourceIsAssignedTo(String resourceId)
    {
        ...
        for (Iterator iter = getResourceMapping().keySet().iterator(); iter.hasNext();)
        {
            resourceClassIdTemp = iter.next().toString();
            Vector resourceVector = (Vector) getResourceMapping().get(resourceClassIdTemp);
    ...
    public int containsRole(String name)
    {
        ...
        {
            if (((ResourceClassModel) getRoles().get(i)).getName().equals(name)) return i;
    ...
    public int containsOrgunit(String name)
    {
        ...
        {
            if (((ResourceClassModel) getOrganizationUnits().get(i)).getName().equals(name)) return i;
    ...
    public int containsResource(String name)
    {
        ...
        {
            if (((ResourceModel) getResources().get(i)).getName().equals(name)) return i;
    ...
    public int containsResource(Vector resourceVector, String name)
    {
        ...
        {
            if (resourceVector.get(i).equals(name)) return i;
    ...
    public void replaceResourceMapping(String oldName, String newName)
    {
        for (Iterator iter = getResourceMapping().keySet().iterator(); iter.hasNext();)
        {
            Vector resourcevalues = (Vector) getResourceMapping().get(iter.next());
            for (int i = 0; i < resourcevalues.size(); i++)
            {
                if (resourcevalues.get(i).equals(oldName)) resourcevalues.set(i, newName);
            }
        }
    }
