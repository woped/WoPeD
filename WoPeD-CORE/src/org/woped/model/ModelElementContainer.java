 ...
 */
public class ModelElementContainer implements WoPeDLogger, Serializable
{
    ...
    public AbstractElementModel addElement(AbstractElementModel theElement)
    {
        if ((Map) getIdMap().get(theElement.getId()) == null)
        {
            ...
            logger.debug("Element: " + theElement.getId() + " added");
        } else
        {
            logger.debug("The Element already exists, did nothing!");
        }
        return theElement;
    }
    ...
    public void addReference(ArcModel arc)
    {
        ...
        if (getElementById(arc.getSourceId()) == null)
        {
            ...
            logger.warn("Source (ID:" + arc.getSourceId() + ") does not exist");
        } else if (getElementById(arc.getTargetId()) == null)
        {
            logger.warn("Target (ID:" + arc.getTargetId() + ") does not exist");
        } else if (containsArc(arc))
        {
            logger.debug("Arc already exists!");
        } else
        {
            ((Map) getIdMap().get(arc.getSourceId())).put(arc.getId(), arc);
            arcs.put(arc.getId(), arc);
            logger.debug("Reference: " + arc.getId() + " (" + arc.getSourceId() + " -> " + arc.getTargetId() + ") added.");
        }
    ...
    {
        getIdMap().remove(id);
        logger.debug("Element (ID:" + id + ") deleted.");
    }
    ...
    public void removeArcsFromElement(Object id)
    {
        ...
        logger.debug("All References from/to (ID:" + id + ") deleted");
    }
    ...
    public void removeArc(Object id)
    {
        ...
        } else logger.warn("Arc with ID: " + id + " does not exists");
    }
    ...
    public void removeArc(ArcModel arc)
    {
        if (arc != null)
        {
            logger.debug("Reference (ID:" + arc.getId() + ") deleted");
            // remove in arc Map
    ...
    public AbstractElementModel getElementById(Object id)
    {
        ...
        {
            logger.debug("Requested Element (ID:" + id + ") does not exists");
            return null;
        }
    ...
    public ArcModel getArcById(Object id)
    {
        ...
        {
            logger.debug(" Requested Arc (ID:" + id + ") does not exists");
            return null;
        }
    }
