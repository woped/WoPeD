...
import org.woped.qualanalysis.service.QualanalysisServiceImplementWoflan;
...
public class SideBar extends CloseableTabbedPane {
	 ...
	 */
	public SideBar(File temporaryFile, IEditor editor,
			AbstractApplicationMediator mediator, boolean autoRefreshStatus) {
		super();
		...
		this.addTab(Messages.getString("AnalysisSidebar.Tab.Beginner"),
				beginnerContainer);
		this.addTab(Messages.getString("AnalysisSidebar.Tab.Expert"),
				expertContainer);
	...
	public IQualanalysisService getQualanalysisService() {
		if(qualanService != null)
			...
			if (editor.isUseWoflanDLL())
				return new QualanalysisServiceImplementWoflan(editor, temporaryFile);
			else
