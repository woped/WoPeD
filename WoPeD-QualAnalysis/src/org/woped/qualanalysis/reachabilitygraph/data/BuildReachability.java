...
public class BuildReachability {
				...
				while (netIt.hasNext()) {
					...
					if (trans.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE
							&& (((OperatorTransitionModel) trans)
									...
									|| ((OperatorTransitionModel) trans)
											.getOperatorType() == OperatorTransitionModel.ANDJOIN_XORSPLIT_TYPE
									|| ((OperatorTransitionModel) trans)
											.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE
									|| ((OperatorTransitionModel) trans)
						...
						if (((OperatorTransitionModel) trans).getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE
							...
							outgoingIter = getPetriNet().getElementContainer()
									.getOutgoingArcs(trans.getId()).keySet()
									.iterator();
						}
	...
	private void checkTransition(TransitionModel transition) {
		...
		Map incomingArcs = getPetriNet().getElementContainer().getIncomingArcs(
				transition.getId());
		// temporary variables
		...
		while (incomingArcsIter.hasNext()) {
			...
			try {
				...
				LoggerManager.warn(Constants.QUALANALYSIS_LOGGER,
						"Reachability: Source not a Place. Ignore arc: "
								+ arc.getId());
			}
		}
		return activePlaces;
	...
	private void setIncomingArcsActive(Object transitionId, boolean active) {
		Iterator incomingArcsIter = getPetriNet().getElementContainer()
	...
	private void sendTokens(ArcModel arc) {
		try {
			...
			LoggerManager.warn(Constants.QUALANALYSIS_LOGGER,
					"Reachability: Cannot send token. Source is not a place. Ignore arc: "
							+ arc.getId());
		}
	}
	...
	private void receiveTokens(ArcModel arc) {
		try {
			...
			LoggerManager.warn(Constants.QUALANALYSIS_LOGGER,
					"Reachability: Cannot receive token. Target is not a place. Ignore arc: "
							+ arc.getId());
		}
	}

	private void setOutgoingArcsActive(Object transitionId, boolean active) {
		Iterator outgoingIter = getPetriNet().getElementContainer()
	...
	private void resetArcStatus() {
		Iterator arcIter = getPetriNet().getElementContainer().getArcMap()
				.keySet().iterator();
		while (arcIter.hasNext()) {
	...
	public void setStatus(Marking marking) {
		...
		Iterator placeIter = getPetriNet().getElementContainer()
				.getElementsByType(PetriNetModelElement.PLACE_TYPE).keySet()
				.iterator();
