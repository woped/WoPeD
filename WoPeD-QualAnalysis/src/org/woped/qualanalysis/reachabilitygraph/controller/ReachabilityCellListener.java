...
public class ReachabilityCellListener implements MouseListener, MouseWheelListener {
    ...
    private void setTokens(ReachabilityPlaceModel place) {
        ...
        SortedMap<String, Integer> tokenMap = marking.getMarking();
        Collection<PlaceModel> places = getPetriNet().getElementContainer().getPlaces();

        for (PlaceModel p : places) {
            String id = p.getId();
            int tokens = marking.isPlaceUnbound(id)? 100 : tokenMap.get(id);
            p.setVirtualTokens(tokens);
        }
    }
