...
public class TokenGameController {
	...
	public void start() {
		...
		if (thisEditor.isSubprocessEditor()) {
			...
			allTransitions = getPetriNet().getElementContainer()
					.getElementsByType(PetriNetModelElement.TRANS_SIMPLE_TYPE);
			allTransitions
					.putAll(getPetriNet().getElementContainer()
							.getElementsByType(
		...
		for (int i = 0; i < refer.getDesktopReference().getComponentCount(); i++) {
			if (a[i] instanceof ReachabilityGraphVC) {
				...
				if (rvc.hasEditor(thisEditor)) {
					...
					((ReachabilityJGraph) rvc.getJGraph(thisEditor))
							.deHighlight();
				}
				break;
			}
	...
	private void resetVirtualTokensInElementContainer(
		...
		// restore origin tokencount
		Iterator<String> placeIter = container.getElementsByType(
				PetriNetModelElement.PLACE_TYPE).keySet().iterator();
		while (placeIter.hasNext()) {
			((PlaceModel) container.getElementById(placeIter.next()))
					.resetVirtualTokens();
		}
		Iterator<String> subpIter = container.getElementsByType(
	...
	private void checkNet() {
		...
		while (transIter.hasNext()) {
			checkTransition((TransitionModel) allTransitions.get(transIter
					.next()));
		}
		...
		if ((petrinet.getElementContainer().getOwningElement() != null)
				&& (sinkPlaces != null)) {
			Iterator<PlaceModel> i = sinkPlaces.iterator();
			while (i.hasNext()) {
				PlaceModel currentSink = i.next();
				currentSink
						.setActivated(currentSink.getVirtualTokenCount() > 0);
			}
		}
		getGraph().updateUI();
	...
	private void setMarkingInRG(IMarking mark) {
		...
		for (int i = 0; i < ParentControl.getDesktopReference()
				.getComponentCount(); i++) {
			if (a[i] instanceof ReachabilityGraphVC) {
				ReachabilityGraphVC rvc = (ReachabilityGraphVC) a[i];
				if (rvc.hasEditor(thisEditor)) {
					((ReachabilityJGraph) rvc.getJGraph(thisEditor))
							.HighlightMarking(mark);
				}
				break;
			}
		}
	...
	private void checkTransition(TransitionModel transition) {
		...
		Map<String, ArcModel> incomingArcs = getPetriNet()
				.getElementContainer().getIncomingArcs(transition.getId());
		Map<String, Object> outgoingArcs = getPetriNet().getElementContainer()
				.getOutgoingArcs(transition.getId());
		// temporary variables

		if (transition.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE
				|| transition.getType() == PetriNetModelElement.SUBP_TYPE) {
			...
			if (operator.getOperatorType() == OperatorTransitionModel.AND_JOIN_TYPE
				...
				if (countIncomingActivePlaces(incomingArcs) == incomingArcs
						.size()) {
					transition.setActivated(true);
				...
				if (countIncomingActivePlaces(incomingArcs) == incomingArcs
					...
					while (outArcs.hasNext()) {
						...
						helpPlace = getPetriNet().getElementContainer()
								.getElementById(
										getPetriNet().getElementContainer()
												.getArcById(ID).getTargetId());
						XorName = transition.getNameValue() + " -> ("
								+ helpPlace.getNameValue() + ")";
				...
				if (countIncomingActivePlaces(incomingArcs) > 0) {
					...
					while (inArcs.hasNext()) {
						...
						if (getPetriNet().getElementContainer().getArcById(ID)
								.isActivated()) {
							// Use a new CreationMap with the arcs id to create the virtual transition
							CreationMap map = CreationMap.createMap();
							map.setId(ID);
	...
	private void transitionClicked(TransitionModel transition, MouseEvent e) {
		if (transition.isActivated()) {
			...
			if (transition.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE
				...
				// simple Transition:
				// "+transition.getId());
				receiveTokens(getPetriNet().getElementContainer()
						.getOutgoingArcs(transition.getId()));
				sendTokens(getPetriNet().getElementContainer().getIncomingArcs(
						transition.getId()));
				if (transition.getType() == PetriNetModelElement.SUBP_TYPE) {
				...
				if (operator.getOperatorType() == OperatorTransitionModel.AND_JOIN_TYPE
					...
					receiveTokens(getPetriNet().getElementContainer()
							.getOutgoingArcs(transition.getId()));
					sendTokens(getPetriNet().getElementContainer()
							.getIncomingArcs(transition.getId()));
					actionPerformed = true;
	...
	private void arcClicked(ArcModel arc) {
		...
		if (arc.isActivated()) {
			PetriNetModelElement source = (PetriNetModelElement) getPetriNet()
					.getElementContainer().getElementById(arc.getSourceId());
			PetriNetModelElement target = (PetriNetModelElement) getPetriNet()
					.getElementContainer().getElementById(arc.getTargetId());
			...
			if (source.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE) {
				...
				if (tempOperator.isFireing()) {
					if (tempOperator.getOperatorType() == OperatorTransitionModel.XOR_SPLIT_TYPE
						...
						if (tempOperator.getOperatorType() != OperatorTransitionModel.XOR_SPLITJOIN_TYPE)
							sendTokens(getPetriNet().getElementContainer()
									.getIncomingArcs(tempOperator.getId()));
						else {
				...
				if (tempOperator.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE
					...
					if (tempOperator.getOperatorType() != OperatorTransitionModel.XOR_SPLITJOIN_TYPE)
						receiveTokens(getPetriNet().getElementContainer()
								.getOutgoingArcs(tempOperator.getId()));
					else {
			...
			for (int i = 0; i < RemoteControl
					.getFollowingActivatedTransitions().size(); i++) {
				helpTransitionReference = (TransitionModel) RemoteControl
						.getFollowingActivatedTransitions().get(i);
				if (arc.getId() == helpTransitionReference.getId()) {
					...
					if (RemoteControl.isRecordSelected()) {
						...
						RemoteControl
								.addHistoryListItem(helpTransitionReference);
					}
				}
			}
			RemoteControl.cleanupTransition();
	...
	private void backwardItem(TransitionModel transition, ArcModel arc) {
		...
		if (transition != null) {
			receiveBackwardTokens(getPetriNet().getElementContainer()
					.getIncomingArcs(transition.getId()));
			sendBackwardTokens(getPetriNet().getElementContainer()
					.getOutgoingArcs(transition.getId()));
			actionPerformed = true;
	...
	private int countIncomingActivePlaces(Map<String, ArcModel> arcsFromPlaces) {
		...
		while (incomingArcsIter.hasNext()) {
			ArcModel arc = getPetriNet().getElementContainer().getArcById(
					incomingArcsIter.next());
			try {
				PlaceModel place = (PlaceModel) getPetriNet()
						.getElementContainer()
				...
				LoggerManager.warn(Constants.QUALANALYSIS_LOGGER,
						"TokenGame: Source not a Place. Ignore arc: "
								+ arc.getId());
			}
		}
		return activePlaces;
	...
	private void setIncomingArcsActive(Object transitionId, boolean active) {
		Iterator<String> incomingArcsIter = getPetriNet().getElementContainer()
				.getIncomingArcs(transitionId).keySet().iterator();
		while (incomingArcsIter.hasNext()) {
			ArcModel arc = getPetriNet().getElementContainer().getArcById(
					incomingArcsIter.next());
			if (active) {
	...
	private void setOutgoingArcsActive(Object transitionId, boolean active) {
		Iterator<String> outgoingIter = getPetriNet().getElementContainer()
				.getOutgoingArcs(transitionId).keySet().iterator();
		while (outgoingIter.hasNext()) {
			getPetriNet().getElementContainer().getArcById(outgoingIter.next())
					.setActivated(active);
		}
	}
	...
	private void sendTokens(ArcModel arc) {
		try {
			PlaceModel place = (PlaceModel) getPetriNet().getElementContainer()
					.getElementById(arc.getSourceId());
			if (place != null) {
				place.sendToken();
	...
	private void resetArcStatus() {
		Iterator<String> arcIter = getPetriNet().getElementContainer()
	...
	private class MouseHandler implements MouseListener {
		...
		public void mousePressed(final MouseEvent e) {
			TransitionModel transition = findTransitionInCell(getGraph()
					.getAllCellsForLocation(e.getPoint().x, e.getPoint().y));
		...
		public void mouseReleased(MouseEvent e) {
			...
			Vector<Object> allCells = getGraph().getAllCellsForLocation(
	...
	public void occurTransitionbyTokenGameBarVC(TransitionModel transition,
		...
		if (checkA == 'a') {
			if (BackWard) {
				backwardItem(null, getPetriNet().getElementContainer()
						.getArcById(transition.getId()));
			} else {
				arcClicked(getPetriNet().getElementContainer().getArcById(
						transition.getId()));
			}
		} else if (checkA == 'S') {
			thisEditor.openTokenGameSubProcess((SubProcessModel) transition);
