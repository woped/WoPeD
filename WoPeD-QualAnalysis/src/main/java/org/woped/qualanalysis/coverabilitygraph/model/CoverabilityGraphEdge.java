...
/**
 * This class represents the connection between 2 reachable markings when firing a specific
 * transition.
 */
public class CoverabilityGraphEdge extends DefaultEdge {
   ...
   */
  public CoverabilityGraphEdge(
      CoverabilityGraphNode from, CoverabilityGraphNode to, TransitionNode trigger) {
    super(null);

    if (from != null) this.setSource(from.getFirstChild());

    if (to != null) this.setTarget(to.getFirstChild());

    this.triggers =
        new TreeSet<>(
            new Comparator<TransitionNode>() {
  ...
  public String getToolTipText() {
    ...
    sb.append(
        String.format(
            "<html><h4>%s</h4><table><tr><th>ID</th><th>Name</th></tr>",
   ...
   *
   * <p>This can be used to combine edges if the only differ in the trigger (same source and
   * target).
   *
   * @param trigger the transition that triggers the edge
   */
  ...
  private void installAttributes() {
    ...
    GraphConstants.setLabelPosition(
        attributes, new Point2D.Double(GraphConstants.PERMILLE * 0.33, 0));
  ...
  public static class CgRouting extends DefaultEdge.DefaultRouting {
    protected java.util.List routeLoop(GraphLayoutCache cache, EdgeView edge) {
      ...
      if (sourceParent != null) {
        ...
        newPoints.add(
            edge.getAttributes()
                .createPoint(from.getX() - loopWidth, from.getY() + height2 + loopHeight * 1.2));
        newPoints.add(
            edge.getAttributes()
                .createPoint(from.getX(), from.getY() + height2 + 1.5 * loopHeight));
        newPoints.add(
            edge.getAttributes()
                .createPoint(from.getX() + loopWidth, from.getY() + height2 + loopHeight * 1.2));
