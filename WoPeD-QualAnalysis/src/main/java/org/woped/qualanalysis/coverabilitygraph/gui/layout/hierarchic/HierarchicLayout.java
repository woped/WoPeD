...
public class HierarchicLayout extends CoverabilityGraphLayoutBase {
  ...
  private void applyHierarchicLayout(CoverabilityGraph graph) {
    ...
    CoverabilityGraphNode initialPlace =
        AbstractReachabilityGraphModel.lookupInitialMarking(markings);
    if (initialPlace != null) {
      ...
      hierarcher(
          initialPlace,
          new Rectangle2D.Double(10, 0, bounds.getWidth(), bounds.getHeight()),
          horizontalSpace,
          verticalSpace,
          toProof);

      LinkedList<CoverabilityGraphNode> toEdit =
          (LinkedList<CoverabilityGraphNode>) toProof.clone();
  ...
  @SuppressWarnings("unchecked")
  private LinkedList<CoverabilityGraphNode> hierarcher(
      CoverabilityGraphNode place,
      Rectangle2D bounds,
      int horizontalSpace,
      int verticalSpace,
      LinkedList<CoverabilityGraphNode> places) {
    ...
    while (edgesIter.hasNext()) {
      ...
      if ((childPlace != null) && !childPlace.isSetRecursiveBounds()) {
        ...
        GraphConstants.setBounds(
            childPlace.getAttributes(),
            new Rectangle2D.Double(
                25 + bounds.getX() + ((puffer + bounds.getWidth()) * edgeCount++),
                bounds.getY() + bounds.getHeight() + verticalSpace,
                bounds.getWidth(),
                bounds.getHeight()));
  ...
  private void hierarcherProofer(LinkedList<CoverabilityGraphNode> places, int horizontalSpace) {
    if (places.size() > 0) {
      ...
      while (iter.hasNext()) {
        ...
        if (GraphConstants.getBounds(actual.getAttributes()).getX()
                == GraphConstants.getBounds(first.getAttributes()).getX()
            && GraphConstants.getBounds(actual.getAttributes()).getY()
                == GraphConstants.getBounds(first.getAttributes()).getY()) {
          // found one !
          ...
          GraphConstants.setBounds(
              ...
              new Rectangle2D.Double(
                  bounds.getX() + horizontalSpace + bounds.getWidth(),
                  bounds.getY(),
                  bounds.getWidth(),
                  bounds.getHeight()));
          changedOne = true;
  ...
  private CoverabilityGraphPort getOtherPort(
      CoverabilityGraphNode place, CoverabilityGraphEdge edge) {
    List<?> ports = place.getChildren();
