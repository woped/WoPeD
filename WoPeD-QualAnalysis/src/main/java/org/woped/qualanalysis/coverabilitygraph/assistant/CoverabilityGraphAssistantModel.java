 ...
 *
 * <p>It can either be used directly by calling the methods to add/modify/remove nodes or edges, or
 * the graph listener can be used do react on the related graph events.
 */
public class CoverabilityGraphAssistantModel extends AbstractReachabilityGraphModel {
  ...
  public void reconnectEdge(
      CoverabilityGraphEdge edge, CoverabilityGraphNode newDestination, boolean sourceReconnected) {
    ...
    if ((newDestination.equals(edge.getSourceNode()) && !sourceReconnected)
        || (newDestination.equals(edge.getTargetNode()) && sourceReconnected)) {
  ...
  /**
   * Adds, updates and removes all elements that has been added/modified/removed since the last call
   * of this method and applies the configured layout to the graph afterwards.
   */
  public void refresh() {
    insertElements();
    updateElements();
  ...
  /**
   * Gets the graph listener that can be added to an {@link CoverabilityGraphAssistant} to call the
   * appropriate methods automatically when graph events occur.
   *
  ...
  protected void initialize() {
    ...
    AbstractLowLevelPetriNetBuilder builder =
        BuilderFactory.createLowLevelPetriNetWithoutTStarBuilder(getEditor());
