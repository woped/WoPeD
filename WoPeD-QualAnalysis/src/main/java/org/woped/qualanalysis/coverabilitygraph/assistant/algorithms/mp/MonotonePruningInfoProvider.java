...
/**
 * This class creates the textual representation of the node analysis steps that are displayed in
 * the sidebar of the assistant view.
 */
class MonotonePruningInfoProvider {
   ...
   */
  MonotonePruningInfoProvider(
      MonotonePruningEventTrigger eventTrigger, SidebarVC sidebar, NodeFormatter nodeFormatter) {
  ...
  private class AnalysisListener extends MonotonePruningAnalysisAdapter {
    ...
    public void nodeSelected(NodeEvent event) {
      ...
      if (node.getState() == MpNodeState.UNPROCESSED) {
        ...
        String processed =
            Messages.getString("CoverabilityGraph.Assistant.MP.NodeInfo.ProcessStep");
        ...
        if (node.getState() == MpNodeState.INACTIVE) {
          ...
          String deactivationStep =
              Messages.getString("CoverabilityGraph.Assistant.MP.NodeInfo.DeactivationStep");
          view.addDetail(deactivationStep, 2, 0);
          view.addDetail(String.valueOf(node.getDeactivatedInStep()), 2, 1);

          String deactivationNode =
              Messages.getString("CoverabilityGraph.Assistant.MP.NodeInfo.DeactivationNode");
    ...
    public void parentCheckComplete(ParentCheckCompletedEvent event) {
      ...
      if (event.getParent() == null) {
        ...
        String neg =
            event.isParentActive()
                ? ""
                : Messages.getString("CoverabilityGraph.Assistant.MP.ParentCheck.Neg");
        String format = Messages.getString("CoverabilityGraph.Assistant.MP.ParentCheck.MsgFormat");
        String marking =
            nodeFormatter
                .getNodeTextFormatter()
                .getMarkingFormatter()
                .getText(event.getParent().getMarking());
    ...
    public void omegaCheckCompleted(OmegaCheckEvent event) {
      ...
      String prevMarkingLabel =
          Messages.getString("CoverabilityGraph.Assistant.MP.OmegaCheck.CalculatedMarking");
      view.addDetail(prevMarkingLabel, 0, 0);

      String prevMarkingValue =
          nodeFormatter
              .getNodeTextFormatter()
              .getMarkingFormatter()
              .getText(event.getPreviousMarking());
      view.addDetail(prevMarkingValue, 0, 1);

      String smallerMarkingsLabel =
          Messages.getString("CoverabilityGraph.Assistant.MP.OmegaCheck.SmallerMarkings");
      view.addDetail(smallerMarkingsLabel, 1, 0);

      int row = 1;
      for (MpNode n : event.getSmallerNodes()) {
        String marking =
            nodeFormatter.getNodeTextFormatter().getMarkingFormatter().getText(n.getMarking());
      ...
      String resultingMarkingLabel =
          Messages.getString("CoverabilityGraph.Assistant.MP.OmegaCheck.ResultingMarking");
      ...
      String resultingMarkingValue =
          nodeFormatter
              .getNodeTextFormatter()
              .getMarkingFormatter()
              .getText(event.getResultingMarking());
    ...
    public void coverCheckCompleted(CoverCheckCompletedEvent event) {
      ...
      String largerNodesLabel =
          Messages.getString("CoverabilityGraph.Assistant.MP.CoverCheck.LargerNodes");
    ...
    public void deactivationCheckCompleted(DeactivationCheckCompletedEvent event) {
      ...
      String headerText =
          Messages.getString("CoverabilityGraph.Assistant.MP.DeactivationCheck.Header");
      view.setHeaderText(headerText);

      String deactivatedNodesLabel =
          Messages.getString("CoverabilityGraph.Assistant.MP.DeactivationCheck.DeactivatedNodes");
    ...
    public void descendantsCheckCompleted(DescendantsCheckCompletedEvent event) {
      ...
      String headerText =
          Messages.getString("CoverabilityGraph.Assistant.MP.DescendantsCheck.Header");
      view.setHeaderText(headerText);

      String activeTransitionsLabel =
          Messages.getString("CoverabilityGraph.Assistant.MP.DescendantsCheck.ActiveTransitions");
    ...
    public void nodeProcessingFinished(NodeEvent event) {
      ...
      String title =
          Messages.getString("CoverabilityGraph.Assistant.MP.NodeAnalysisFinished.Header");
      ProgressDetailsView view = new ProgressDetailsView();
      view.setHeaderText(title);

      String message =
          Messages.getString("CoverabilityGraph.Assistant.MP.NodeAnalysisFinished.Description");
