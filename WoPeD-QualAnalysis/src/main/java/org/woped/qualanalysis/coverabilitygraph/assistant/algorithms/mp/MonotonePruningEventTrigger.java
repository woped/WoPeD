...
class MonotonePruningEventTrigger {
   ...
   */
  void fireEdgeReconnectedEvent(
      CoverabilityGraphEdge edge, CoverabilityGraphNode oldNode, CoverabilityGraphNode newNode) {
  ...
  /**
   * Fires an event that there are no more changes in this analysis step and the listeners should
   * refresh their output.
   */
   ...
   */
  void fireOmegaCheckCompletedEvent(
      IMarking oldMarking, IMarking newMarking, Set<MpNode> smallerNodes) {
   ...
   *
   * @param largerNodes the active nodes whose marking covers the marking of the currently processed
   *     node
   */
  ...
  /**
   * Fires the event, that the deactivation check of the currently processed node has been
   * completed.
   *
   * @param deactivatedNodes the active nodes, that has been deactivated by the currently processed
   *     node.
   */
  void fireDeactivationCheckCompleted(Set<MpNode> deactivatedNodes) {
    DeactivationCheckCompletedEvent event =
        new DeactivationCheckCompletedEvent(source, deactivatedNodes);
   ...
   *
   * @param activatedTransitions the enabled transitions in the marking of the currently processed
   *     node
   */
  void fireDescendantsCheckCompletedEvent(Collection<TransitionNode> activatedTransitions) {
    DescendantsCheckCompletedEvent event =
        new DescendantsCheckCompletedEvent(source, activatedTransitions);
