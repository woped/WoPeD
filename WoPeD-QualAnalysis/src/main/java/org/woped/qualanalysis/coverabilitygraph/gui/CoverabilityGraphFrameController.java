 ...
 *
 * <p>It is a WoPeD design decision that maximum one coverability graph is visible at the same time.
 * That is why this class has been implemented as singleton.
 *
 * <p>This class holds a view controller for each coverability graph that has been generated in the
 * current session. It is responsible for displaying the related graph when an editor frame has been
 * activated and displaying dialogs related to the coverability graph.
 */
public class CoverabilityGraphFrameController extends JInternalFrame {
  ...
  /**
   * Highlight all markings which covers the provide marking in the coverability graph of the
   * provided petri net.
   *
   * @param editor the editor of the petri net
  ...
  void enableAssistantView() {
    ...
    if (!activeGraph.containsView(CoverabilityGraphAssistantView.VIEW_NAME)) {
      CoverabilityGraphAssistantView assistantView =
          new CoverabilityGraphAssistantView(activatedNet);
    ...
    switch (fileFilter.getFilterType()) {
      ...
      default:
        LoggerManager.warn(
            ...
            "Unable to save File. Unknown file type: " + fileFilter.getFilterType());
    }
  ...
  private void createViewController(IEditor editor) {
    ...
    if (editorFrame != null) editorFrame.addInternalFrameListener(new EditorFrameListener(editor));
  ...
  private void showTokenGameRunningWarning() {
    ...
    CoverabilityGraphWarning.showReachabilityWarning(
        desktop, "QuanlAna.ReachabilityGraph.SimulationWarning");
  }

  private void showGraphOutOfSyncWarning() {
    //noinspection SpellCheckingInspection
    CoverabilityGraphWarning.showReachabilityWarning(
        this, "QuanlAna.ReachabilityGraph.RefreshWarning");
  }
