...
/**
 * This class guides the user through the process of building the minimal coverability graph (set)
 * introduced by Pierre-Alain Reynier and Frederic Servais in their paper Minimal Coverability Set
 * for Petri Nets: Karp and Miller Algorithm with Pruning
 */
public class CoverabilityGraphAssistantUsingMonotonePruning implements CoverabilityGraphAssistant {
   ...
   */
  public CoverabilityGraphAssistantUsingMonotonePruning(
      IEditor editor, SidebarVC sidebarVC, CoverabilityGraphAssistantModel graphModel) {
    eventTrigger = new MonotonePruningEventTrigger(this);
    builder = new MonotonePruningGraphBuilder(editor, eventTrigger);
    stateManager =
        new MonotonePruningVisualStateManager(
            eventTrigger, graphModel.getViewFactory().getNodeFormatter());
    infoProvider =
        new MonotonePruningInfoProvider(
            eventTrigger, sidebarVC, graphModel.getViewFactory().getNodeFormatter());
