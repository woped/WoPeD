...
/**
 * An abstract adapter for receiving analysis events. All methods in this class are empty. The class
 * exists for convenience for creating listener objects.
 *
 * <p>Analysis events let you track the current state of the analysis of an coverability graph node
 * and provide the necessary information to create a textual description or to highlight the
 * coverability graph.
 *
 * <p>Extend this class to create a {@link MonotonePruningAnalysisListener} and override the methods
 * for the events of interest. Add an instance of your class to the {@link
 * org.woped.qualanalysis.coverabilitygraph.assistant.algorithms.mp.MonotonePruningEventTrigger#addAnalysisListener(MonotonePruningAnalysisListener)}
 * method. The relevant methods are invoked and the event object is passed to it when an event
 * occurs.
 */
public abstract class MonotonePruningAnalysisAdapter implements MonotonePruningAnalysisListener {
   ...
   *
   * <p>The possible relation are defined in the enum {@link NodeRelatedEvent.Relation}.
   *
   * @param event the event object containing additional information
   */
  @Override
   ...
   *
   * <p>This test checks if the direct ancestor of the node is still active. If it is, the analysis
   * continues, otherwise the node is deactivated and the analysis is finished.
   *
   ...
   *
   * <p>This checks if active nodes are covered by the new created node. If so the marking of the
   * new node is replaced by an omega marking.
   *
   ...
   *
   * <p>This test checks if an active node covers the new created node. If so, the new node is set
   * inactive an tha analysis is finished.
   *
   ...
   *
   * <p>This test checks, if the new node covers any active nodes or their ancestors.
   *
   ...
   *
   * <p>This test checks if the marking if there exists any activated transitions. If so it adds the
   * resulting markings to the unprocessed nodes.
   *
