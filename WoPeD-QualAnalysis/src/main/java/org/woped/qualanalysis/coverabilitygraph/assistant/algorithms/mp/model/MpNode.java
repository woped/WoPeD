...
/**
 * This class is represents a node in the coverability graph built with the monotone pruning
 * algorithm.
 *
 * <p>It basically extends with a state and information about the process.
 */
public class MpNode extends CoverabilityGraphNode {
  ...
  public void setState(MpNodeState newState) {
    ...
    if (currentState != newState) notifyStateChangedListeners(currentState, newState);
  }
  ...
  public MpNode getParentNode() {
    ...
    if (directAncestors.size() > 1)
      throw new IllegalStateException("There is more than one parent.");
  ...
  public int getProcessedInStep() throws IllegalStateException {
    if (processedInStep == -1)
      throw new IllegalStateException("The node has not been processed yet.");
  ...
  public int getDeactivatedInStep() {
    if (deactivatedInStep < 0)
      throw new IllegalStateException("The node has not been deactivated yet");
   ...
   *
   * @param node the node that has caused the deactivation or null if the has not been deactivated
   *     yet.
   */
