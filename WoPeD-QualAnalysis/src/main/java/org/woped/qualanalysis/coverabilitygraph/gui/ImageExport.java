    ...
    if (cells.length > 0) {
      ...
      if (dimension.width < MAX_WIDTH && dimension.height < MAX_HEIGHT) {
        ...
        JOptionPane.showMessageDialog(
            null,
            Messages.getString("QuanlAna.ReachabilityGraph.ExportFailed"),
            Messages.getString("QuanlAna.ReachabilityGraph.ExportFailed.Title"),
  ...
  static boolean saveJPG(RenderedImage image, File file) {
    if (image != null) {
      try {
        ...
        LoggerManager.info(
            Constants.QUALANALYSIS_LOGGER, "File saved to: " + file.getAbsolutePath());
        return true;
      } catch (IOException e) {
        e.printStackTrace();
