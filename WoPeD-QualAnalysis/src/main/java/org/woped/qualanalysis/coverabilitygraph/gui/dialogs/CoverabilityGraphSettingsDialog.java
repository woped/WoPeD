/**
 * ReachabilityGraph implementation was done by Manuel Fladt and Benjamin Geiger. The code was
 * written for a project at BA Karlsruhe in 2007/2008 under authority of Prof. Dr. Thomas Freytag
 * and Andreas Eckleder.
 *
 * <p>This class was written by
 *
...
public class CoverabilityGraphSettingsDialog extends JDialog {
  ...
  public CoverabilityGraphSettingsDialog(CoverabilityGraphVC graphVC) {
    ...
    Point location =
        new Point(
            (int) rgp.getLocationOnScreen().getX() + rgp.getWidth() / 2 - width / 2,
  ...
  private void initComponents() {
    ...
    JLabel graphVisual =
        new JLabel(
            "<html><b>"
                + Messages.getString("QuanlAna.ReachabilityGraph.Settings.GraphSection")
                + "</b></html>");
    graphVisual.setBounds(new Rectangle(20, 15, 220, 16));

    JLabel placeStyleHeader =
        new JLabel(Messages.getString("CoverabilityGraph.SettingsDialog.MarkingNotation.Header"));
    placeStyleHeader.setBounds(new Rectangle(20, 40, 180, 16));

    multiSetOption =
        new JRadioButton(
            Messages.getString("CoverabilityGraph.SettingsDialog.MarkingNotation.MultiSet"));
    ...
    tokenVectorOption =
        new JRadioButton(
            Messages.getString("CoverabilityGraph.SettingsDialog.MarkingNotation.TokenVector"));
    ...
    JLabel colorSchemeHeader =
        new JLabel(Messages.getString("CoverabilityGraph.SettingsDialog.ColorScheme.Header"));
    colorSchemeHeader.setBounds(new Rectangle(20, 90, 180, 16));

    grayGraphRb =
        new JRadioButton(Messages.getString("QuanlAna.ReachabilityGraph.Settings.Color.Grayscale"));
    grayGraphRb.setBounds(new Rectangle(30, 110, 90, 22));

    colorGraphRb =
        new JRadioButton(Messages.getString("QuanlAna.ReachabilityGraph.Settings.Color.Colored"));
    ...
    String notSupportedText =
        Messages.getString("QuanlAna.ReachabilityGraph.Settings.Color.NotSupported");
    JLabel colorSchemesNotSupportedLabel =
        new JLabel(String.format("<html><p><small>%s</small></p></html>", notSupportedText));
    colorSchemesNotSupportedLabel.setBounds(20, 110, 180, 22);
    colorSchemesNotSupportedLabel.setForeground(Color.DARK_GRAY);

    parallelRoutingCb =
        new JCheckBox(Messages.getString("QuanlAna.ReachabilityGraph.Settings.ParallelRouting"));
    parallelRoutingCb.setSelected(getParallelRoutingEnabled());
    parallelRoutingCb.setBounds(new Rectangle(30, 140, 180, 22));

    placeHeightLabel =
        new JLabel(Messages.getString("QuanlAna.ReachabilityGraph.Settings.Place.Height"));
    ...
    placeWidthLabel =
        new JLabel(Messages.getString("QuanlAna.ReachabilityGraph.Settings.Place.Width"));
    ...
    hierarchicLabel =
        new JLabel(
            "<html><b>"
                + Messages.getString("QuanlAna.ReachabilityGraph.Settings.HierarchicSection")
                + "</b></html>");
    hierarchicLabel.setBounds(new Rectangle(20, 240, 200, 22));

    hierarchicSpaceHorizontalLabel =
        new JLabel(Messages.getString("QuanlAna.ReachabilityGraph.Settings.Hierarchic.Horizontal"));
    ...
    hierarchicSpaceVerticalLabel =
        new JLabel(Messages.getString("QuanlAna.ReachabilityGraph.Settings.Hierarchic.Vertical"));
  ...
  private void setParallelRoutingEnabled(boolean enabled) {
    ...
    if (enabled)
      settings.edgeRouting =
          ParallelRouter.getSharedInstance(
              rgp.getActiveView().getGraphModel().getGraph().getGraphLayoutCache());
  ...
  private boolean getColored() {
    return this.settings.colorSchemeSupported
        && this.settings.colorScheme.equals(CoverabilityGraphColorScheme.COLORED_SCHEME());
  }
  ...
  private String getMarkingNotation() {
    if (this.settings.markingFormatter instanceof MultiSetMarkingFormatter)
      return NOTATION_MULITISET;
  ...
  private class SaveButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (integerValueChecker(
          CoverabilityGraphSettingsDialog.this.hierarchicSpaceHorizontalTf.getText())) {
      ...
      if (integerValueChecker(
          CoverabilityGraphSettingsDialog.this.hierarchicSpaceVerticalTf.getText())) {
      ...
      if (integerValueChecker(CoverabilityGraphSettingsDialog.this.placeWidthTf.getText())) {
        ...
        int newValue =
            Integer.parseInt(CoverabilityGraphSettingsDialog.this.placeWidthTf.getText());
        if (oldValue != newValue) {
      ...
      if (integerValueChecker(CoverabilityGraphSettingsDialog.this.placeHeightTf.getText())) {
        if (getPlaceHeight()
            != Integer.parseInt(CoverabilityGraphSettingsDialog.this.placeHeightTf.getText())) {
          setPlaceHeight(
              Integer.parseInt(CoverabilityGraphSettingsDialog.this.placeHeightTf.getText()));
        }
      } else {
      ...
      if (getParallelRoutingEnabled()
          != CoverabilityGraphSettingsDialog.this.parallelRoutingCb.isSelected()) {
        setParallelRoutingEnabled(
            CoverabilityGraphSettingsDialog.this.parallelRoutingCb.isSelected());
      }
