...
class GraphLayout {
  ...
  /**
   * Calculates the positions of the nodes according to their size and their first occurrence in the
   * graph.
   *
  ...
  private void registerNodes(int level) {
    ...
    for (CoverabilityGraphNode node : thisLevel.elements) {
      ...
      thisLevel.width =
          thisLevel.width == 0 ? nodeWidth : thisLevel.width + horizontalGap + nodeWidth;
