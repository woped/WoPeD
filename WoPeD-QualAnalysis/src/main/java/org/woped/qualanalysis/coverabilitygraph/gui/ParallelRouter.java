...
public class ParallelRouter extends LoopRouting {

  private static final long serialVersionUID = 7198873066070511317L;

  /** Singleton to reach parallel edge router */
  protected static final ParallelRouter sharedInstance = new ParallelRouter();
   ...
   */
  public static ParallelRouter getSharedInstance(GraphLayoutCache cache) {
    ParallelRouter.cache = cache;
    return ParallelRouter.sharedInstance;
  }

  private static GraphLayoutCache cache;
  ...
  public List<Object> routeEdge(EdgeView edge) {
    ...
    if ((null == nodeFrom) || (null == nodeTo)) {
      // LoggerManager.debug(Constants.QUALANALYSIS_LOGGER, "-- route() " +
      // this.getClass().getName() + "EdgeView has no source or target view : " + edge.toString());
      return null;
    }
    ...
    if (edges.length >= 2) {
      ...
      if (edge.getSource() == null || edge.getSource().getParentView() == null) {
        from =
            nodeFrom.getPerimeterPoint(
                edge, AbstractCellView.getCenterPoint(nodeFrom), perimeterPoint);
      } else {
        from =
            edge.getSource()
                .getParentView()
      ...
      if (edge.getTarget() == null
          || edge.getTarget().getParentView() == null) { // INV: nodeTo != null
      ...
      if (from != null && to != null) {
        ...
        double sizeFrom =
            Math.max(nodeFrom.getBounds().getWidth(), nodeFrom.getBounds().getHeight()) / 2.;
        double sizeTo =
            Math.max(nodeTo.getBounds().getWidth(), nodeTo.getBounds().getHeight()) / 2.;

        // Calc position of central point
        double edgeMiddleDeparture =
            (Math.sqrt(dx * dx + dy * dy) - sizeFrom - sizeTo) / 2 + sizeFrom;
            ...
            if (allAttributes != null) {
              ...
              if (lineStyle == GraphConstants.STYLE_BEZIER
                ...
                GraphConstants.setLabelPosition(
                    allAttributes, new Point2D.Double(x, r * edgeMiddleDeparture / 79));
              } else {
                // TODO, correct this positioning
                GraphConstants.setLabelPosition(allAttributes, new Point2D.Double(x, r));
              }
            }
          }
        }
  ...
  public static double getEdgeSeparation() {
    return ParallelRouter.edgeSeparation;
  }
  ...
  public static void setEdgeSeparation(double edgeSeparation) {
    ParallelRouter.edgeSeparation = edgeSeparation;
  }
  ...
  public static double getEdgeDeparture() {
    return ParallelRouter.edgeDeparture;
  }
  ...
  public static void setEdgeDeparture(double edgeDeparture) {
    ParallelRouter.edgeDeparture = edgeDeparture;
  }
   ...
   */
  protected Object[] getParallelEdges(
      GraphLayoutCache cache, EdgeView edge, CellView cellView1, CellView cellView2) {
    ...
    while (iter.hasNext()) {
      ...
      if (DefaultGraphModel.isVertex(model, childCell) && (!cache.isVisible(childCell))) {
    ...
    while (iter1.hasNext()) {
      ...
      while (iter2.hasNext()) {
        ...
        Object[] edges = DefaultGraphModel.getEdgesBetween(model, tempCell1, tempCell2, false);
        if (edges.length > 0) {
          // for (int i = 0; i < edges.length; i++) {
          // System.out
