 ...
 *
 * <p>An assistant implements an specific algorithm to build an coverability graph. It has to
 * provide a graphical an textual representation of the performed steps. The construction of the
 * nodes and edges is done by the {@link CoverabilityGraphAssistantModel} class. It communicates
 * with the graph model via raising the events defined in the {@link CoverabilityGraphListener}
 * interface. The {@code CoverabilityGraphAssistantModel} provides the functionality to recognize
 * mouse interactions and calls the related methods defined in this interface if an interaction
 * occurs.
 */
public interface CoverabilityGraphAssistant {
   ...
   *
   * <p>A double click on a node triggers this action.
   *
   ...
   *
   * <p>Via this listener the assistant receives notifications about user interaction with the
   * graph. e.g. the user has clicked on a node
   *
