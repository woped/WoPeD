...
public class TreeLayout extends CoverabilityGraphLayoutBase {
  ...
  protected void setNodeBounds() {
    ...
    if (hasCycles(root)) {
      GraphLayout graphLayout =
          new GraphLayout(initialNode, settings.horizontalGap, settings.verticalGap);
      nodeBounds = graphLayout.getNodeBounds();
    } else {
      Configuration<CoverabilityGraphNode> config =
          new DefaultConfiguration<>(settings.verticalGap, settings.horizontalGap);
      org.abego.treelayout.TreeLayout<CoverabilityGraphNode> tree =
          new org.abego.treelayout.TreeLayout<>(root, new NodeExtentProvider(), config);
  ...
  private boolean hasCycles(
      TreeNode root, CoverabilityGraphNode node, Set<CoverabilityGraphNode> visitedNodes) {
