...
/**
 * This class listens for application events related to the coverability graph and calls the
 * appropriates methods if an event occurs.
 */
public class CoverabilityGraphEventProcessor extends AbstractEventProcessor {
  ...
  public CoverabilityGraphEventProcessor(AbstractApplicationMediator mediator) {
    ...
    // cg frame controller can't be initialized here because the user interface has not been created
    // at this point
  }
  ...
  public void processViewEvent(AbstractViewEvent event) {
    LoggerManager.debug(
        Constants.QUALANALYSIS_LOGGER, "-> processViewEvent " + this.getClass().getName());
  ...
  private void setZoom(AbstractViewEvent event) {
    try {
      ...
      LoggerManager.warn(
          ...
          "Could not extract zoom factor from event: " + ex.getLocalizedMessage());
    }
  }
