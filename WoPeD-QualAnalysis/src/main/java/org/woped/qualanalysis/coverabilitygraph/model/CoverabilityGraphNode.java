...
public class CoverabilityGraphNode extends DefaultGraphCell {
  ...
  // TODO: The methods recursive Bounds are only used for hierarchic layout and should be moved from
  // this class to a specific wrapper
  /**
  ...
  public String getToolTipText() {
    ...
    for (PlaceNode place : places) {
      String token =
          this.marking.isPlaceUnbound(place)
              ? Marking.UNBOUND_SIGN
              : String.valueOf(this.marking.getTokens(place));
   ...
   *
   * <p>Incoming edges are edges where this node is the target of the edge.
   *
   ...
   *
   * <p>Outgoing edges are edges where this node is the source of the edge.
   *
  ...
  private Collection<CoverabilityGraphEdge> getEdges(EdgeDirection edgeDirection) {
    ...
    for (Object elem : port.getEdges()) {
      ...
      CoverabilityGraphNode source =
          (CoverabilityGraphNode) ((CoverabilityGraphPort) edge.getSource()).getParent();
      CoverabilityGraphNode target =
          (CoverabilityGraphNode) ((CoverabilityGraphPort) edge.getTarget()).getParent();
      if (source == null || target == null) continue;

      if (source.equals(this)
          && (edgeDirection == EdgeDirection.Outgoing || edgeDirection == EdgeDirection.Both)) {
        edges.add(edge);
      }

      if (target.equals(this)
          && (edgeDirection == EdgeDirection.Incoming || edgeDirection == EdgeDirection.Both)) {
        edges.add(edge);
      }
    }
