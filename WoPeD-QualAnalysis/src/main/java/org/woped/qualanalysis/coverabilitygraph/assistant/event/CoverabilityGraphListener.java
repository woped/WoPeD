 ...
 *
 * <p>The class that is interested in processing graph events either implements this interface or
 * extends the abstract {@link CoverabilityGraphAdapter} class (overriding only the methods of
 * interest).
 *
 * <p>The listener object is then registered with a provider using the providers {@code
 * addCoverabilityGraphAssistantListener} method. Graph events are created when a node or edge is
 * added or removed or if an edge as been reconnected.
 */
public interface CoverabilityGraphListener extends EventListener {
   ...
   *
   * <p>This event could be used to clean up states before a restart is performed.
   */
