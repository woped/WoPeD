...
public abstract class CoverabilityGraphLayoutBase implements CoverabilityGraphLayout {
    ...
    for (CoverabilityGraphEdge edge : getGraphModel().getEdges()) {
      if (edge.getSourceNode().equals(edge.getTargetNode())) {
        ...
        GraphConstants.setLabelPosition(
            map, new Point2D.Double(GraphConstants.PERMILLE * 0.38, 20));
   ...
   *
   * <p>The root node contains the initial marking or the petri net.
   *
  ...
  private Rectangle2D.Double calculateNodeSize(CoverabilityGraphNode node) {
    ...
    double width =
        Math.max(
            graphModel.getLayoutSettings().minNodeSize.getWidth(),
            bounds.getWidth() + 2 * graphModel.getLayoutSettings().nodePadding);
    double height =
        Math.max(
            graphModel.getLayoutSettings().minNodeSize.getHeight(),
            bounds.getHeight() + 2 * graphModel.getLayoutSettings().nodePadding);
    return new Rectangle2D.Double(bounds.getX(), bounds.getY(), width, height);
  }

  /**
   * Sometimes the graph does not fit to the bounds. Especially with self loops at the root node. In
   * this cases the an offset is added to each node.
   */
