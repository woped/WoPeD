...
public class TPNExport {
  ...
  public static boolean save(String fileName, PetriNetModelProcessor net2save) {
    ...
    try {
      ...
      Iterator<AbstractPetriNetElementModel> rootIter =
          net2save.getElementContainer().getRootElements().iterator();
      ...
      while (rootIter.hasNext()) {
        ...
        if (currentModel.getType() == AbstractPetriNetElementModel.PLACE_TYPE) {
          ...
          fos.write(
              getLine4Transition(
                  currentModel.getNameValue(), currentModel, net2save.getElementContainer()));
          ...
          Iterator<AbstractPetriNetElementModel> simpleTransIter =
              aalstModel.getSimpleTransContainer().getRootElements().iterator();
          while (simpleTransIter.hasNext()) {

            AbstractPetriNetElementModel simpleTransModel =
                (AbstractPetriNetElementModel) simpleTransIter.next();
            if (aalstModel
  ...
  private static String getLine4Transition(
      String name, AbstractPetriNetElementModel currentModel, ModelElementContainer container) {
    String line = null;
