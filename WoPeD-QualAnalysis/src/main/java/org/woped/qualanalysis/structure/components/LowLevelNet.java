...
public class LowLevelNet {
  ...
  public int getMaxFlow(FlowNode source, FlowNode sink) {
    ...
    LoggerManager.debug(
        ...
        "Calculating flow from '"
            + source.getPetriNetNode().getId()
            + "' to '"
            + sink.getPetriNetNode().getId()
            + "'");
    ...
    while (!done) {
      ...
      while ((!SINKFOUND) && (!done)) {
        ...
        // LoggerManager.debug(Constants.QUALANALYSIS_LOGGER, "Removing node X='" +
        // X.getPetriNetNode().getId() + "'");
        // dumpList(R, "R= ");
        ...
        while (Y.hasNext()) {
          ...
          // "'->'" + temp_arc.getTarget().getPetriNetNode().getId() +"', Current Flow=" +
          // temp_arc.getFlow() + ")" );
          ...
          if ((temp_arc.getFlow() < temp_arc.getCapacity()) && !temp_arc.getTarget().isVisited()) {
            ...
            temp_arc
                .getTarget()
                .setZ(Math.min(temp_arc.getCapacity() - temp_arc.getFlow(), X.getZ()));

            temp_arc.getTarget().setVisited(true);
            R.addLast(temp_arc.getTarget());

            // LoggerManager.debug(Constants.QUALANALYSIS_LOGGER, "Adding node Y.END()='" +
            // temp_arc.getTarget().getPetriNetNode().getId() + "'");
          }
        }
        ...
        while (Y.hasNext()) {
          ...
          if ((temp_arc.getFlow() > 0) && !temp_arc.getSource().isVisited()) {
            ...
            // LoggerManager.debug(Constants.QUALANALYSIS_LOGGER, "Adding node Y.START()='" +
            // temp_arc.getSource().getPetriNetNode().getId() + "'");
          }
        }
        SINKFOUND = sink.isVisited();
        done = R.isEmpty(); // Abbruchbedingung ueberpruefen
      }
      // resetTempData();
