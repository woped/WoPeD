 ...
 *
 * <p>It consists of a set of places, a set of transitions and a weight function.
 *
 * <p>The flow relation between the places and the transitions is contained in the nodes itself in
 * the predecessor and successor properties.
 *
 * <p>The weight function returns the weight of an direct connection between two nodes or {@code 0}
...
public interface ILowLevelPetriNet extends INodeNet<AbstractNode> {
  ...
  /**
   * if a node with the id of the provided node already exists the existing node will returned. if
   * the provided node does not exist, the provided node will returned
   *
   * @param node example.
