...
public class SoundnessCheckImplement implements ISoundnessCheck {
  ...
  /**
   * @return the MarkingNet basing on the LowLevelPetriNet without t* (if not existing it will be
   *     instantiated)
   */
  private IMarkingNet getMarkingNetWithoutTStar() {
    if (markingNetWithoutTStar == null) {
      markingNetWithoutTStar =
          BuilderFactory.createMarkingNet(
              BuilderFactory.createLowLevelPetriNetWithoutTStarBuilder(editor)
                  .getLowLevelPetriNet());
    }
  ...
  /**
   * @return the MarkingNet basing on the LowLevelPetriNet with t* (if not existing it will be
   *     instantiated)
   */
  private IMarkingNet getMarkingNetWithTStar() {
    if (markingNetWithTStar == null) {
      markingNetWithTStar =
          BuilderFactory.createMarkingNet(
              BuilderFactory.createLowLevelPetriNetWithTStarBuilder(editor).getLowLevelPetriNet());
    }
  ...
  public Set<AbstractPetriNetElementModel> getNonLiveTransitions() {
    if (nonLiveTransitions == null) {
      ...
      if (getDeadTransitions().size() != 0
        ...
        for (TransitionNode transition :
            AlgorithmFactory.createNonLiveTranstionTest(getMarkingNetWithTStar())
  ...
  public Set<AbstractPetriNetElementModel> getDeadTransitions() {
    if (deadTransitions == null) {
      ...
      for (TransitionNode transition :
          AlgorithmFactory.createDeadTransitionTest(getMarkingNetWithoutTStar())
              .getDeadTransitions()) {
  ...
  public Set<AbstractPetriNetElementModel> getUnboundedPlaces() {
    if (unboundedPlaces == null) {
      ...
      for (PlaceNode place :
          AlgorithmFactory.createUnboundedPlacesTest(getMarkingNetWithTStar())
              .getUnboundedPlaces()) {
