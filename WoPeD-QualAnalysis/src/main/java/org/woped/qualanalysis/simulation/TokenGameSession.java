...
/**
 * The TokenGameSession class manages all interaction with the token game toolbar and implements
 * some of the logic to navigate the token game. It is important to note that there is one
 * TokenGameSession instance per token game, while there is one TokenGameController per petrinet,
 * where petrinet can be a top-level net or a subprocess. If more than one token game is running,
 * two TokenGameSession instances exist.
 *
 * @author weirdsoul
 */
public class TokenGameSession implements Runnable {
  ...
  /**
   * Reset a token game session into it's initial state. This spans across sub processes and will
   * bring execution back to the initial state of the main process
   */
  ...
  /**
   * The ExpertView and SlimView Objects will either been created or shown, depending on the case if
  ...
  public void addControlElements() {
    ...
    // the InternalFrameListener may be bound to the ReachablilityGraph-Frame or another
    // JInternalFrame
  ...
  /**
   * This method let the active transition occur (currently only for sequences, as soon as two
   * transitions are active, the method cannot occur so far)
   *
   * @param BackWard If true, specifies that we should occur the transition "backwards", that is
   *     sending tokens from post- to pre-set
   * @param stepsInto If true, specifies that we should step into a sub process. Effective only when
   *     not playing from history and not playing backwards
   */
  public synchronized void occurTransition(boolean BackWard, boolean stepInto) {
    ...
    if (BackWard) {
      ...
      if (BackwardTransitionToOccur != null) {
        ...
        boolean thisContainer = isTransitionInThisContainer(currentContainer);
        if (!thisContainer) {
          ...
          if (TransitionToOccur instanceof SubProcessModel) {
            SubProcessModel subProcessElement = (SubProcessModel) TransitionToOccur;
        ...
        tgController.occurTransitionbyTokenGameBarVC(TransitionToOccur, BackWard, false);
      }
    } else {
      ...
      if (followingActivatedTransitions.size() > 0) {
        tgController.occurTransitionbyTokenGameBarVC(TransitionToOccur, BackWard, stepInto);
      }
    }
  }
   ...
   *
   * <p>There are two positive case to consider:
   *
   * <ol>
  ...
  public void createSaveableHistory() {
    if (HistoryVector != null) {
      SaveableSimulation =
          new SimulationModel(
              PetriNet.getNewElementId(AbstractPetriNetElementModel.SIMULATION_TYPE),
              "Default",
              (Vector<TransitionModel>) HistoryVector.clone(),
              PetriNet.getLogicalFingerprint(),
              new Date());
      newHistory = true;
    }
  }
  ...
  /**
   * Will clear the History-Vector, the HistoryContent and will set the newHistory-variable to
   * "true"
   */
   ...
   */
  /**
   * Automatic TokenGame This Method occur automatic all transition if autoplayback is true. Choice
   * transition will be occured by random choice
   *
   * <p>ToDo: 1) refreshNet after occurence - done 2) random choice only if no probabilities are set
   */
  public void run() {
  ...
  /**
   * Sets the EndOfAutoPlay flag and wait for the auto-play thread to exit if parameter is set to
   * true
   *
  ...
  /**
   * Cleans up the ChoiceBox and the ChoiceArray. Is called by the
   * TokenGameController.transitionClicked method and therefore makes it possible to step through
   * the net with in-Editor-clicks or Remote-clicks
   */
  ...
  /**
   * Pre-select sub process as the next transition to occur, but only if there is only one sub
   * process, so the selection is deterministic
   */
  public void setSubProcessTransition() {
    ...
    for (TransitionModel t : followingActivatedTransitions) {
      ...
      if (activatedSubProcessCount == 1) this.TransitionToOccur = subProcess;
    }
  }
}
