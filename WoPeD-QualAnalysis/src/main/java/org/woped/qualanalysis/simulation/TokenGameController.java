 ...
 *     <br>
 *     <p>The <code>TokenGameController</code> provides the visual and/or model sided TokenGame of a
 *     Petrinet. <br>
 *     <br>
 ...
 *     If you only set the petrinet via Constructor and the Graph with the setter method, you'll
 *     have to enable the visualTokenGame explicit with the method <code>enableVisualTokenGame()
 *     </code>.<br>
 *     <br>
 *     2) Only model sided TokenGame: <br>
 *     If you have set the the <code>PetriNet</code> and the <code>WoPeDJGraph</code> with the
 *     Constructor you have to call <code>disableVisualTokenGame()</code>.<br>
 *     <p>Created on 21.09.2004
 */
public class TokenGameController implements ITokenGameController {
  ...
  /**
   * Starts the TokenGame. TokenGame Start method changed. Now it will only open the
   * RemoteController No "checkNet()" this will be done later.
   */
  public void start() {
    ...
    if (thisEditor.isSubprocessEditor()) {
      ...
      allTransitions.putAll(
          getPetriNet()
              .getElementContainer()
              .getElementsByType(AbstractPetriNetElementModel.SUBP_TYPE));
  ...
  private void resetVirtualTokensInElementContainer(ModelElementContainer container) {
    // restore origin tokencount
    Iterator<String> placeIter =
        container.getElementsByType(AbstractPetriNetElementModel.PLACE_TYPE).keySet().iterator();
    while (placeIter.hasNext()) {
      ((PlaceModel) container.getElementById(placeIter.next())).resetVirtualTokens();
    }
    Iterator<String> subpIter =
        container.getElementsByType(AbstractPetriNetElementModel.SUBP_TYPE).keySet().iterator();
  ...
  public void enableVisualTokenGame() {
    ...
    IQualanalysisService qualanService =
        QualAnalysisServiceFactory.createNewQualAnalysisService(thisEditor);
  ...
  public void checkNet() {
    ...
    getGraph().updateUI();
    RemoteControl.fillChoiceBox(); // Fills the Choicebox with the active Transitions that have been
    // encountered through checkTransition()
    // Check if there is a transition to choose in SlimChoiceBox
    LoggerManager.debug(
        Constants.QUALANALYSIS_LOGGER,
        "           ... DONE (" + (System.currentTimeMillis() - begin) + " ms)");
    setMarkingInRG(
        (BuilderFactory.createCurrentMarking(
            BuilderFactory.createLowLevelPetriNetWithoutTStarBuilder(thisEditor)
                .getLowLevelPetriNet(),
            true)));
  }
  ...
  private void checkTransition(TransitionModel transition, TokenGameStats tokenGameStats) {
    Map<String, ArcModel> incomingArcs =
        getPetriNet().getElementContainer().getIncomingArcs(transition.getId());

    Map<String, Object> outgoingArcs =
        getPetriNet().getElementContainer().getOutgoingArcs(transition.getId());
    // temporary variables
    ...
    if (transition.getType() == AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE
      ...
      if (transition.isActivated()) {
        // This will add all currently active postSet Transitions to the
        // TokenGameBarVC-Autochoice-List
   ...
   * @param stepInto true if we should step into a subprocess rather than stepping over it
   */
   ...
   *
   * <p>Checks if either the target or the source of the arc is an operator. If so, it fires the
   * operator triggered by the arc.
   *
   * <p>Just for testing purposes.
   *
  ...
  void executeOperators(ArcModel arc) {
    AbstractPetriNetElementModel source =
        getPetriNet().getElementContainer().getElementById(arc.getSourceId());
    AbstractPetriNetElementModel target =
        getPetriNet().getElementContainer().getElementById(arc.getTargetId());
    ...
    if (source.getType() == AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE) {
      ...
      if (tempOperator.isActivated()) {
        if (tempOperator.getOperatorType() == OperatorTransitionModel.XOR_SPLIT_TYPE
            || tempOperator.getOperatorType() == OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE
          ...
          if (tempOperator.getOperatorType() != OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE) {
            consumeTokens(
                getPetriNet().getElementContainer().getIncomingArcs(tempOperator.getId()));
          } else {
            ...
            if (centerPlace == null) {
              ...
              Map<String, AbstractPetriNetElementModel> sourceElements =
                  innerContainer.getSourceElements(arc.getTargetId());
              ...
              for (AbstractPetriNetElementModel element : sourceElements.values()) {
                if (element instanceof TransitionModel) {
                  ...
                  if (transition.isActivated()) {
                    ArcModel innerArc =
                        innerContainer.findArc(centerPlace.getId(), transition.getId());
      ...
      if (target.getType() == AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE) {
        ...
        if (tempOperator.getOperatorType() == OperatorTransitionModel.XOR_JOIN_TYPE
          ...
          if (tempOperator.getOperatorType() != OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE) {
            produceTokens(
                getPetriNet().getElementContainer().getOutgoingArcs(tempOperator.getId()));
          } else {
            ...
            if (centerPlace != null) {
              ...
              Map<String, AbstractPetriNetElementModel> targetElements =
                  innnerContainer.getTargetElements(arc.getSourceId());
              ...
              for (AbstractPetriNetElementModel element : targetElements.values()) {
                if (element instanceof TransitionModel) {
                  ...
                  ArcModel innerArc =
                      innnerContainer.findArc(innerTransition.getId(), centerPlace.getId());
  ...
  /**
   * This method has been implemented to allow backward-stepping through the net. It cares about
   * stepping back transitions as well as arcs. This method is called by the
   * TokenGameController.occurTransition by TokenGameBarVC
   *
   * @param transition
   * @param arc
   */
   ...
   *
   * <p>This operation is only useful, if the transition is a kind of xor operator. If a other kind
   * of operator is passed then the action won't be performed.
   *
  ...
  boolean reverseOperatorTransition(ArcModel arc) {
    ...
    if (target.getType() == AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE) {
      ...
      if (tempOperator.getOperatorType() != OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE) {
        ...
        if (centerPlace != null) {
          ...
          ModelElementContainer innerContainer = tempOperator.getSimpleTransContainer();
          Map<String, AbstractPetriNetElementModel> targetElements =
              innerContainer.getTargetElements(source.getId());
            ...
            if (element instanceof TransitionModel) {
              ...
              ArcModel innerArc =
                  tempOperator
                      ...
                      .findArc(transition.getId(), centerPlace.getId());
    ...
    if (source.getType() == AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE) {
      ...
      if (tempOperator.getOperatorType() != OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE) {
        ...
        if (centerPlace != null) {
          Map<String, AbstractPetriNetElementModel> sourceElements =
              tempOperator.getSimpleTransContainer().getSourceElements(arc.getTargetId());
          ...
          for (AbstractPetriNetElementModel element : sourceElements.values()) {
            ...
            if (element instanceof TransitionModel) {
              ...
              ArcModel innerArc =
                  tempOperator
                      .getSimpleTransContainer()
                      .findArc(centerPlace.getId(), transition.getId());
              centerPlace.addVirtualTokens(innerArc.getInscriptionValue());
            }
          }
        }
   ...
   *
   * <p>if param {@code activate} is set to true, all arcs will be checked, if the fulfill the
   * requirement to activate the transition. If so, the state of the arc will be set to true.
   *
   * <p>if param {@code activate} is set to false, the activated state of all arcs will be set to
   * false without any checks.
   *
   * <p>Access package local for testing purposes.
   *
  ...
  void setIncomingArcsActive(Object transitionId, boolean activate) {
    Iterator<String> incomingArcsIter =
        getPetriNet().getElementContainer().getIncomingArcs(transitionId).keySet().iterator();
   ...
   *
   * <p>If param {@code newState} is set to {@code true}, the source place of the arc will be
   * checked, if it fulfills the requirements to enable the target transition.
   *
   * <p>If param {@code newState} is set to {@code false}, the state will be set without any further
   * checks.
   *
   * <p>Access package-locale for testing purposes.
   *
  ...
  void setArcActiveState(ArcModel arc, boolean newState) {
    if (newState) {
      PlaceModel place =
          (PlaceModel) getPetriNet().getElementContainer().getElementById(arc.getSourceId());
  ...
  private void setOutgoingArcsActive(Object transitionId, boolean active) {
    Iterator<String> outgoingIter =
        getPetriNet().getElementContainer().getOutgoingArcs(transitionId).keySet().iterator();
   ...
   *
   * <p>The arc must point from a place to a transition.
   *
   * <p>Access package local for testing purposes.
   *
   ...
   *
   * <p>The arc must point from a transition to a place.
   *
   * <p>Access package local for testing purposes.
   *
  ...
  void produceTokens(ArcModel arc) {
    try {
      PlaceModel place =
          (PlaceModel) getPetriNet().getElementContainer().getElementById(arc.getTargetId());
      if (place != null) {
   ...
   *
   * <p>The arc must point from a place to a transition. The tokens will be added to the places
   * virtual token count.
   *
   * <p>Access package private for testing purposes.
   *
   * @param arc the arc to fire reverse.
   */
   ...
   */
  public void occurTransitionbyTokenGameBarVC(
      TransitionModel transition, boolean BackWard, boolean stepInto) {
  ...
  /** Open an new sub process window based on the specified sub process transition */
  ...
  /**
   * this method is similiar to stop(). it will reset all tokens and playactions but it will not
   * activate editor. through play button you could simulate again
   */
