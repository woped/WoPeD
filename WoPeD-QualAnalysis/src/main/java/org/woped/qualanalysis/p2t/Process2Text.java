...
public class Process2Text {
    ...
    for (String ids : textelements.keySet().toArray(new String[textelements.keySet().size()])) {
      for (String tempID : ids.split(",")) {
        if (tempID.equals(id)) {
          ...
          // if there are multiple IDs one match is sufficient and a break will prevent a single
  ...
  private String getIDOfPhrase(String phraseline) {
    ...
    String declaration =
        phraseline.substring(
            0,
            phraseline.indexOf(
                ">")); // the content between the beginning of the line and the first ">"
    int start =
        declaration.indexOf(idstart)
            + idstart.length(); // the position right after "id=""/start of the declaration
  ...
  private String getTextOfPhrase(String phraseline) {
    ...
    start =
        phraseline.indexOf('>') + 1; // Position after the first ">"/ the beginning of the content
