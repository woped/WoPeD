 ...
 *     Depending on status of toolbar toggle button this class does the coloring of handle pairs for
 *     each editor separately. <br>
...
public class NetColorScheme implements INetColorScheme {
  ...
  public void update() {
    ...
    if (countEditors() > 0) {
      qualanService =
          QualAnalysisServiceFactory.createNewQualAnalysisService(
              mediatorReference.getMediatorReference().getUi().getEditorFocus());
  ...
  public void startUnderstandColoring() {
    ...
    Iterator<Set<ClusterElement>> handleClusterSetIter =
        qualanService.getM_handleClusters().iterator();
    while (handleClusterSetIter.hasNext()) {
      ...
      while (handleClusterIter.hasNext()) {
        ...
        if ((!mediatorReference
                .getUIReference()
                .getEditorFocus()
                .getModelProcessor()
                .getElementContainer()
                .containsElement(element.m_element.getId())
            && ((owningElement = element.m_element.getRootOwningContainer().getOwningElement())
                != null))) highlightElement = owningElement;

        // If algorithm mode is set to 2 (plain petri net with handle correction) highlight
        // the XorSplit/-Join van der Aalst operator instead of the place in front and behind,
        // respectively.
        ...
        highlightElement.setUnderstandColoringActive(
            ConfigurationManager.getConfiguration().getColorOn());
  ...
  private AbstractPetriNetElementModel handleXorSplitPlaceRelation(
      AbstractPetriNetElementModel highlightElement) {
    if (highlightElement.getType() == AbstractPetriNetElementModel.PLACE_TYPE) {
      // Get postset of the place
      Set<AbstractPetriNetElementModel> postset =
          NetAlgorithms.getDirectlyConnectedNodes(
              highlightElement, NetAlgorithms.connectionTypeOUTBOUND);
      // Get preset of the place
      Set<AbstractPetriNetElementModel> preset =
          NetAlgorithms.getDirectlyConnectedNodes(
              highlightElement, NetAlgorithms.connectionTypeINBOUND);
      ...
      if (postset.size() == 2) {
        ...
        postsetCheck1 =
            (postsetCheck1.getRootOwningContainer().getOwningElement() != null)
                ? postsetCheck1.getRootOwningContainer().getOwningElement()
                : postsetCheck1;

        postsetCheck2 =
            (postsetCheck2.getRootOwningContainer().getOwningElement() != null)
                ? postsetCheck2.getRootOwningContainer().getOwningElement()
                : postsetCheck2;

        if (postsetCheck1.equals(postsetCheck2)
            && postsetCheck1.getType() == AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE) {
      ...
      // or 1 Xor-Join operator in the preset.
        ...
        if (preset.size() == 2) {
          ...
          // get the van der Aalst operator from the owning container and set it as
          // highlightElement.
          presetCheck1 =
              (presetCheck1.getRootOwningContainer().getOwningElement() != null)
                  ? presetCheck1.getRootOwningContainer().getOwningElement()
                  : presetCheck1;

          presetCheck2 =
              (presetCheck2.getRootOwningContainer().getOwningElement() != null)
                  ? presetCheck2.getRootOwningContainer().getOwningElement()
                  : presetCheck2;

          if (presetCheck1.equals(presetCheck2)
              && presetCheck1.getType() == AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE) {
