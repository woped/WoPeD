...
public class CurrentNetPnml {
  ...
  /**
   * Reads all objects from the editor and creates a string with all necessary informations in the
   * PNML format. Required for webservice.
   *
   * @author Martin Meitz
   */
  public void setPnmlString() {
    ...
    try {
      ...
      LoggerManager.error(
          Constants.PARAPHRASING_LOGGER, "Could not write to the stream. " + e.getMessage());
    }
  }
  ...
  private void saveModelElementContainer(NetType iNet, ModelElementContainer elementContainer) {
    ...
    /* ##### ARCS ##### */
    this.arcCounter = 0;
    Set<AbstractPetriNetElementModel> connectedTransitions =
    ...
    while (arcIter.hasNext()) {
      ...
      AbstractPetriNetElementModel currentTargetModel =
          elementContainer.getElementById(currentArc.getTargetId());
      AbstractPetriNetElementModel currentSourceModel =
          elementContainer.getElementById(currentArc.getSourceId());
    ...
    while (currentTransition.hasNext()) {
      OperatorTransitionModel currentConnectedModel =
          (OperatorTransitionModel) currentTransition.next();
      Iterator<String> innerArcIter =
          currentConnectedModel.getSimpleTransContainer().getArcMap().keySet().iterator();
      while (innerArcIter.hasNext()) {
        ArcModel currentInnerArc =
            currentConnectedModel.getSimpleTransContainer().getArcMap().get(innerArcIter.next());

        ArcModel currentOuterArc = null;
        if (elementContainer.getElementById(currentInnerArc.getSourceId()) != null) {
          currentOuterArc =
              elementContainer.findArc(
                  currentInnerArc.getSourceId(), currentConnectedModel.getId());
        }
        if (elementContainer.getElementById(currentInnerArc.getTargetId()) != null) {
          currentOuterArc =
              elementContainer.findArc(
                  currentConnectedModel.getId(), currentInnerArc.getTargetId());
        }

        initArc(
            iNet.addNewArc(),
            (currentOuterArc != null) ? currentOuterArc : currentInnerArc,
            currentInnerArc);
      }
    }
