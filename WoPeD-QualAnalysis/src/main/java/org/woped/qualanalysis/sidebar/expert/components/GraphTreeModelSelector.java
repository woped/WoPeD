...
public class GraphTreeModelSelector implements TreeSelectionListener, GraphSelectionListener {
  ...
  public GraphTreeModelSelector(
      IEditor e, JTree treeObject, AbstractApplicationMediator mediator, boolean highlightOnly) {
    m_currentEditor = e;
  ...
  public void valueChanged(TreeSelectionEvent e) {
    ...
    for (int i = 0; (selection != null) && (i < selection.length); ++i) {
      ...
      for (int j = 0; j < sme.length; ++j) processedSelection.add(sme[j]);
    }
    ...
    for (Iterator<Object> i = processedSelection.iterator(); i.hasNext(); ) {
      ...
      if (element instanceof AbstractPetriNetElementModel) {
        ...
        if (m_currentEditor
                .getModelProcessor()
                .getElementContainer()
                .getElementById(((currentObject).getId()))
            == currentObject)
          ...
          AbstractPetriNetElementModel parent =
              currentObject.getRootOwningContainer().getOwningElement();
          if (parent instanceof SubProcessModel) {
            IEditor newEditor =
                m_mediator.createSubprocessEditor(true, m_currentEditor, (SubProcessModel) parent);
  ...
  public void valueChanged(GraphSelectionEvent arg0) {
    ...
    Map<AbstractPetriNetElementModel, NodeNetInfo> treeItemMap =
        new HashMap<AbstractPetriNetElementModel, NodeNetInfo>();
