...
/**
 * creates the expert page for the sideBar and the netAnalysisDialog not many changes to old
 * NetAnalysisDialog but code refactored
 *
...
public class ExpertPage extends JPanel {
   ...
   */
  public ExpertPage(
      IEditor editor, AbstractApplicationMediator mediator, IQualanalysisService qualanService) {
    ...
    GraphTreeModelSelector treeSelectionChangeHandler =
        new GraphTreeModelSelector(editor, treeObject, mediator, true);
  ...
  private DefaultMutableTreeNode getNodeTree(IQualanalysisService qualanService) {
    ...
    DefaultMutableTreeNode wronglyUsedOperatorsInfo =
        ExpertFactory.getWronglyUsedOperatorsInfo(qualanService);
    DefaultMutableTreeNode freeChoiceViolationsInfo =
        ExpertFactory.getFreeChoiceViolationsInfo(qualanService);
    ...
    DefaultMutableTreeNode sourceTransitionsInfo =
        ExpertFactory.getSourceTransitionsInfo(qualanService);
    DefaultMutableTreeNode sinkTransitionsInfo =
        ExpertFactory.getSinkTransitionsInfo(qualanService);
    // DefaultMutableTreeNode unconnectedNodesInfo =
    // ExpertFactory.getUnconnectedNodesInfo(qualanService);
    // DefaultMutableTreeNode notStronglyConnectedInfo =
    // ExpertFactory.getNotStronglyConnectedInfo(qualanService);
    DefaultMutableTreeNode sccsInfo =
        ExpertFactory.getStronglyConnectedComponentsInfo(qualanService);
    ...
    DefaultMutableTreeNode wronglyMarkedPlacesInfo =
        ExpertFactory.getWronglyMarkedPlacesInfo(qualanService);
    DefaultMutableTreeNode boundednessInfo = ExpertFactory.getBoundednessInfo();
    DefaultMutableTreeNode unboundedPlacesInfo =
        ExpertFactory.getUnboundedPlacesInfo(qualanService);
    DefaultMutableTreeNode livenessInfo = ExpertFactory.getLivenessInfo();
    DefaultMutableTreeNode deadTransitionsInfo =
        ExpertFactory.getDeadTransitionsInfo(qualanService);
    DefaultMutableTreeNode nonLiveTransitionsInfo =
        ExpertFactory.getNonLiveTransitionsInfo(qualanService);
