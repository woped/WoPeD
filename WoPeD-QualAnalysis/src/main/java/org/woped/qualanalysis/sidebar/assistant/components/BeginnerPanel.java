...
public abstract class BeginnerPanel extends JPanel implements MouseListener {
  ...
  protected static final Border BOTTOM_BORDER = BorderFactory.createEmptyBorder(0, 0, bottomGap, 0);

  protected static final Border BOTTOM_RIGHT_BORDER =
      BorderFactory.createEmptyBorder(0, 0, bottomGap, rightGap);
   ...
   * @param header - String for the header of the page
   */
   ...
   *
   * @param headerString - String of entry
   * @param nodeIterator - iterator of nodes for highlighting
   * @param number - number of elements / errors
   * @param helpString - string to help the user with the error
   */
  protected void createEntry(
      String headerString, Iterator<?> nodeIterator, int number, String helpString) {
   ...
   * @param number - number of elements / errors
   * @param helpString - string to help the user with the error
   * @param imageIcon - String of image icon to show under the helptext
   */
  ...
  protected void createEntry(
    ...
    ArrayList<AbstractPetriNetElementModel> elements =
      ...
      do {
        ...
        if (aem instanceof AbstractPetriNetElementModel)
          if (((AbstractPetriNetElementModel) aem).getHierarchyLevel() == 0) {
            ...
            elements.add(
                ((AbstractPetriNetElementModel) aem).getRootOwningContainer().getOwningElement());
          }
        else if (aem instanceof Set<?>) {
          Collection<AbstractPetriNetElementModel> nodeSet =
              (Collection<AbstractPetriNetElementModel>) aem;
          secCounter++;
          ...
          ClickLabel cLabel =
              new ClickLabel(
                  SUB_POINT + groupOrPair + " # " + secCounter, nodeSet.iterator(), editor);
  ...
  private void addClickLabel(
      String headerString,
      int number,
      ArrayList<ClickLabel> labelSet,
      ArrayList<AbstractPetriNetElementModel> elements) {
    JLabel clickLabel;
    if (labelSet.isEmpty())
      clickLabel = new ClickLabel(Messages.getString(headerString) + COLON, elements, editor);
    else clickLabel = new JLabel(Messages.getString(headerString) + COLON);
   ...
   *
   * @param subHeader - String for Subheader
   * @param dml - the detailsmouselistener for the details icon
   * @param status - the status of the analysis (true - correct, false - incorrect)
   */
  protected void createAnalysisEntry(String subHeader, DetailsMouseListener dml, boolean status) {
    ...
    JLabel correct = null;
    if (status) correct = new JLabel(Messages.getImageIcon(CORRECT_ICON));
    else {
  ...
  /** creates and place an empty entry under the other components to fill the place */
