...
public class ExpertFactory extends DefaultMutableTreeNode {
  ...
  protected static DefaultMutableTreeNode getTransitionsInfo(IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumTransitions")
            + ": "
            + qualanService.getTransitions().size(),
  ...
  protected static DefaultMutableTreeNode getSubprocessesInfo(IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumSubprocesses")
            + ": "
            + qualanService.getSubprocesses().size(),
  ...
  protected static DefaultMutableTreeNode getWronglyUsedOperatorsInfo(
    ...
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumWronglyUsedOperators")
            + ": "
            + qualanService.getWronglyUsedOperators().size(),
        qualanService.getWronglyUsedOperators().iterator()) {
  ...
  protected static DefaultMutableTreeNode getFreeChoiceViolationsInfo(
    ...
    return new NodeGroupListNetInfo(
        Messages.getString(prefix + "NumFreeChoiceViolations")
            + ": "
            + qualanService.getFreeChoiceViolations().size(),
        qualanService.getFreeChoiceViolations().iterator()) {
      @Override
  ...
  protected static DefaultMutableTreeNode getSComponentsInfo(IQualanalysisService qualanService) {
    return new NodeGroupListNetInfo(
        Messages.getString(prefix + "NumSComponents")
            + ": "
            + qualanService.getSComponents().size(),
        qualanService.getSComponents().iterator()) {
      @Override
      public String getGroupDisplayString(int nIndex, Collection<?> group) {
  ...
  /**
   * Show the places that are not covered by any S-Component of the net. If such a place exists the
   * net is not s-coverable
   *
   * @return node containing information about uncovered places
   */
  protected static DefaultMutableTreeNode getSCompUncoveredInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NotSCovered") + ": " + qualanService.getNotSCovered().size(),
        qualanService.getNotSCovered().iterator()) {
      @Override
      public int getInfoState() {
        if (getChildCount() > 0) {
          return InfoStateERROR;
   ...
   */
  protected static DefaultMutableTreeNode getStronglyConnectedComponentsInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupListNetInfo(
        Messages.getString(prefix + "stronglyConnectedComponents")
            + ": "
            + qualanService.getStronglyConnectedComponents().size(),
        qualanService.getStronglyConnectedComponents().iterator()) {
      @Override
      public String getGroupDisplayString(int nIndex, Collection<?> group) {
   ...
   * @return information about
   */
  protected static DefaultMutableTreeNode getConnectedComponentsInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupListNetInfo(
        Messages.getString(prefix + "connectedComponents")
            + ": "
            + qualanService.getConnectedComponents().size(),
        qualanService.getConnectedComponents().iterator()) {
      @Override
      public String getGroupDisplayString(int nIndex, Collection<?> group) {
  ...
  protected static DefaultMutableTreeNode getPTHandlesInfo(IQualanalysisService qualanService) {
    return new NodeGroupListNetInfo(
        ...
        qualanService.getPTHandles().iterator()) {
      @Override
      public String getGroupDisplayString(int nIndex, Collection<?> group) {
  ...
  protected static DefaultMutableTreeNode getTPHandlesInfo(IQualanalysisService qualanService) {
    return new NodeGroupListNetInfo(
        ...
        qualanService.getTPHandles().iterator()) {
      @Override
      public String getGroupDisplayString(int nIndex, Collection<?> group) {
  ...
  protected static DefaultMutableTreeNode getSourcePlacesInfo(IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumSourcePlaces")
            + ": "
            + qualanService.getSourcePlaces().size(),
        qualanService.getSourcePlaces().iterator()) {
  ...
  protected static DefaultMutableTreeNode getSinkPlacesInfo(IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        ...
        qualanService.getSinkPlaces().iterator()) {
  ...
  protected static DefaultMutableTreeNode getSourceTransitionsInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumSourceTransitions")
            + ": "
            + qualanService.getSourceTransitions().size(),
        qualanService.getSourceTransitions().iterator()) {
  ...
  protected static DefaultMutableTreeNode getSinkTransitionsInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumSinkTransitions")
            + ": "
            + qualanService.getSinkTransitions().size(),
        qualanService.getSinkTransitions().iterator()) {
  ...
  protected static DefaultMutableTreeNode getUnconnectedNodesInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumUnconnectedNodes")
            + ": "
            + +qualanService.getNotConnectedNodes().size(),
        qualanService.getNotConnectedNodes().iterator()) {
  ...
  protected static DefaultMutableTreeNode getNotStronglyConnectedInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumNotStronglyConnectedNodes")
            + ": "
            + qualanService.getNotStronglyConnectedNodes().size(),
        qualanService.getNotStronglyConnectedNodes().iterator()) {
  ...
  /**
   * @return node containing information mistakes in the initial marking (source place should
   *     contain one token, all other places should contain no token)
   */
  protected static DefaultMutableTreeNode getWronglyMarkedPlacesInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumWronglyMarkedPlaces")
            + ": "
            + qualanService.getWronglyMarkedPlaces().size(),
        qualanService.getWronglyMarkedPlaces().iterator()) {
  ...
  protected static DefaultMutableTreeNode getUnboundedPlacesInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumUnboundedPlaces")
            + ": "
            + qualanService.getUnboundedPlaces().size(),
        qualanService.getUnboundedPlaces().iterator()) {
  ...
  protected static DefaultMutableTreeNode getDeadTransitionsInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumDeadTransitions")
            + ": "
            + qualanService.getDeadTransitions().size(),
        qualanService.getDeadTransitions().iterator()) {
  ...
  protected static DefaultMutableTreeNode getNonLiveTransitionsInfo(
      IQualanalysisService qualanService) {
    return new NodeGroupNetInfo(
        Messages.getString(prefix + "NumNonLiveTransitions")
            + ": "
            + qualanService.getNonLiveTransitions().size(),
        qualanService.getNonLiveTransitions().iterator()) {
