...
public class NodeNetInfo extends NetInfo {
  ...
  public NodeNetInfo(AbstractPetriNetElementModel node, boolean addChildren) {
    ...
    if (addChildren
        && (node instanceof InnerElementContainer)
        && (typeId == OperatorTransitionModel.XOR_SPLIT_TYPE
            ...
            || typeId == OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE
      ...
      while (innerIterator.hasNext()) {
        try {
          ...
          AbstractPetriNetElementModel owningElement =
              current.getRootOwningContainer().getOwningElement();
  ...
  // going
  // ! to be listed
  private static String getConnectedNodesString(
      AbstractPetriNetElementModel node, int connectionType, boolean superOnly) {
    StringBuffer result = new StringBuffer();
    Iterator<AbstractPetriNetElementModel> relevantNodes =
        NetAlgorithms.getDirectlyConnectedNodes(node, connectionType).iterator();
    ...
    if (result.length() > 0) result = new StringBuffer("(" + result.toString() + ")");
    return result.toString();
  }

  private static String getNodeString(
      AbstractPetriNetElementModel node, AbstractPetriNetElementModel nodeOwner) {
  ...
  public Object[] getReferencedElements() {
    ...
    result[0] =
        (((m_nodeOwner != null)
                && (m_nodeOwner.getType() != AbstractPetriNetElementModel.SUBP_TYPE))
            ? m_nodeOwner
            : m_nodeObject);
    return result;
  }
