...
public class WorkflowPage extends BeginnerPanel {
  ...
  public WorkflowPage(BeginnerPanel previous, SideBar sideBar) {
    super(previous, sideBar, Messages.getString(PREFIX + "WorkflowAnalysis"));
  ...
  public void addComponents() {
    ...
    if (numSourcePlaces != 1) {
      createEntry(
          PREFIX + "NumSourcePlaces",
          qualanalysisService.getSourcePlaces().iterator(),
          numSourcePlaces,
    ...
    if (numSinkPlaces != 1) {
      createEntry(
          PREFIX + "NumSinkPlaces",
          qualanalysisService.getSinkPlaces().iterator(),
          numSinkPlaces,
    ...
    if (numSourceTransitions != 0) {
      createEntry(
          PREFIX + "NumSourceTransitions",
          qualanalysisService.getSourceTransitions().iterator(),
          numSourceTransitions,
    ...
    if (numSinkTransitions != 0) {
      createEntry(
          PREFIX + "NumSinkTransitions",
          qualanalysisService.getSinkTransitions().iterator(),
          numSinkTransitions,
    ...
    if (numNotStronglyConnectedNodes != 0) {
      createEntry(
          PREFIX + "NumNotStronglyConnectedNodes",
    ...
    if (numArcViolations != 0) {
      createEntry(
          PREFIX + "NumArcWeightViolations",
          qualanalysisService.getArcWeightViolations(),
