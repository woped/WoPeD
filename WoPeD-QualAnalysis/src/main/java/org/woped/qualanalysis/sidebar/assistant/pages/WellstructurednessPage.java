...
public class WellstructurednessPage extends BeginnerPanel {
  ...
  public WellstructurednessPage(BeginnerPanel previous, SideBar sideBar) {
    super(previous, sideBar, Messages.getString(PREFIX + "WellStructurednessAnalysis"));
  ...
  public void addComponents() {
    if (numPTHandles != 0) {
      createEntry(
          PREFIX + "NumPTHandles",
          qualanalysisService.getPTHandles().iterator(),
          numPTHandles,
    ...
    if (numTPHandles != 0) {
      createEntry(
          PREFIX + "NumTPHandles",
          qualanalysisService.getTPHandles().iterator(),
          numTPHandles,
