...
public class TextToIntermediateConverter {
	...
	public TextToIntermediateConverter(RPST<ControlFlow,Node> rpst, ProcessModel process, EnglishLabelHelper lHelper, String imperativeRole, boolean imperative) throws FileNotFoundException, JWNLException {
		this.rpst = rpst;
...
//								role = getRole(tNode);
	...
	public ConverterRecord convertXORGeneral(RPSTNode<ControlFlow, Node> node) {
		...
		ConditionFragment post = new ConditionFragment("execute", "one of the alternative branches","", "", ConditionFragment.TYPE_ONCE, new HashMap<String, ModifierRecord>());
...
//			cFrag.verb_IsPassive = true;
			...
			// Determine postcondition  
			ConditionFragment post = new ConditionFragment("finish", "loop", "", "", ConditionFragment.TYPE_ONCE, new HashMap<String, ModifierRecord>());
			post.verb_IsPassive = true;
			post.bo_isSubject = true;
			post.setFragmentType(AbstractFragment.TYPE_JOIN);
...
//		String role = process.getGateways().get(Integer.valueOf(node.getEntry().getId())).getLane().getName();
	...
	public ConverterRecord convertANDGeneral(RPSTNode<ControlFlow, Node> node, int activities, ArrayList<Node> conditionNodes) {
		...
		// Statement about negative case (process is finished)
		ConditionFragment post = new ConditionFragment("execute", "all " + activities + " branch", "", "", ConditionFragment.TYPE_ONCE, new HashMap<String, ModifierRecord>());
		post.verb_isPast = true;
		post.verb_IsPassive = true;
		post.bo_isSubject = true;
...
//				role = process.getGateways().get(Integer.valueOf(node.getExit().getId())).getLane().getName();
				role = getRole(node);
				post = new ConditionFragment("finish", lHelper.getNoun(verb), role, "", ConditionFragment.TYPE_ONCE, new HashMap<String, ModifierRecord>());
				post.sen_headPosition = true;
				post.verb_isPast = true;
				post.setFragmentType(AbstractFragment.TYPE_JOIN);
				post.addAssociation(Integer.valueOf(node.getEntry().getId()));
			} else {
				post = new ConditionFragment("finish", "both branch", "", "", ConditionFragment.TYPE_ONCE, new HashMap<String, ModifierRecord>());
				post.bo_isPlural = true;
				post.sen_headPosition = true;
				post.bo_hasArticle = false;
				post.bo_isSubject = true;
