...
public class OSXCheckboxMenuItem extends JCheckBoxMenuItem {
  ...
  class ShortcutActionListener implements ActionListener {
     ...
     */
    ShortcutActionListener(
        AbstractApplicationMediator mediator, String action_id, int event_id, JComponent target) {
      action = ActionFactory.getStaticAction(action_id);
      // VisualController.getInstance().addElement(action, VisualController.WITH_EDITOR,
    ...
    this.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            if (currentItem.isSelected()) synchronizedItem.setSelected(true);
            else synchronizedItem.setSelected(false);
          }
        });
    synchronizedItem.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent evt) {
            if (synchronizedItem.isSelected()) currentItem.setSelected(true);
            else currentItem.setSelected(false);
          }
        });
