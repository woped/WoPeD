...
public class GlobalShortcutEventDispatcher implements KeyEventDispatcher {
  ...
  private GlobalShortcutEventDispatcher(
      JCommandButton observerButton, String propertiesPrefixForShortcuts) {
    prefix = propertiesPrefixForShortcuts;
    button = observerButton;
  }

  private GlobalShortcutEventDispatcher(
      JCommandButton observedButton,
   ...
   */
  public static void addShortcutListener(
      JCommandButton observedButton, String propertiesPrefixForShortcuts) {
    KeyboardFocusManager.getCurrentKeyboardFocusManager()
        .addKeyEventDispatcher(
            new GlobalShortcutEventDispatcher(observedButton, propertiesPrefixForShortcuts));
  }
   ...
   */
  public static void addShortcutButtonListener(
      JCommandButton observedButton,
      String propertiesPrefixForShortcuts,
      boolean ignoreDefaultKeyMask) {
    KeyboardFocusManager.getCurrentKeyboardFocusManager()
        .addKeyEventDispatcher(
            new GlobalShortcutEventDispatcher(
                observedButton, propertiesPrefixForShortcuts, ignoreDefaultKeyMask));
  }
   ...
   * @param propertiesPrefixForShortcuts the prefix in messages.properties
   */
  public static void addShortcutClickListener(
      JCommandButton observedButton, String propertiesPrefixForShortcuts) {
    GlobalShortcutEventDispatcher shortcutClickListener =
        new GlobalShortcutEventDispatcher(observedButton, propertiesPrefixForShortcuts);
    shortcutClickListener.buttonRequiresMouseClick = true;
    KeyboardFocusManager.getCurrentKeyboardFocusManager()
        .addKeyEventDispatcher(shortcutClickListener);
  }
    ...
    if (e.getID() == KeyEvent.KEY_RELEASED) {
      if (e.getWhen() - lastKeyPressed > delay) {
        ...
        if (!ignoreDefaultKeyMask) {
          if (e.getKeyCode() == Messages.getShortcutKey(prefix)
              && e.getModifiers()
                  == (Messages.getShortcutModifier(prefix)
          ...
          if (e.getKeyCode() == Messages.getShortcutKey(prefix)
              && e.getModifiers() == Messages.getShortcutModifier(prefix)) {
            doButtonClick();
          }
        }
