...
public class DefaultUserInterface extends MainFrame
    implements IUserInterface, InternalFrameListener {
  ...
  public DefaultUserInterface(TaskBarVC taskBar, StatusBarVC statusBar) {
    ...
    setBounds(
        ConfigurationManager.getConfiguration().getWindowX(),
        ConfigurationManager.getConfiguration().getWindowY(),
        (int) ConfigurationManager.getConfiguration().getWindowSize().getWidth(),
  ...
  public void addEditor(IEditor editor) {
    if (editor != null) {
      frame =
          new DefaultEditorFrame(
              (EditorVC) editor,
              new EditorOperations(editor),
              new EditorData(),
              new PetriNetResourceEditor((EditorVC) editor));
  ...
  public void quit() {
    ...
    action.actionPerformed(
        new ViewEvent(this, AbstractViewEvent.VIEWEVENTTYPE_GUI, AbstractViewEvent.EXIT));
  ...
  public void arrangeFrames() {
    ...
    if (allOpenFrames.size() != 0) {
      ...
      LoggerManager.debug(
          Constants.GUI_LOGGER, "Frames" + allOpenFrames.size() + ":" + xCount + "x" + yCount);
      ...
      for (int i = 0; i < allOpenFrames.size(); i++) {
        ...
        if ((row + 1) == yCount) {
          ...
          currentFrame.setBounds(
              new Rectangle(col * lastRowWidth, row * frameHeight, lastRowWidth, frameHeight));
        } else {
          currentFrame.setBounds(
    ...
    for (int i = 0; i < allFrames.length && !foundEditor; i++) {
      if (allFrames[i] instanceof org.woped.starter.DefaultEditorFrame) {
        try {
          ...
          // This exception isn't critical, so no need for handling this exception. Just a focus
          // thing. ;)
        }
      }
    }
  ...
  /**
   * This method provides the possibility to have a "started" TokenGame and the "ProcessTab" is
   * viewed when simulation is running
   */
  ...
  /**
   * Fix modality in case a frame gets activated due to an external event. This will make sure that
   * if there is at least one modal frame, this frame will remain in focus with the other ones
   * staying disabled
   *
   * @return true if modality had to be fixed because there is a modal frame and it was not the one
   *     currently due to an external event, false otherwise
   */
