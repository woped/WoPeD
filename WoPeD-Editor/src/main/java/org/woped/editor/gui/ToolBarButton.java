...
/**
 * Part of a small example showing basic use of JToolBar. The point here is that dropping a regular
 * JButton in a JToolBar (or adding an Action) doesn't give you what you want -- namely a small
 * button just enclosing the icon, and with text labels (if any) below the icon, not to the right of
 * it. 1999 Marty Hall, http://www.apl.jhu.edu/~hall/java/
 */
...
public class ToolBarButton extends JToggleButton implements MouseListener {
   ...
   */
  public static AbstractButton createButton(
      String text, String iconPath, WoPeDAction action, int enableing) {
    return createButton(
        text, iconPath, action, enableing, VisualController.IGNORE, VisualController.IGNORE, false);
  }
  ...
  public static AbstractButton createButton(
      ...
      boolean toggleable) {
    return createButton(
        Messages.getTitle(propertiesPrefix),
        Messages.getIconLocation(propertiesPrefix),
        action,
        enableing,
        visible,
        selected,
  ...
  public static AbstractButton createButton(String text, WoPeDAction action, int enableing) {
    return createButton(
        text, action, enableing, VisualController.IGNORE, VisualController.IGNORE, false);
  }
  ...
  /**
   * @param action The Action which should be executed whenever the new Button gets pressed.
   * @param toggleable Defines if the new Button should be toggleable
