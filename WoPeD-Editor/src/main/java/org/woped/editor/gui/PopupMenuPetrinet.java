...
/**
 * Creates the Popupmenu and adds the MenuItems to the VisualController. Elements will be created
 * when the get-Method is called the first time. All MenuItems are JMenuItems.
 *
 * @author Thomas Pohl
 */
...
public class PopupMenuPetrinet extends JPopupMenu {
  ...
  public static PopupMenuPetrinet getInstance() {
    if (c_instance == null) {
      ...
      VisualController.getInstance()
          .propertyChange(new PropertyChangeEvent(c_mediator, "InternalFrameCount", null, null));
    }
    c_instance.pack();
    return c_instance;
  }
  ...
  private JMenuItem getRemoveTokenMenuItem() {
    if (m_removeTokenMenuItem == null) {
      ...
      //            VisualController.getInstance().addElement(action,
      // VisualController.TOKEN_PLACE_SELECTION, VisualController.PLACE_SELECTION,
  ...
  private JMenuItem getAddAndSplitMenuItem() {
    if (m_addAndSplitMenuItem == null) {
      ...
      //            VisualController.getInstance().addElement(action, VisualController.NO_SELECTION,
      // VisualController.NO_SELECTION, VisualController.IGNORE);
    }
