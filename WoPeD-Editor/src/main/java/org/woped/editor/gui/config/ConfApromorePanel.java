 ...
 *     <br>
 *     The <code>ConfApromorePanel</code> is the <code>AbstractConfPanel</code> for the
 *     configuration of the apromore servers. <br>
...
public class ConfApromorePanel extends AbstractConfPanel {
    ...
    if (saveButton.isEnabled()) {
      int result =
          JOptionPane.showConfirmDialog(
              ...
              Messages.getString("Configuration.Apromore.Dialog.SaveChanges.Title"),
  ...
  public void saveConfig() {
    ...
    if (ConfigurationManager.getConfiguration().isSetApromoreServers()) {
      ...
      ConfigurationManager.getConfiguration()
          .setCurrentApromoreIndex(serverComboBox.getSelectedIndex());
      ConfigurationManager.getConfiguration()
      ...
      ConfigurationManager.getConfiguration()
          .setApromoreManagerPath(getManagerPathText().getText());
  ...
  public void readConfiguration() {
    ...
    if (ConfigurationManager.getConfiguration().isSetApromoreServers())
      getServersComboBox()
          .setSelectedIndex(ConfigurationManager.getConfiguration().getCurrentApromoreIndex());

    getServerNameTextUser()
        .setText(ConfigurationManager.getConfiguration().getApromoreServerName());
    ...
    getServerPortText()
        .setText("" + ConfigurationManager.getConfiguration().getApromoreServerPort());
    ...
    if (ConfigurationManager.getConfiguration().isSetApromorePassword() == true)
      try {
        getPasswordText()
            .setText(
                ApromorePasswordSecurity.decode(
                    ConfigurationManager.getConfiguration().getApromorePassword()));
      } catch (NoSuchAlgorithmException
          | InvalidKeyException
          | NoSuchPaddingException
          | IllegalBlockSizeException
    ...
    getUseProxyCheckbox()
        .setSelected(ConfigurationManager.getConfiguration().getApromoreUseProxy());
      ...
      enabledPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getTitle("Configuration.Apromore.Enabled.Panel")),
  ...
  private JPanel getServerListPanel() {
    if (listPanel == null) {
      ...
      listPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("Configuration.Apromore.ServerList.Panel.Title")),
              BorderFactory.createEmptyBorder(10, 10, 10, 10)));
  ...
  private JPanel getSettingsPanel() {
    if (settingsPanel == null) {
      ...
      settingsPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("Configuration.Apromore.Settings.Panel.Title")),
              BorderFactory.createEmptyBorder(10, 10, 10, 10)));
      c.weightx = 0.5;
      // c.fill = GridBagConstraints.HORIZONTAL;
      c.gridx = 0;
  ...
  private JPanel getProxyPanel() {
    if (proxyPanel == null) {
      ...
      proxyPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("Configuration.Apromore.ProxySettings.Panel.Title")),
              BorderFactory.createEmptyBorder(10, 10, 10, 10)));
      d.weightx = 0.5;
  ...
  private JCheckBox getUseBox() {
    if (useBox == null) {
      ...
      useBox.setToolTipText(
          "<html>" + Messages.getString("Configuration.Apromore.Label.Use") + "</html>");
      ...
      changeButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              ...
              if (servers != null) {
                setApromoreServerID(
                    servers[serverComboBox.getSelectedIndex()].getApromoreServerID());
              }
              setButtonsToAddOrChange();
  ...
  private JLabel getServerNameLabelUser() {
    if (serverNameLabelUser == null) {
      serverNameLabelUser =
          new JLabel(
              "<html>"
                  + Messages.getString("Configuration.Apromore.Label.ServerNameDisplayed")
                  + "</html>");
  ...
  private JLabel getManagerPathLabel() {
    if (managerPathLabel == null) {
      managerPathLabel =
          new JLabel(
              "<html>"
                  + Messages.getString("Configuration.Apromore.Label.ManagerPath")
                  + "</html>");
  ...
  private void setTextFields() {
    ...
    try {
      passwordText.setText(
          ApromorePasswordSecurity.decode(servers[currentIndex].getApromorePassword()));
    } catch (NoSuchAlgorithmException
        | InvalidKeyException
        | NoSuchPaddingException
        | IllegalBlockSizeException
  ...
  private void addKeyListeners() {
    ...
    managerPathText.addKeyListener(
        new KeyListener() {
          @Override
          public void keyTyped(KeyEvent e) {}
  ...
  public void changeApromoreServer(int ID) {
    ...
    setNameAlreadyInUse(false);
    ConfigurationManager.getConfiguration()
        .changeApromoreServerSettings(
            ID, name, url, port, path, user, password, useProxy, proxyUrl, proxyPort);
    setChangeServer(false);
    if (ID == 0) {
      setButtonsToDefault();
  ...
  public boolean checkInput() {
    ...
    if (useBox.isSelected()
        && (serverURLText.getText().equals("") || serverPortText.getText().equals(""))) {
      inputOK = false;
      message = Messages.getString("Apromore.UI.MandatoryFields");
    }

    if (useBox.isSelected()
        && (usernameText.getText().equals("") || passwordText.getPassword().length == 0)) {
      if (inputOK) {
        url = serverURLText.getText() + ":" + serverPortText.getText() + "/portal/login.zul";
        message =
            Messages.getString("Apromore.UI.MandatoryFields")
    ...
    if (!message.equals("")) {
      ...
      if (addButton) {
        Object[] options = {
          Messages.getString("Apromore.UI.CreateUser"), Messages.getString("Dialog.Ok")
        };
        int option =
