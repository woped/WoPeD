  ...
  public void processViewEvent(AbstractViewEvent event) {
    ...
    if (editor != null) {
      ...
      if (editor.getLastMousePosition() != null) {
        map.setPosition(
            (int) (editor.getLastMousePosition().getX() / editor.getGraph().getScale()),
      ...
      switch (event.getOrder()) {
        ...
        case AbstractViewEvent.OPEN_PROPERTIES:
          ...
          if (element != null) {
            if (element instanceof TransitionModel) {
              new TransitionPropertyEditor(
                  (JFrame) getMediator().getUi(),
                  (Point) editor.getLastMousePosition(),
                  (TransitionModel) element,
                  editor);
            }
            if (element instanceof PlaceModel) {
              new PlacePropertyEditor(
                  (JFrame) getMediator().getUi(),
                  (Point) editor.getLastMousePosition(),
                  (PlaceModel) element,
                  editor);
        ...
        case AbstractViewEvent.ADD_EXT_TRIGGER:
          ...
          editor.createTrigger(
              getCreateTriggerMap(
                  editor.getGraph().getSelectionCell(), TriggerModel.TRIGGER_MESSAGE));
          break;
        case AbstractViewEvent.ADD_RES_TRIGGER:
          editor.createTrigger(
              getCreateTriggerMap(
                  editor.getGraph().getSelectionCell(), TriggerModel.TRIGGER_RESOURCE));
          break;
        case AbstractViewEvent.ADD_TIME_TRIGGER:
          cell = editor.getGraph().getSelectionCell();
          removeResources(editor, cell);
          editor.createTrigger(
              getCreateTriggerMap(editor.getGraph().getSelectionCell(), TriggerModel.TRIGGER_TIME));
          break;
        ...
        case AbstractViewEvent.SHOWSIDEBAR:
          ...
          editor
              .getEditorPanel()
              .setSideTreeViewVisible(!editor.getEditorPanel().isSideTreeViewVisible());
          break;
        case AbstractViewEvent.SHOWOVERVIEW:
          // visbility of overview panel
          editor
              .getEditorPanel()
              .setOverviewPanelVisible(!editor.getEditorPanel().isOverviewPanelVisible());
          break;
        case AbstractViewEvent.SHOWTREEVIEW:
          // visibility of elements tree panel
          editor
              .getEditorPanel()
              .setTreeviewPanelVisible(!editor.getEditorPanel().isTreeviewPanelVisible());
          break;
        ...
        case AbstractViewEvent.TOKENGAME_AUTO:
          ...
          if (tgbController != null) {
            ...
            LoggerManager.error(
                Constants.EDITOR_LOGGER, "Tokengame controls used while tokengame was inactive");
          break;
          // TODO(P2T):
