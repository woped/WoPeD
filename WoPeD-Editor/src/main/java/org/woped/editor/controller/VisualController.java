...
/**
 * @author Thomas Pohl
 *     <p>The VisualController enables and disables, changes the visibility and selection status.
 *     Changes are initiated from external events via PropertyChangeEvents and GraphSelectionEvents.
 */
public class VisualController
    implements PropertyChangeListener, IClipboaredListener, GraphSelectionListener, MouseListener {
  ...
  private Map<WoPeDAction, IVisibility> actionVisibilityMap =
      new HashMap<WoPeDAction, IVisibility>();
    ...
    if (action != null) {
      try {
        ...
        while (it.hasNext()) {
          ...
          if ((target instanceof JMenuItem)) {
            if (target.getClass().getName().equals("org.woped.starter.osxMenu.OSXMenuItem")
                || target instanceof JCheckBoxMenuItem) {
              target.setEnabled(status);
        ...
        LoggerManager.warn(
            ...
            "Could not change enabling status (code: " + action.getValue("NAME") + "): " + status);
      }
    }
  ...
  protected static boolean setSelected(WoPeDAction action, boolean status) {
    if (action != null) {
      try {
        ...
        LoggerManager.warn(
            Constants.EDITOR_LOGGER,
            "Could not change selection status (code: " + action.getValue("NAME") + "): " + status);
      }
    }
    ...
    if (action != null) {
      try {
        ...
        while (it.hasNext()) {
          ...
          if (target instanceof JMenuItem) {
            if (target.getClass().getName().equals("org.woped.starter.osxMenu.OSXMenuItem")
                || target instanceof JCheckBoxMenuItem) target.setVisible(true);
            else target.setVisible(status);
          }
        }
  ...
  /**
   * Adds a object which is supposed to be controlled, using DefaultVisibility. Note that each
   * aspect of an action's visibility (enabled, visible, selected) must depend on exactly one
   * attribute. If this is too simplistic for you, please supply a custom IVisibility implementation
   ...
   *
   * @param obj the newly added Object
   * @param visibility decide about visibility options for the specified WoPeDAction
   */
  ...
  /**
   * Changes the selected the selected, visible and enable status for all subscribed objects.
   *
   * @param attribute
   * @param status
   */
  ...
  private void updateStatusAll() {
    for (WoPeDAction action : this.actionVisibilityMap.keySet()) updateStatus(action);
  }
   ...
   *
   * <ul>
   *   <li>The Number of InternalFrames(Editors) changes.
   *   <li>A different InternalFrame gets selected.
   *   <li>The Drawmode gets changes.
   *   <li>The TokenGame gets started or stopped.
   *   <li>The Understandability Coloring gets activated.
   * </ol>
   *
  ...
  protected void checkSelection() {
    ...
    if (isActive()) {
      ...
      setStatus(
          NODE_OR_XORARC_SELECTION,
          (transitionSelected | placeSelected | xorArcSelected) && !arcpointSelected);
  ...
  protected void checkAproMoRe() {
    ...
    setStatus(
        APROMORE_EXPORT,
        ConfigurationManager.getConfiguration().getApromoreUse()
            & am.getUi().getEditorFocus() != null);
  }
  ...
  protected void checkActiveEditor() {
    ...
    if (am.getUi().getEditorFocus() != null) {
      setStatus(SUBPROCESS_EDITOR, !(am.getUi().getEditorFocus() instanceof SubprocessEditorVC));
    } else {
    ...
    if (am.getUi().getEditorFocus() != null) {
      ...
      if (undoManager != null) {
        ...
        if (am.getUi().getEditorFocus() instanceof SubprocessEditorVC)
          undo =
              ((WoPeDUndoManager) undoManager)
                  .canUndo(am.getUi().getEditorFocus().getModelProcessor().getElementContainer());
  ...
  /**
   * Objects implementing IVisibility control the visual representation of WoPeDAction instances in
   * menus etc. Whether an action is visible, enabled or selected depends on attributes describing
   * application-wide state, such as whether an editor window is currently open and in focus
   * (WITH_EDITOR) or whether a token game is active (TOKENGAME).
   *
   * <p>For convenience, there is a default implementation called DefaultVisibility, covering simple
   * cases where visual representation depends on only one attribute for each visible, enabled and
   * selected.
   *
   * @author weirdsoul
   */
  public interface IVisibility {
    /**
     * Return whether an associated action should be visible, depending on the specified status
     *
    ...
    /**
     * Return whether an associated action should be enabled, depending on the specified status
     *
     * @param status An array of attributes that are either enabled or disabled
    ...
    /**
     * Return whether an associated action should be selected, depending on the specified status
     *
     * @param status An array of attributes that are either enabled or disabled
  ...
  private class DefaultVisibility implements IVisibility {
    ...
    public DefaultVisibility(int attributeEnabled, int attributeVisible, int attributeSelected) {
