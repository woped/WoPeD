...
public class EditorPanel extends JPanel {
  ...
  public EditorPanel(
      IEditor editor,
      AbstractApplicationMediator centralMediator,
    ...
    if (loadUI) {
      ...
      m_mainSplitPane =
          new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, m_scrollPane, m_rightSideTreeView);
  ...
  public String getName() {
    return super.getName() == null
        ? Messages.getString("Document.Title.Untitled")
        : super.getName();
  }
  ...
  public void setSideTreeViewVisible(boolean showTreeView) {
    m_mainSplitPane.setDividerLocation(
        showTreeView
            ? m_mainSplitPane.getLastDividerLocation()
            : m_mainSplitPane.getMaximumDividerLocation());
  }
  ...
  public JPanel getTreeviewPanel() {
    if (treeviewPanel == null) {
      ...
      GraphTreeModelSelector selectionHandler =
          new GraphTreeModelSelector(editor, m_treeObject, centralMediator, false);
      ...
      HideLabel treeviewPanelClose =
          new HideLabel(Messages.getImageIcon("AnalysisSideBar.Cancel"), treeviewPanel);
  ...
  public JPanel getOverviewPanel() {
    if (overviewPanel == null) {
      ...
      HideLabel overviewPanelClose =
          new HideLabel(Messages.getImageIcon("AnalysisSideBar.Cancel"), overviewPanel);
      overviewPanel.add(
          overviewLabel,
          new GridBagConstraints(
              0,
  ...
  public void setSavedLayoutInfo(EditorLayoutInfo layoutInfo) {
    if (layoutInfo != null) {
      ...
      VisualController.getInstance()
          .propertyChange(
              new PropertyChangeEvent(centralMediator, "InternalFrameCount", null, null));
  ...
  public void initializeAnalysisSideBar() {
    ...
    qualitativeAnalysisSideBar =
        new SideBar(editor, centralMediator, autoRefreshStatus, tStarCheckBox);
    ...
    // is a workflow net)
    tStarCheckBox.addActionListener(
        new ActionListener() {
    ...
    // define close button
    HideLabel semanticPanelClose =
        new HideLabel(Messages.getImageIcon("AnalysisSideBar.Cancel"), analysisSideBarPanel);

    analysisSideBarPanel.add(
        semanticLabel,
        new GridBagConstraints(
            0,
            0,
  ...
  public void showAnalysisBar() {
    ...
    if (!analysisBarVisible) {
      ...
      m_rightSideTreeViewWithAnalysisBar =
          new JSplitPane(JSplitPane.VERTICAL_SPLIT, m_rightSideTreeView, analysisSideBarPanel);
      ...
      m_rightSideTreeViewWithAnalysisBar.setMinimumSize(
          rightSideTreeViewWithAnalysisBarMinimumSize);
  ...
  /**
   * Shows the metrics sidebar and resize the editor window. Replaces the normal EditorSplitPane
   * with another SplitPane with the MetricsSidebar on the right side.
   *
   * @author Mathias Gruschinske, Stefan Hackenberg
   */
  public void showMetricsBar() {
    ...
    if (!metricsBarVisible) {
      ...
      m_rightSideTreeViewWithAnalysisBar =
          new JSplitPane(JSplitPane.VERTICAL_SPLIT, m_rightSideTreeView, sideBar);
  ...
  public void setRotateSelected(boolean rotateSelected) {
    ...
    if (analysisBarVisible && !metricsBarVisible) qualitativeAnalysisSideBar.showTStarIfPossible();
  }
  ...
  public void rotateLayout(EditorVC editorVC) {
    ...
    editorVC
        .getEditorPanel()
        .m_orientation
        .rotateView(editorVC.getModelProcessor().getElementContainer());
    ...
    else editorVC.getEditorPanel().editorSize.resize(true);
  ...
  public void initializeP2TSideBar() {
    ...
    // Define close button
    HideLabel p2tPanelClose =
        new HideLabel(Messages.getImageIcon("AnalysisSideBar.Cancel"), p2tSideBarPanel);
  ...
  public void showP2TBar() {
    ...
    if (!p2TBarVisible) {
      ...
      m_rightSideTreeViewWithAnalysisBar =
          new JSplitPane(JSplitPane.VERTICAL_SPLIT, m_rightSideTreeView, p2tSideBarPanel);
      if (isOverviewPanelVisible() && isTreeviewPanelVisible()) {
        m_rightSideTreeViewWithAnalysisBar.setDividerLocation(200);
        m_rightSideTreeViewWithAnalysisBar.setEnabled(true);
      } else if (isOverviewPanelVisible() || isTreeviewPanelVisible()) {
  ...
  public void showT2PBar(String text) {
    ...
    JSplitPane scrollPaneWithT2PBar =
        new JSplitPane(JSplitPane.VERTICAL_SPLIT, m_scrollPane, t2pBar);
  ...
  /**
   * @author Svenja label with mouse listener and icon to close the overviewpanel or the
   *     treeviewPanel
   */
