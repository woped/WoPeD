...
/**
 * The logical model created in the background - this model is the basis for the work of the
 * Layouting Algorithm
 *
...
class LogicalModel {
  ...
  public LogicalModel(
      DefaultListModel objectsAssignedListModel,
      DefaultListModel objectsUnassignedListModel,
      DefaultTreeModel RolesTreeModel,
      DefaultMutableTreeNode RolesTopNode,
      DefaultTreeModel GroupsTreeModel,
      DefaultMutableTreeNode GroupsTopNode,
      DefaultTreeModel superRolesTreeModel,
      DefaultTreeModel superGroupsTreeModel,
      DefaultMutableTreeNode superRolesTopNode,
      DefaultMutableTreeNode superGroupsTopNode) {
    ...
    for (int j = 0; j < superRolesTreeModel.getChildCount(superRolesTopNode); j++) {
      ...
      SuperRolesTreeNode currentNode =
          (SuperRolesTreeNode) superRolesTreeModel.getChild(superRolesTopNode, j);
    ...
    for (int j = 0; j < superGroupsTreeModel.getChildCount(superGroupsTopNode); j++) {
      String currentSuperGroupName =
          superGroupsTreeModel.getChild(superGroupsTopNode, j).toString();
      CompoundGroup currentSuperGroup = new CompoundGroup(currentSuperGroupName);
      allCompoundGroups.add(currentSuperGroup);

      SuperGroupsTreeNode currentNode =
          (SuperGroupsTreeNode) superGroupsTreeModel.getChild(superGroupsTopNode, j);
