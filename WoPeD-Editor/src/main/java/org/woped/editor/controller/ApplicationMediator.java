...
/**
 * This Class should be the Mediator for the Editor VC... It must be implemented (inherit) by each
 * GUI. The Mediator must register each VC... Actionperformig is only run by messages. Each VC must
 * implement the VC Interface in order to perform actions. The Method is calles by the Mediator...
 * The actions must be an AbstractWopedAction, which get the VC in the Constructor!
 *
...
public class ApplicationMediator extends AbstractApplicationMediator {
  ...
  public ApplicationMediator(IUserInterface ui, IGeneralConfiguration conf) {
    ...
    getVepController()
        .register(AbstractViewEvent.VIEWEVENTTYPE_APPLICATION, new ApplicationEventProcessor(this));
    getVepController()
        .register(AbstractViewEvent.VIEWEVENTTYPE_EDIT, new EditorEventProcessor(this));
  ...
  public IEditor createEditor(boolean undoSupport, boolean loadUI) {
    EditorVC editor =
        new EditorVC(EditorVC.ID_PREFIX + editorCounter, clipboard, undoSupport, this, loadUI);
  ...
  // ! @return reference to new editor object
  @Override
  public IEditor createSubprocessEditor(
      boolean undoSupport, IEditor parentEditor, SubProcessModel subProcess) {
    IEditor editor = null;
    ...
    if (editor == null) {
      ...
      editor =
          new SubprocessEditorVC(
              EditorVC.ID_PREFIX + editorCounter,
              clipboard,
              undoSupport,
              parentEditor,
              subProcess,
  ...
  public void setUi(IUserInterface ui) {
    if (ui != null) {
      PropertyChangeSupport propertyChangeSupport =
          new PropertyChangeSupport(ui.getPropertyChangeSupportBean());
