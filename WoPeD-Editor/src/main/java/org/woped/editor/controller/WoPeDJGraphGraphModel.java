...
public class WoPeDJGraphGraphModel extends DefaultGraphModel {
   ...
   *
   * <p>This method is overwritten to supply a custom undoable edit object that will take care of
   * the custom model (ModelElementContainer et al) while performing undo / redo
   */
  protected DefaultGraphModel.GraphModelEdit createEdit(
      Object[] inserted,
      Object[] removed,
      Map attributes,
      ConnectionSet cs,
    ...
    DefaultGraphModel.GraphModelEdit edit =
        new WoPeDUndoableEdit(inserted, removed, attributes, cs, pm, editor);
    if (edit != null) {
      if (edits != null) for (int i = 0; i < edits.length; i++) edit.addEdit(edits[i]);
      edit.end();
    }
    return edit;
  }
  ...
  public class WoPeDUndoableEdit extends DefaultGraphModel.GraphModelEdit {
    ...
    public void undo() throws CannotUndoException {
      // @TODO: The order in which we update the model is determined by the fact that we are using
      // the view
      // to update the model here, e.g. we use the view's undo manager to restore information about
      // deleted arcs.
      // This is why we have to perform insert operations into the model after its view has been
      // updated.
    ...
    private void insertElements(Object[] elementsInput) {
      ...
      if (elements != null) {
        for (int i = 0; i < elements.length; i++) {
          LoggerManager.debug(
              Constants.EDITOR_LOGGER, "Trying to create " + elements[i].getClass());
          if (elements[i] instanceof AbstractPetriNetElementModel) {
            m_editor
                .getModelProcessor()
                .getElementContainer()
                .addElement((AbstractPetriNetElementModel) elements[i]);
            ...
            ((TransitionModel) ((GroupModel) trigger.getParent()).getMainElement())
                .getToolSpecific()
                .setTrigger((TriggerModel) elements[i]);
          } else {
            LoggerManager.debug(
                Constants.EDITOR_LOGGER, "Could not insert type:" + elements[i].getClass());
          }
        }
      }
      ...
      // This means that when the tree model update is triggered, we really don't have our jgraph
      // model
