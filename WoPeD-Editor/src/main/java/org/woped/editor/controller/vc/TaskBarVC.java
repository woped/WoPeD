...
public class TaskBarVC extends JPanel implements IViewController, IEditorAware {
  ...
  public void selectEditor(IEditor editor) {
    ...
    if (oldEditor != null) oldEditor.getGraph().clearSelection();
   ...
   * with a reference to the current listener. If the viewEvent not null Clone viewListener to a
   * vector If the vector not null To Test is to check the viewEvent
   */
