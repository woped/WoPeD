 ...
 *     On the left you can add <code>AbstractConfPanel</code> s to the tree. Clicking on an node,
 *     the assisitaed Panel will show up in the right. <br>
...
public class ConfigVC extends JDialog implements IViewController {
   ...
   * @param modal true if in modal mode
  ...
  /**
   * Usage: <br>
   * Just call <code>addConfNodePanel([name of the parent node], [your AbstractConfPanel]);</code>
   */
  ...
  public void export() {
    ...
    if (jfc.getSelectedFile() != null) {
      ...
      if (((FileFilterImpl) jfc.getFileFilter()).getFilterType() == FileFilterImpl.XMLFilter) {
        savePath =
            savePath + Utils.getQualifiedFileName(jfc.getSelectedFile().getName(), xmlExtensions);
      }
  ...
  public void importConf() {
    ...
    if (jfc.getSelectedFile() != null) {
      ...
      if (((FileFilterImpl) jfc.getFileFilter()).getFilterType() == FileFilterImpl.XMLFilter) {
        readPath =
            readPath + Utils.getQualifiedFileName(jfc.getSelectedFile().getName(), xmlExtensions);
      }
  ...
  private JTabbedPane getTabbedPane() {
    if (tabbedPane == null) {
      ...
      colorPanel =
          new ConfUnderstandabilityPanel(Messages.getString("Configuration.ColorLayout.Title"));
      tabbedPane.addTab(
          Messages.getString("Configuration.ColorLayout.ColorPanel.Title"), colorPanel);
      ...
      p2tPanel = new ConfNLPToolsPanel(Messages.getString("Configuration.P2T.Title"));
      tabbedPane.addTab(Messages.getString("Configuration.P2T.Title"), p2tPanel);
              ...
              if (applyConfiguration())
                if (tabbedPane.getSelectedComponent() == aproPanel
                    && (aproPanel.getNameAlreadyInUse() == true
                        || !((ConfApromorePanel) aproPanel).getInputOK())) {
                  return;
                } else ConfigVC.this.dispose();
            }
          });
    }
    return okButton;
