 ...
 *     <br>
 *     The WoPeDJGraph is a representation of <code>com.jgraph.JGraph</code>. It has its own
 *     VertexViews, Element creation and connection rules.
 *     <p>29.04.2003
 */
@SuppressWarnings("serial")
public class WoPeDJGraph extends AbstractGraph {
  private int minPreferredWidth =
      0; /// Minimal preferred width. Used to override the preferred width of the JGraph.
  private int minPreferredHeight =
      0; /// Minimal preferred height. Used to override the preferred height of the JGraph.

  public WoPeDJGraph(
      WoPeDJGraphGraphModel model, BasicMarqueeHandler editorMarquee, ViewFactory viewFactory) {
   ...
   */
  public WoPeDJGraph(
      WoPeDJGraphGraphModel model,
      BasicMarqueeHandler editorMarquee,
  ...
  static {
    ...
    tmp.put(new Integer(AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE), currentSet);
    tmp.put(new Integer(AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE), currentSet);
    tmp.put(new Integer(AbstractPetriNetElementModel.SUBP_TYPE), currentSet);
    ...
    tmp.put(new Integer(AbstractPetriNetElementModel.PLACE_TYPE), currentSet);
  ...
  public boolean isValidConnection(
      AbstractPetriNetElementModel sourceCell, AbstractPetriNetElementModel targetCell) {
    ...
    if (result == true) {
      ...
      // as NetAlgorithms already sees the connection even if it doesn't exist in the jgraph model
      // yet
      if (sourceCell.getType() == AbstractPetriNetElementModel.SUBP_TYPE) {
        ...
        for (Iterator<?> i = sourceCell.getPort().edges(); i.hasNext(); ) {
          ...
          if (o instanceof Edge) {
            Edge e = (Edge) o;
            if ((e.getSource() == sourceCell.getPort()) && (e.getTarget() != targetCell.getPort()))
              ++nNumOutgoing;
          }
        }
      ...
      if (targetCell.getType() == AbstractPetriNetElementModel.SUBP_TYPE) {
        ...
        for (Iterator<?> i = targetCell.getPort().edges(); i.hasNext(); ) {
          ...
          if (o instanceof Edge) {
            Edge e = (Edge) o;
            if ((e.getTarget() == targetCell.getPort()) && (e.getSource() != sourceCell.getPort()))
              ++nNumIncoming;
          }
        }
  ...
  public void drawNet(PetriNetModelProcessor processor) {
    for (Iterator<AbstractPetriNetElementModel> iter =
            processor.getElementContainer().getRootElements().iterator();
        iter.hasNext(); ) {
      ...
      if (element.getType() == AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE
        ...
        if (((TransitionModel) element).hasTrigger()) {
          ...
          getModel()
              .insert(
                  new Object[] {((TransitionModel) element).getToolSpecific().getTrigger()},
                  hm,
                  null,
                  pm,
                  null);
        ...
        if (((TransitionModel) element).hasResource()) {
          ...
          getModel()
              .insert(
                  new Object[] {((TransitionModel) element).getToolSpecific().getTransResource()},
                  hm,
                  null,
                  pm,
                  null);
      ...
      getGraphLayoutCache()
          .insertGroup(
              group,
              new Object[] {element, ((AbstractPetriNetElementModel) element).getNameModel()});
    }
    for (Iterator<ArcModel> iter = processor.getElementContainer().getArcMap().values().iterator();
        iter.hasNext(); ) {
   ...
   *
   * @param p a point in model coordinates.
  ...
  public Point2D snap(Point2D p) {
    if (gridEnabled && p != null) {
      double sgs = gridSize
          ...
          ;
      p.setLocation(
          Math.round(Math.round(p.getX() / sgs) * sgs),
   ...
   *
   * @param r a rectangle in model coordinates.
  ...
  public Rectangle2D snap(Rectangle2D r) {
    if (gridEnabled && r != null) {
      double sgs = gridSize
          ...
          ;
      r.setFrame(
          Math.round(Math.round(r.getX() / sgs) * sgs),
   ...
   *
   * @param d a dimension in model coordinates to snap to.
  ...
  public Dimension2D snap(Dimension2D d) {
    if (gridEnabled && d != null) {
      double sgs = gridSize
          ...
          ;
      d.setSize(
          1 + Math.round(Math.round(d.getWidth() / sgs) * sgs),
  ...
  /**
   * Sets the minimum preferred width returned by getPreferredSize()- Allows overriding of the
   * graph's preferred width.
   *
  ...
  /**
   * Sets the minimum preferred height returned by getPreferredSize(). Allows overriding of the
   * graph's preferred height.
   *
  ...
  /**
   * Returns the preferred size of the graph. The returned values are those returned by
   * super.getPreferredSize(), except when overridden by setMinPreferredWidth() and
   * setMinPreferredHeight().
   */
