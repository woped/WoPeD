...
public class Wait extends BaseActivity<TWait> {
  ...
  public static final int _DEADLINE = 0;
  public static final int _DURATION = 1;
  ...
  public final void fillWait(String Condition, int WaitConditionType) {
    ...
    if (WaitConditionType == Wait._DEADLINE) {
      this.getActivity().addNewUntil().setValue(this.generateDeadlineCondition());
    } else if (WaitConditionType == Wait._DURATION) {
      this.getActivity().addNewFor().setValue(this.generateDurationCondition());
    } else {
  ...
  public BaseActivity<?> saveInformation(BPELadditionalPanel bip) {
    ...
    if (panel
        ...
        .equalsIgnoreCase(Messages.getString("Transition.Properties.BPEL.Wait.Duration"))) {
      this.fillWait(panel.getDuration(), Wait._DURATION);
    } else if (panel
        .getSelectedRadioButton()
        .equalsIgnoreCase(Messages.getString("Transition.Properties.BPEL.Wait.Deadline"))) {
