...
public class ArcPropertyEditor extends JDialog {
  ...
  private void initialize(Point position) {
    ...
    // setup default keys
    ((JSpinner.DefaultEditor) weightSpinner.getEditor())
        .getTextField()
        .addKeyListener(defaultKeyListener);
    ((JSpinner.DefaultEditor) probabilitySpinner.getEditor())
        .getTextField()
        .addKeyListener(defaultKeyListener);
  ...
  private JPanel getGeneralSection() {
    if (this.generalSection == null) {
      ...
      generalSection.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(Messages.getString("Arc.Properties.General")),
  ...
  private JPanel getProbabilitySection() {
    if (probabilitySection == null) {
      ...
      probabilitySection.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(Messages.getString("Arc.Properties.Probability")),
  ...
  private JSpinner getWeightSpinner() {
    ...
    if (weightSpinner == null) {
      SpinnerModel weightModel =
          new SpinnerNumberModel(arc.getInscriptionValue(), 1, Integer.MAX_VALUE, 1);
  ...
  private JSpinner getProbabilitySpinner() {
    if (probabilitySpinner == null) {
      SpinnerModel probabilitySpinnerModel =
          new SpinnerNumberModel(arc.getProbability() * 100, 0, 100, 5);
  ...
  private JCheckBox getCheckBox() {
    if (displayProbabilityCheckbox == null) {
      displayProbabilityCheckbox =
          new JCheckBox(
              Messages.getString("Arc.Properties.Display.Check"), arc.displayProbability());
  ...
  private void apply() {
    ...
    if (newProbabilityVisibility != arc.displayProbability())
      arc.displayProbability(newProbabilityVisibility);
