 ...
 *     <p>This is the dialog which enables the user to choose a webservice and import the according
 *     wsdl-url to the partner-link-site
 */
...
public class UDDIDialog extends JDialog {
  ...
  protected void showErrorPopup(String title, String message) {
    ...
    errorPopup.setLocation(this.getLocation().x + 90, this.getLocation().y + 50);
...
class SetUrl implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    adaptee.showErrorPopup(
        Messages.getString("Transition.Properties.BPEL.Error"),
        Messages.getString("Transition.Properties.BPEL.UDDI.NoWSDL"));
    // adaptee.dispose();
  }
}
...
class FindBusiness implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (busname.equals("") || busname == null) {
      adaptee.showErrorPopup(
          Messages.getString("Transition.Properties.BPEL.Error"),
          Messages.getString("Transition.Properties.BPEL.UDDI.WrongValueError"));
    } else {
      // get UDDI-Server-URL
      String UDDIServerName = (String) adaptee.CBuddiServer.getSelectedItem();
      String UDDIServerURL =
          adaptee.modelElementContainer.findUddiVariableByName(UDDIServerName).getURL();

      String[] buslist = UDDI.find_business(UDDIServerURL, busname);
      if (buslist == null || buslist.length == 0) {
        adaptee.showErrorPopup(
            Messages.getString("Transition.Properties.BPEL.Error"),
            Messages.getString("Transition.Properties.BPEL.UDDI.NoResult"));
      } else {
        if (buslist.length >= 40) {
          adaptee.showErrorPopup(
              Messages.getString("Transition.Properties.BPEL.UDDI.ResultWarning"),
              Messages.getString("Transition.Properties.BPEL.UDDI.fortyResultWarning"));
        }
