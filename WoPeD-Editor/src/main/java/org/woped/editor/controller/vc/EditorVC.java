 ...
 *     <br>
 *     <p>The Editor is the basic Class for Modelling Perti-Nets. Each Editor contains one Petri-Net
 *     (Model), and one WoPeDGraph (JGraph).
 *     <p>
 *     <p>Created on 29.04.2003
 */
public class EditorVC
  ...
  public EditorVC(
    ...
    if (loadUI)
      this.m_graph =
          new WoPeDJGraph(
              new WoPeDJGraphGraphModel(this),
              marqueehandler,
  ...
  public void closeEditor() {
    ...
    this.fireViewEvent(
        new ViewEvent(this, AbstractViewEvent.VIEWEVENTTYPE_GUI, AbstractViewEvent.CLOSE, null));
    clearYourself();
    System.gc();
    try {
      Thread.sleep(1000);
  ...
  public void updateNet() {
    ...
    LoggerManager.debug(
        Constants.EDITOR_LOGGER, "Net updated. (" + (System.currentTimeMillis() - begin) + " ms)");
  }
    ...
    if (map.isValid()) {
      ...
      if (map.getPosition() != null) {
        ...
        Point point =
            new Point(
                (int)
                    ((getLastMousePosition().getX() / getGraph().getScale()
                        - element.getWidth() / 2)),
                (int)
                    ((getLastMousePosition().getY() / getGraph().getScale()
                        - element.getHeight() / 2)));
      ...
      if (element instanceof AbstractPetriNetElementModel) {
        ...
        if (map.getNamePosition() == null) {
          if (isRotateSelected()) {
            element
                .getNameModel()
                .setPosition((element.getX() + element.getWidth()), (element.getY()) + 1);
          } else {
            element
                .getNameModel()
                .setPosition((element.getX() - 1), (element.getY() + element.getHeight()));
          }
        } else {
      ...
      LoggerManager.error(
          Constants.EDITOR_LOGGER, "Could not create Element. CreationMap is NOT VALID");
      return null;
    }
  }
    ...
    if (isValidConnection(sourceId, targetId)) {
      if (connectionExists(sourceId, targetId)) {
        ...
        if (map.getArcLabelPosition() != null) {
          arc.setProbabilityLabelPosition(
              new Point2D.Double(
                  map.getArcLabelPosition().getX(), map.getArcLabelPosition().getY()));
        }
      }
    } else {
      LoggerManager.warn(
          Constants.EDITOR_LOGGER,
          "Requested connection (" + sourceId + " -> " + targetId + ") is not valid, did nothing!");
    }
  ...
  private boolean isValidConnection(String sourceId, String targetId) {
    return getGraph()
        .isValidConnection(
            getModelProcessor().getElementContainer().getElementById(sourceId),
      ...
      if (toDelete[i] instanceof ArcModel) {
        ...
        if (toDelete[i] instanceof TransitionModel) {
          if (((TransitionModel) toDelete[i]).getToolSpecific().getTrigger() != null) {
            deleteCell(
                ((TransitionModel)
                        getModelProcessor().getElementContainer().getElementById(element.getId()))
                    .getToolSpecific()
                    .getTrigger(),
                withGraph);
          }
        ...
        (DefaultGraphCell)
            getGraph()
                .getNextCellForLocation(
                    null, getLastMousePosition().getX(), getLastMousePosition().getY());
  ...
  public void copySelection() {
    ...
    while (!toBeProcessed.isEmpty()) {
      ...
      if (currentElement instanceof GroupModel) {
        ...
        currentElement =
            getModelProcessor()
                .getElementContainer()
                .getElementById(((TriggerModel) currentElement).getOwnerId());
      }
    ...
    LoggerManager.debug(
        Constants.EDITOR_LOGGER,
        "Elements copied. (" + (System.currentTimeMillis() - begin) + " ms)");
  ...
  /**
   * paste selected elements into active frame. if active frame = source frame, paste elements with
   * an offset or to the mouse position (if point is given) if active frame != source frame, paste
   * elements in middle of new frame or at mouse position (if point is given)
   */
  public void paste(Point2D... points) {
    ...
    while (eleIter.hasNext()) {
      ...
      if (this != m_clipboard.getM_sourceEditor()) {
              ...
              new Point(
                  (int)
                      (((int) points[0].getX() / 2)
                          ...
                          + tempMap.getPosition().getX()
                          - middleOfSelection.x),
                  (int)
                      (((int) points[0].getY() / 2)
                          - tempMap.getSize().getX2() / 2
                          + tempMap.getPosition().getY()
                          - middleOfSelection.y));
        } else {
          currentPosition =
              new Point(
                  (int)
                      ((getEditorPanel().getWidth() / 2)
                          - tempMap.getSize().getX1() / 2
                          + tempMap.getPosition().getX()
                          - middleOfSelection.x),
                  (int)
                      ((getEditorPanel().getHeight() / 2)
                          - tempMap.getSize().getX2() / 2
                          + tempMap.getPosition().getY()
                          - middleOfSelection.y));
        }
        ...
        // position for name
        if (isRotateSelected()) {
          tempMap.setNamePosition(
              currentPosition.x + tempMap.getSize().getX1(), currentPosition.y - 1);
        } else {
          tempMap.setNamePosition(
              currentPosition.x - 1, currentPosition.y + tempMap.getSize().getX2());
        }
      ...
      while (arcIter.hasNext()) {
        ...
        if ((this.getEditorPanel().getContainer().getActionMap().get(currentArcMap.getArcSourceId())
                != null)
            || ((currentArcMap.getArcSourceId().equals(oldElementId))
    ...
    LoggerManager.debug(
        Constants.EDITOR_LOGGER,
        "Elements pasted. (" + (System.currentTimeMillis() - begin) + " ms)");
  ...
  void pasteArcs(
      Map<String, CreationMap> pasteArcs, int deltaX, int deltaY, Vector<Object> toSelectElements) {
  ...
  /**
   * method to "move" selected elements to another position implemented as create copy of all
   * elements and delete originals afterwards
   */
  ...
  /**
   * Moves all Elementes in the Object-Array <code>toMove</code> <code>dx</code> in x-direction and
  ...
  /* ########## View and utils methods ########### */ private void move(
      Object toMove[],
      int dx,
      int dy,
      HashMap<GraphCell, AttributeMap> changes,
      boolean isrecursiv) {
    if (changes == null) {
  ...
  /**
   * Method scaleNet changes the coordinates of all elements by multiplicating with <code>factor
   * </code>.<br>
   * The values will be converted to int. This method is usefull for nets created with other tools
   * using different cooridates.
   *
   * @param factor
   */
  public void scaleNet(double factor) {

    Iterator<AbstractPetriNetElementModel> iter =
        getModelProcessor().getElementContainer().getRootElements().iterator();
  ...
  /**
   * Terminates a running token game session this net is part of. A token game session spans across
   * multiple nets if sub processes exist. This method will close all sub processes and reset the
   * token game to it's initial state, then will disable the token game for the top net
   */
  ...
  /**
   * Zooms the net. <br>
   * The factor should be between <code>MIN_SCALE</code> and <code>MAX_SCALE</code>. Multiplicate
   * the factor with 100 to get the percent value. Set <code>absolute</code>=<code>true</code> in
   * order to zoom the to the factor, not by the the factor.
   *
   * @param factor
   * @param absolute
   */
  public void zoom(double factor, boolean absolute) {
     ...
     *
     * <p>if (graphpad.getCurrentGraph() .getScale() < 8) { // "Zero Length String passed to
     * TextLayout constructor" graphpad.getCurrentDocument() . setScale(getGraph().getScale() * 2);
     * if (getGraph().getSelectionCell() != null) getGraph().scrollCellToVisible
     * (graphpad.getCurrentGraph().getSelectionCell()); }
     */
    getGraph().stopEditing();
    Rectangle2D oldVisRect =
        getGraph().fromScreen(getEditorPanel().m_scrollPane.getViewport().getViewRect());
    double scale;
    // Check if absolute
    if (absolute) {
      scale = factor / 100;
    ...
    Rectangle newVisRect =
        new Rectangle2D.Double(
                visibleRect.getX() + oldVisRect.getCenterX() - visibleRect.getCenterX(),
                visibleRect.getY() + oldVisRect.getCenterY() - visibleRect.getCenterY(),
  ...
  /**
   * Invoked after a cell has changed in some way. The vertex/vertices may have changed bounds or
   * altered adjacency, or other attributes have changed that may affect presentation.
   */
  public void graphChanged(GraphModelEvent e) {
    ...
    if (getGraph().getLastEdited() != null) {
      ...
      AbstractPetriNetElementModel aem =
          getModelProcessor().getElementContainer().getElementById(nM.getOwnerId());
      if (aem != null) {
  ...
  /**
   * Fires a ViewEvent to each listener as long as the event is not consumed. The event is also set
   * with a reference to the current listener.
   */
   ...
   *
   * @deprecated Use {@link
   *     org.woped.editor.controller.vc.EditorPanel#rotateLayout(org.woped.editor.controller.vc.EditorVC)}
   *     instead
   */
  ...
  public void startBeautify(int ixIntervall, int iyIntervall, int counter) {
    ...
    if (!sAnalysis.isWorkflowNet()) {
      JOptionPane.showMessageDialog(
          this.getEditorPanel(), Messages.getString("File.Error.GraphBeautifier.NoNet.Text"));
    } else {
  ...
  public void setCreateElementType(int createElementType) {
    ...
    VisualController.getInstance()
        .propertyChange(
            new PropertyChangeEvent(m_mediator, "DrawMode", oldValue, createElementType));
  }
  ...
  /**
   * Returns the drawing mode. If the net is in drawing mode, clicking the left mouse button will
   * draw the Element with the set creation type.
   *
   * @return drawing mode
  ...
  /**
   * Sets the drawing mode. If the net is in drawing mode, clicking the left mouse button will draw
   * the Element with the set creation type.
   *
   * @param flag
   * @see EditorVC#getCreateElementType()
   */
  ...
  public void openTokenGameSubProcess(SubProcessModel subProcess) {
    ...
    IQualanalysisService qualanService =
        QualAnalysisServiceFactory.createNewQualAnalysisService(newEditorWindow);
  ...
  /**
   * sets on of the Arrows of the mainSplitPane-divider visible or invisible based on window sizes
            ...
            subEditor =
                getMediator()
                    .createSubprocessEditor(
                        true, editor, (SubProcessModel) element.getMainElement());
            this.fireViewEvent(
                new ViewEvent(
                    subEditor, AbstractViewEvent.VIEWEVENTTYPE_GUI, AbstractViewEvent.CLOSE, null));
          }
        }
      } catch (Exception e) {
