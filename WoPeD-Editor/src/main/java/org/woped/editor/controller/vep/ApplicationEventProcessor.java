  ...
  public void processViewEvent(AbstractViewEvent event) {
    ...
    switch (event.getOrder()) {
      ...
      case AbstractViewEvent.DRAWMODE_TRANSITION:
        if (currentEditor.getCreateElementType()
      ...
      case AbstractViewEvent.CONFIG:
        ...
        if (config == null) {
          config =
              (ConfigVC)
                  getWopedMediator()
                      .createViewController(ApplicationMediator.VIEWCONTROLLER_CONFIG);
        }
        if (getMediator().getUi() != null) {
          config.setLocation(
              Utils.getCenterPoint(getMediator().getUi().getBounds(), config.getSize()));
        }
      ...
      case AbstractViewEvent.UPDATE:
        for (Iterator<IEditor> iter = getMediator().getUi().getAllEditors().iterator();
            iter.hasNext(); ) {
          currentEditor = iter.next();
          currentEditor
              .getGraph()
              .setGridVisible(ConfigurationManager.getConfiguration().isShowGrid());
          for (Iterator<String> iterator =
            ...
            currentEditor
                .getModelProcessor()
                .getElementContainer()
                .getArcById(iterator.next())
                .initAttributes();
          }

          currentEditor.updateNet();
          VisualController.getInstance()
              .propertyChange(new PropertyChangeEvent(this, "Update", null, null));
        }
        break;
