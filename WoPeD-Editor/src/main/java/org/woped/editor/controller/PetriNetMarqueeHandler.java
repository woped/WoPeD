 ...
 *     <br>
 *     The idea of the marquee handler is to act as a high - level mouse handler, with additional
 *     painting capabilites
 *     <p>29.04.2003
 */
public class PetriNetMarqueeHandler extends AbstractMarqueeHandler {
  ...
  /**
   * The mousePressed method is used to display the popup menu, or to initiate the connection
   * establishment, if the global port variable has been set.
   *
  ...
  public void mousePressed(final MouseEvent e) {
    ...
    if (getGraph().getSelectionCell() instanceof NameModel) getGraph().clearSelection();

    super.mousePressed(e);
  }

  /**
   * The mouseDragged method is messaged repeatedly, before the mouseReleased method is invoked. The
   * method is used to provide the live preview, that is, to draw a line between the source and
   * target port for visual feedback.
   *
  ...
  /**
   * The method mouseReleased is called when the mouse button is released. If a valid source and
   * target port exist, the connection is established using the editor's connect method
   *
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (getEditor().isDrawingMode()
        && (SwingUtilities.isRightMouseButton(e) || e.getClickCount() == 2)) {
      getEditor().setDrawingMode(false);
    ...
    if (SwingUtilities.isRightMouseButton(e)) {
      ...
      if (e.getClickCount() == 2) {
        ...
        if (ConfigurationManager.getConfiguration().isSmartEditing()
          ...
          Object element =
              ((firstPort != null) ? ((DefaultPort) firstPort.getCell()).getParent() : null);

          if (element instanceof AbstractPetriNetElementModel) {
            allowConnection =
                ((AbstractPetriNetElementModel) element).getAllowOutgoingConnections();
          }
          ...
          if (e != null
            ...
            CreationMap map = CreationMap.createMap();
            map.setArcSourceId(
                ((AbstractPetriNetElementModel) ((DefaultPort) source).getParent()).getId());
            map.setArcTargetId(
                ((AbstractPetriNetElementModel) ((DefaultPort) target).getParent()).getId());
            getEditor().create(map, true);
            e.consume();
          }
        }
  ...
  /**
   * The marquee handler also implements the mouseMoved method, which is messaged independently of
   * the others, to change the mouse pointer when over a port
   *
    ...
    if (e != null) {
      if (getGraph().getPortForLocation(e.getPoint().getX(), e.getPoint().getY()) != null
        ...
        e.consume();
              ...
              || cell instanceof AbstractPetriNetElementModel
              || cell instanceof NameModel)
          && getGraph().isEnabled()) {
        // Set Cursor on Graph (Automatically Reset)
  ...
  public boolean isUngroupable(Object object) {
    return (object == null
        ? false
        : object instanceof GroupModel && ((GroupModel) object).isUngroupable());
  }
}
