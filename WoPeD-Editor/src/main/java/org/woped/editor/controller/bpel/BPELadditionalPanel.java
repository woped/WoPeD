 ...
 *     <p>This is the basic class for the different BPEL activity panels. It contains methods and
 *     data that is used in the activity panels' dialogs.
 *     <p>Created on 16.01.2008
 */
...
public abstract class BPELadditionalPanel extends JPanel {
  ...
  protected void showNewPartnerLinkDialog() {
    ...
    dialog.add(
        new JLabel(Messages.getString("Transition.Properties.BPEL.NewVariable.Name") + ":"), c);
    ...
    c.fill = GridBagConstraints.HORIZONTAL;
    dialog.add(
        new JLabel(Messages.getString("Transition.Properties.BPEL.PartnerLinkType") + ":"), c);
  ...
  protected void showNewVariableDialog() {
    dialog = new JDialog(t_editor, true);
    dialog.setVisible(false);
    dialog.setTitle(Messages.getString("Transition.Properties.BPEL.NewVariable"));
    ...
    if (this.VariableName == null) this.VariableName = new JTextField("");
    ...
    c.insets = new Insets(0, 5, 0, 0);
    dialog.add(
        new JLabel(Messages.getString("Transition.Properties.BPEL.NewVariable.Type") + ":"), c);
  ...
  private JTextField getWSDLFileTextField() {
    if (wsdlFileTextField == null) {
      ...
      wsdlFileTextField.addKeyListener(
          new KeyListener() {
  ...
  private JButton getLocalWSDLButton() {
    if (searchLocalWSDLButton == null) {
      ...
      searchLocalWSDLButton.addActionListener(
          new ActionListener() {
  ...
  private JButton getReadWSDLButton() {
    if (readWSDLButton == null) {
      ...
      readWSDLButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              tryToGetDataFromWsdl();
            }
  ...
  private JButton getUDDIButton() {
    if (searchByUDDIButton == null) {
      ...
      searchByUDDIButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
  ...
  private JComboBox getPartnerLinkTypeComboBox() {
    if (partnerLinkTypeComboBox == null) {
      ...
      partnerLinkTypeComboBox.addItemListener(
          new ItemListener() {
  ...
  private JButton getPartnerOKButton() {
    if (okPartnerButton == null) {
      ...
      okPartnerButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              ...
              if ((partnerLinkTypeComboBox.getItemCount() != 0)
                  && (partnerRoleComboBox.getItemCount() != 0)) {
                String partnerLinkType =
                    partnerLinkTypeComboBox.getSelectedItem().toString().trim();
                ...
                if (name.equals("")
                    ...
                    || (partnerRole.equals(Messages.getString("Transition.Properties.BPEL.NoRole"))
                        && myRole.equals(
                            Messages.getString("Transition.Properties.BPEL.NoRole")))) {
                  showErrorPopup(
                      Messages.getString("Transition.Properties.BPEL.MissingEntries"),
                ...
                // check if a partner link with this name has already been created
                  ...
                  // if the messages of the WSDL file were already saved as variables in the
                  // datamodel don't save them for a second time.
                ...
                showErrorPopup(
                    Messages.getString("Transition.Properties.BPEL.NoPartnerLinks"),
  ...
  private JButton getPartnerCancelButton() {
    if (cancelPartnerButton == null) {
      cancelPartnerButton =
          new JButton(Messages.getString("Transition.Properties.BPEL.Buttons.Cancel"));
      cancelPartnerButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              dialog.dispose();
            }
  ...
  private JButton getVariableOKButton() {
    ...
    okVariableButton.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            modelElementContainer.addVariable(
                VariableName.getText(), getVariableTypesComboBox().getSelectedItem().toString());
            dialog.dispose();
          }
        });
  ...
  private JButton getVariableCancelButton() {
    if (cancelVariableButton == null) {
      cancelVariableButton =
          new JButton(Messages.getString("Transition.Properties.BPEL.Buttons.Cancel"));
      cancelVariableButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              dialog.dispose();
            }
  ...
  public void addVariablesToModelElementContainer(String pathToWsdlFile) {
    try {
      ...
      for (Message message : messages) {
        modelElementContainer.addVariable(
            ("var_" + message.getMessageName()), message.getMessageName());
        ;
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public String getPartnerLinkName() {
    if (partnerLinkNameTextField.getText() == null) return null;
    return partnerLinkNameTextField.getText().toString();
  }

  public String getPartnerLinkType() {
    if (partnerLinkTypeComboBox.getSelectedItem() == null) return null;
    return partnerLinkTypeComboBox.getSelectedItem().toString();
  }

  public String getPartnerRole() {
    if (partnerRoleComboBox.getSelectedItem() == null) return null;
    return partnerRoleComboBox.getSelectedItem().toString();
  }

  public String getMyRole() {
    if (myRoleComboBox.getSelectedItem() == null) return null;
  ...
  public void tryToGetDataFromWsdl() {
    ...
    if (wsdlFileTextField.getText().length() == 0) {
      ...
      showErrorPopup(
          Messages.getString("Transition.Properties.BPEL.InvalidFilePath"),
      ...
      try {
        ...
        showErrorPopup(
            Messages.getString("Transition.Properties.BPEL.ErrorWhileReadingWsdlFile"),
            Messages.getString("Transition.Properties.BPEL.ErrorWhileReadingWsdlFileTitle"));
  ...
  protected void showErrorPopup(String title, String message) {
    ...
    errorPopup.setLocation(dialog.getLocation().x + 90, dialog.getLocation().y + 50);
  ...
  protected void fillVariableToComboBox(JComboBox box) {
    ...
    while (iter.hasNext()) box.addItem(iter.next());
  }
