...
/**
 * The Class <code>SGYGraph</code> represents a Graph, containing SGYElements. This graph can be
 * beautified and drawn.
 *
...
public class SGYGraph {
  ...
  /**
   * lineType is required for the optimal distance; 0 = Standard; 1 = there are Triggers; 2 = there
   * are Triggers and Resources
   */
   ...
   * @see <a href="https://en.wikipedia.org/wiki/Layered_graph_drawing">Sugiyama-style graph
   *     drawing</a>
   */
  public void beautify(int forgivenessCounter) {
    this.removeArcPoints();
    this.addDummyVertices();
    if (forgivenessCounter > 0) this.forgivenessCounter = forgivenessCounter;
  ...
  public void draw(int x, int y) {
    ...
    if (editor.getGraph().getUndoManager() != null) {
      ((WoPeDUndoManager) editor.getGraph().getUndoManager()).setEnabled(false);
    }
    ...
    for (ArrayList<SGYElement> aGraph : graph) {
      for (SGYElement anAGraph : aGraph) {
        ...
        if (!modelToArrange.isDummyVertex) {
          xPosition = xOffset + modelToArrange.hierarchy * ixInterval;
          yPosition = modelToArrange.yCoordinate * iyInterval;
          modelToArrange.editorElement.setPositionGroup(xPosition, yPosition);
        } else {
          if (modelToArrange.dummyTarget.hierarchy - modelToArrange.dummySource.hierarchy > 2) {
            xPosition = xOffset + modelToArrange.hierarchy * ixInterval;
            yPosition = modelToArrange.yCoordinate * iyInterval;
            ArcModel arc = this.getArc(modelToArrange.dummySource, modelToArrange.dummyTarget);
            Point addArcPoint = new Point(xPosition + 20, yPosition + 20);
    ...
    if (editor.getGraph().getUndoManager() != null) {
      ((WoPeDUndoManager) editor.getGraph().getUndoManager()).setEnabled(true);
    }
  }
  ...
  /** Gets all Elements, builds parent/child-relationships, assigns the hierarchies */
  ...
  /**
   * Goes through the Graph for all model's children (and grandchildren) and sets their hierarchy
   *
  ...
  private void setChildrenHierarchies(SGYElement ele) {
    ...
    while (childIterator.hasNext()) {
      ...
      if (child.hierarchy < ele.hierarchy + 1) child.hierarchy = ele.hierarchy + 1;
      if (maxHierarchy < ele.hierarchy + 1) maxHierarchy = ele.hierarchy + 1;
      if (!child.children.isEmpty()) setChildrenHierarchies(child);
    }
  }

  /**
   * Flips an arc, changes the relations of the relevant elements and changes the assignment of
   * itself to an element
   */
  ...
  /** cycle-detection: Fills the <@code arcIDsToTurn> with the arcs that lead to a cycle. */
  private void detectCycles(SGYElement element) {
    ...
    for (ArcModel arc : element.arcsOut) {
      ...
      // checking if the following element was already visited. If this is the case, here is a
      // cycle.
      if (nextElement.cycleDetectionVisited == 0) detectCycles(nextElement);
      else if (nextElement.cycleDetectionVisited == 1) cycleArcs.add(arc);
    }
  ...
  /**
   * Add dummy vertices to the graph. Fills the connection between two elements with Dummy Vertices
   * if hierarchy difference is greater than 1
   */
    ...
    for (ArrayList<SGYElement> aGraph : this.graph) {
      for (SGYElement model : aGraph) {
        ...
        for (SGYElement child : model.children) {
          int iHierarchyDifference = child.hierarchy - model.hierarchy;
          if (iHierarchyDifference > 1) {
            ...
            for (int k = 0; k < singleDummyVertices.length; k++) {
              singleDummyVertices[k] = new SGYElement(null, true, model, child);
            }
            int arrayCounter = 0;
            for (int k = model.hierarchy + 1; k < child.hierarchy; k++) {
              ...
              if (singleDummyVertices.length > 1) {
                if (arrayCounter == 0) {
                  singleDummyVertices[arrayCounter].parents.add(model);
                  singleDummyVertices[arrayCounter].children.add(
                      singleDummyVertices[arrayCounter + 1]);
                  firstDummyVertices.add(singleDummyVertices[arrayCounter]);
                } else if (arrayCounter == singleDummyVertices.length - 1) {
                  singleDummyVertices[arrayCounter].parents.add(
                      singleDummyVertices[arrayCounter - 1]);
                  singleDummyVertices[arrayCounter].children.add(child);
   ...
   *
   * <p>Removes a dummy vertices if parent and child have the same y-coordinate as the dummy.
   */
      ...
      for (SGYElement model : aGraph) {
        ...
        if (model.isDummyVertex) {
          ...
          if (child != null && parent != null) {
            if (parent.yCoordinate == model.yCoordinate && model.yCoordinate == child.yCoordinate) {
  ...
  /** sweeps through the graph in order to find solution with minimum of crossing arcs */
      ...
      for (int i = 1; i < copyGraph.size(); i++) {
        ...
        for (int j = 0; j < copyGraph.get(i).size(); j++) {
          ...
          for (SGYElement parent : model.parents) {
            neighbour = parent;
            sumIndex = sumIndex + copyGraph.get(i - 1).indexOf(neighbour);
          }
          if (copyGraph.get(i).get(j).parents.isEmpty()) {
            copyGraph.get(i).get(j).barycenter = sumIndex;
          } else {
            copyGraph.get(i).get(j).barycenter = sumIndex / copyGraph.get(i).get(j).parents.size();
            sumIndex = 0;
          }
        }
        newOrder(copyGraph, i);
        direction = true;
        ArrayList<SGYElement> layerN1 = (ArrayList<SGYElement>) copyGraph.get(i - 1).clone();
        ArrayList<SGYElement> layerN2 = (ArrayList<SGYElement>) copyGraph.get(i).clone();
        global_crossings = global_crossings + (crossCount(layerN1, layerN2, direction));
      }
      ...
      for (int i = copyGraph.size() - 1; i >= 1; i--) {
        ...
        for (int j = 0; j < copyGraph.get(i - 1).size(); j++) {
          for (SGYElement aChildren : copyGraph.get(i - 1).get(j).children) {
            neighbour = aChildren;
            sumIndex = sumIndex + copyGraph.get(i).indexOf(neighbour);
          }
          if (copyGraph.get(i - 1).get(j).children.isEmpty()) {
            copyGraph.get(i - 1).get(j).barycenter = sumIndex;
          } else {
            copyGraph.get(i - 1).get(j).barycenter =
                sumIndex / copyGraph.get(i - 1).get(j).children.size();
            sumIndex = 0;
          }
        }
        newOrder(copyGraph, i - 1);
        direction = false;
        ArrayList<SGYElement> layerN1 = (ArrayList<SGYElement>) copyGraph.get(i).clone();
        ArrayList<SGYElement> layerN2 = (ArrayList<SGYElement>) copyGraph.get(i - 1).clone();
        global_crossings = global_crossings + (crossCount(layerN1, layerN2, direction));
      }
      if (breakCondition(copyGraph)) {
        break;
  ...
  /** arranges the elements of a layer in a new order according to the barycenter */
  private ArrayList<ArrayList<SGYElement>> newOrder(
    ...
    for (int j = 0; j < copyGraph.get(i).size() - 1; j++) {
      for (int k = j + 1; k < copyGraph.get(i).size(); k++) {
        if (copyGraph.get(i).get(j).barycenter == copyGraph.get(i).get(k).barycenter) {
    ...
    for (int j = 1; j < copyGraph.get(i).size(); j++) {
      ...
      int k = j - 1;
      while (k >= 0 && copyGraph.get(i).get(k).barycenter > (element.barycenter)) {
  ...
  private int crossCount(
      ArrayList<SGYElement> layerN1, ArrayList<SGYElement> layerN2, boolean direction) {
  ...
  /**
   * includes the breaking condition; if number of crossings is zero or there are no further
   * improvements (forgivenessCounter = 0), it will stop crossing reduction
   */
  @SuppressWarnings("unchecked")
  ...
  private void verticalCoordinateAssignment() {
    ...
    for (int i = graph.size() - 1; i >= 0; i--) { // going through the graph from the last layers
      ...
      int maxLayerY = 0;
      takenPlacesInLayer =
          new HashSet<>(); // containing the places that are already used by an element in the
      // actual layer
      for (SGYElement anActLayer : actLayer) {
        ...
        // it is possible that the planned best position is already in use. In this cas we try to
        // place it in the next possible place after it.
        for (int k = 0; k < 1000000; k++) {
          if (!takenPlacesInLayer.contains(planPos + k)) {
            actElement.yCoordinate = planPos + k;
            if (planPos + k > maxLayerY) maxLayerY = planPos + k;
  ...
  private void removeArcPoints() {
    ...
    for (ArcModel arc :
        this.editor.getModelProcessor().getElementContainer().getArcMap().values()) {
