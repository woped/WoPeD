...
public class PlaceView extends PetriNetElementView {
  ...
  private class PlaceRenderer extends PetriNetElementRenderer {
    ...
    public void paint(Graphics g) {
      ...
      int relevantTokens =
          (PlaceView.this.getVirtualTokenCount() == PlaceView.this.getTokenCount())
              ? PlaceView.this.getTokenCount()
              : PlaceView.this.getVirtualTokenCount();
      switch (relevantTokens) {
        case 1:
          g.setColor(Color.BLACK);
          g.fillOval(
              b + (int) (d.getWidth() / 2) - TOKEN_RADIUS,
              b + (int) (d.getHeight() / 2) - TOKEN_RADIUS,
              TOKEN_RADIUS * 2,
              TOKEN_RADIUS * 2);
          break;
        case 2:
          g.setColor(Color.BLACK);
          g.fillOval(
              b + (int) (d.getWidth() / 3) - TOKEN_RADIUS,
              b + (int) (d.getHeight() / 2) - TOKEN_RADIUS,
              TOKEN_RADIUS * 2,
              TOKEN_RADIUS * 2);
          g.fillOval(
              b + (int) (d.getWidth() / 3 * 2) - TOKEN_RADIUS,
              b + (int) (d.getHeight() / 2) - TOKEN_RADIUS,
              TOKEN_RADIUS * 2,
              TOKEN_RADIUS * 2);
          break;
        case 3:
          ...
          g.fillOval(
              b + (int) (d.getWidth() / 2) - TOKEN_RADIUS,
              b + (int) (d.getHeight() / 3) - TOKEN_RADIUS,
              TOKEN_RADIUS * 2,
              TOKEN_RADIUS * 2);
          g.fillOval(
              b + (int) (d.getWidth() / 3) - TOKEN_RADIUS,
              b + (int) (d.getHeight() / 3 * 2) - TOKEN_RADIUS,
              TOKEN_RADIUS * 2,
              TOKEN_RADIUS * 2);
          g.fillOval(
              b + (int) (d.getWidth() / 3 * 2) - TOKEN_RADIUS,
