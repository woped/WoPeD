...
public class PetriNetElementView extends AbstractElementView {
  ...
  protected abstract class PetriNetElementRenderer extends VertexRenderer {
    ...
    protected Color getFillColor() {
      return getFillColor(
          ((AbstractPetriNetElementModel) (PetriNetElementView.this.getCell())).getColor());
    }
    ...
    protected Color getFillColor(Color understandabilityColor) {
      ...
      if (((AbstractPetriNetElementModel) (PetriNetElementView.this.getCell())).isHighlighted())
        ...
        return result;
      } else if (((AbstractPetriNetElementModel) (PetriNetElementView.this.getCell()))
          .isRGHighlighted()) {
    ...
    protected Color getInnerDrawingsColor() {
      ...
      if (isActive()) return (new Color(50, 200, 100));
      else {
        if (bordercolor == null) return Color.BLACK;
    ...
    // ! @param directionRightSide if true, draw the arrow such that it points to the right side of
    // the element
    protected void drawOperatorArrow(
        Graphics g, boolean positionRightSide, boolean directionRightSide) {
      Dimension d = getSize();
      int b = borderWidth;
      ...
      g.setColor(
          new Color(drawingColor.getRed(), drawingColor.getGreen(), drawingColor.getBlue(), 25));
    ...
    // XX
    protected void drawOperatorArrow2(
        Graphics g, OperatorPosition arrowPosition, OperatorDirection arrowDirection) {
