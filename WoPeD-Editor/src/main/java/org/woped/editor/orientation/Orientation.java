...
/**
 * @author Projectgroup09 handels the rotation of the whole view and the single elements
 *     <p>created on 20091202
 */
public class Orientation {
  ...
  /**
   * rotates the elements including names (caption), arcs, resources, triggers for the whole view
   *
  ...
  public void rotateView(ModelElementContainer mec) {
    ...
    while (elementsIterator.hasNext()) {
      ...
      if (element != null) {
        ...
        element
            .getNameModel()
            .setPosition(element.getNameModel().getY(), element.getNameModel().getX());
        ...
        if (element instanceof TransitionModel) {
          ...
          if (tm.hasTrigger()) {
            ...
            if (newX < negX) negX = newX;
            if (newY < negY) negY = newY;
          }
          if (tm.hasResource()) {
    ...
    for (String elementId : mec.getIdMap().keySet()) {
      ...
      if (element != null) {
        ...
        element
            .getNameModel()
            .setPosition(
                element.getNameModel().getX() + moveX, element.getNameModel().getY() + moveY);
  ...
  private void rotateTransitionIcon(TransitionModel transition, int direction) {
    ...
    if (direction == TURN_LEFT) {
      newPosition =
          (currentPosition + (OperatorPosition.values().length - 1))
              % OperatorPosition.values().length;
    } else {
