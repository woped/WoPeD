...
public class EditorSize {
   ...
   *
   * @param dim the dimension to set as new size
   * @param downSize true if editor can be smaller than before resize
   */
  private void setSize(Dimension dim, boolean downSize) {
    ...
    if (!downSize) {
      if (currentSize.height > dim.height) dim.height = currentSize.height;
      if (currentSize.width > dim.width) dim.width = currentSize.width;
    }
    ...
    } else if (editorSizeWithSideBar != null) editorSizeWithSideBar = null;
  }
  ...
  private void calculateNewEditorSize() {
    ...
    if (newEditorSize.width > maxEditorSize.width) newEditorSize.width = maxEditorSize.width;
