...
public class EditorVC extends JPanel implements KeyListener, GraphModelListener, ClipboardOwner, GraphSelectionListener, IEditor {
	...
	public EditorVC(String id, EditorClipboard clipboard, int modelProcessorType, boolean undoSupport) {
		...
		if (!DefaultStaticConfiguration.ACTIVATE_TREE_VIEW) {
			...
			add(m_scrollPane);
		} else { // Furute Feature with treeview
			...
			if (transition != null && transition instanceof TransitionModel) {
				...
				if (transition.getParent() instanceof GroupModel) {
					...
					if (map.getTriggerPosition() == null) {
						...
						map.setTriggerPosition(map.getTriggerPosition().x == -1 ? transition.getX() + 10 : map.getTriggerPosition().x, map.getTriggerPosition().y == -1 ? transition.getY() - 20 : map
								.getTriggerPosition().y);
					}
		...
		if (map.isValid()) {
			...
			if (element instanceof PetriNetModelElement) {
				...
				if (map.getNamePosition() == null)
					map.setNamePosition((int) (element.getX() - 1), (int) (element.getY() + (element.getHeight())));
				if (map.getName() == null)
					map.setName(element.getId().toString());
				...
				if (map.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE || map.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE || map.getType() == PetriNetModelElement.SUBP_TYPE) {
					...
					if (map.getTriggerType() != -1)
						createTrigger(map);
					if (map.getTriggerPosition() != null)
						((TransitionModel) element).getToolSpecific().getTrigger().setPosition(map.getTriggerPosition());
					if (map.getResourceOrgUnit() != null && map.getResourceRole() != null)
						createTransitionResource(map);
					if (map.getResourcePosition() != null)
						((TransitionModel) element).getToolSpecific().getTransResource().setPosition(map.getResourcePosition());
	...
	public void deleteOnlyCells(Object toDelete[], boolean withGraph) {
		...
		if (withGraph)
			getGraph().getModel().remove(toDelete);
		updateNet();
		getGraph().repaint();
	}
	...
	public void pasteAtPosition(int x, int y) {
		...
		if (x == -1 && y == -1) {
			...
			while (eleIter.hasNext()) {
				...
				if (leftestElement.getPosition().x > currentMap.getPosition().x)
					leftestElement = (CreationMap) currentMap.clone();
				else if (leftestElement.getPosition().x == currentMap.getPosition().x && leftestElement.getPosition().y < currentMap.getPosition().y)
					leftestElement = (CreationMap) currentMap.clone();
			}
