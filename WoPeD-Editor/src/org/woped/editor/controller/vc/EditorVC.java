			...
			if (element instanceof PetriNetModelElement) {
				...
				if (map.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE || map.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE || map.getType() == PetriNetModelElement.SUBP_TYPE) {
					...
					if (map.getTriggerType() != -1)
						createTrigger(map);
					if (map.getTriggerPosition() != null)
						((TransitionModel) element).getToolSpecific().getTrigger().setPosition(map.getTriggerPosition().getX1(), map.getTriggerPosition().getX2());
					if (map.getResourceOrgUnit() != null && map.getResourceRole() != null)
						createTransitionResource(map);
					if (map.getResourcePosition() != null)
						((TransitionModel) element).getToolSpecific().getTransResource().setPosition(map.getResourcePosition().getX1(), map.getResourcePosition().getX2());
	...
	public void pasteAtPosition(int x, int y) {
		...
		if (x == -1 && y == -1) {
			...
			while (eleIter.hasNext()) {
				...
				if (leftestElement.getPosition().getX1() > currentMap.getPosition().getX1())
					leftestElement = (CreationMap) currentMap.clone();
				else if (leftestElement.getPosition().getX1() == currentMap.getPosition().getX1() && leftestElement.getPosition().getX2() < currentMap.getPosition().getX2())
					leftestElement = (CreationMap) currentMap.clone();
			}
