...
public class GraphTreeModelSelector implements TreeSelectionListener, GraphSelectionListener {
	...
	{		
		...
		Map<AbstractElementModel,NodeNetInfo> treeItemMap =
			new HashMap<AbstractElementModel, NodeNetInfo>();
		...
		// Temporary selection path for adding / removing to the tree selection path
		...
		for (int i = 0;i<cells.length;++i)
		{
			...
			if (currentCell instanceof AbstractElementModel)			
			{
				...
				NodeNetInfo correspondingTreeItem =
					treeItemMap.get(current);
