...
public class PetriNetMarqueeHandler extends AbstractMarqueeHandler {
    ...
    public void mousePressed(final MouseEvent e) {
        ...
        if (SwingUtilities.isRightMouseButton(e)) {
            ...
            if (getEditor().getGraph().getFirstCellForLocation(l.getX(), l.getY()) != getEditor().getGraph()
                    .getSelectionCell()) {
    ...
    /**
     * The mouseDragged method is messaged repeatedly, before the mouseReleased method is invoked. The method is used to provide the livepreview, that is, to
    ...
    /**
     * The method mouseReleased is called when the mouse button is released. If a valid source and target port exist, the connection is established using the
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (e != null)// can be called when a key is pressed
        {
            ...
            if (SwingUtilities.isRightMouseButton(e)) {
                ...
                Object c = getEditor().getGraph().getFirstCellForLocation(getEditor().getLastMousePosition().getX(),
                        getEditor().getLastMousePosition().getY());
                // Display PopupMenu
                PopupMenuPetrinet.getInstance().show(c, getEditor().getGraph(),
                ...
                if (e.getClickCount() == 2) {
                    ...
                    if (ConfigurationManager.getConfiguration().isSmartEditing() && port == null && firstPort != null
                        ...
                        if (allowConnection) {
                            ...
                            map.setArcSourceId(((AbstractElementModel) (source).getParent()).getId());
                    ...
                    if (getEditor().getGraph().getFirstCellForLocation(e.getX(), e.getY()) instanceof ArcModel
                            && e.isShiftDown()) {
                        getEditor().setLastMousePosition(e.getPoint());
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (e != null && getEditor().isDrawingMode()) {
            ...
            if (e != null) {
                if (getEditor().getGraph().getPortForLocation(e.getPoint().getX(), e.getPoint().getY()) != null
                        && getEditor().getGraph().isEnabled() /* && getEditor().getGraph().isPortsVisible() */) {
                    ...
                    if ((cell instanceof GroupModel || cell instanceof AbstractElementModel || cell instanceof NameModel)
                            && getEditor().getGraph().isEnabled()) {
                        // Set Cusor on Graph (Automatically Reset)
                        getEditor().getGraph().setPortsVisible(true);
