 ...
 * 13.10.2003
 */
public class WoPeDAction extends AbstractAction implements WoPeDLogger
{
    ...
    public void setSelected(boolean selected)
    {
        if (m_selected != selected)
        {
            ...
            for (int i = 0; i < getPropertyChangeListeners().length; i++)
            {
                ...
                {
                    logger.error("Could not select Object", e);
                }
            }
        }
    }
    ...
    protected static boolean setSelected(Object obj, boolean status)
    {
        if (obj != null)
        {
            ...
            } catch (Exception e)
            {
                if (obj != null)
                {
                    logger.warn("Could not change the status for " + obj.getClass(), e);
                } else
                {
                    logger.error("Could not change the status. Object was null!", e);
                }
            }
