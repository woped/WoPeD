...
import org.woped.controller.AbstractMarqueeHandler;
import org.woped.controller.AbstractViewEvent;
import org.woped.controller.ApplicationMediator;
import org.woped.controller.EditorClipboard;
...
import org.woped.controller.WoPeDUndoManager;
import org.woped.controller.petrinet.PetriNetMarqueeHandler;
import org.woped.controller.uml.UMLMarqueeHandler;
import org.woped.gui.IEditorProperties;
...
import org.woped.model.UMLModelProcessor;
import org.woped.model.petrinet.GraphTreeModel;
...
import org.woped.utilities.ImageSelection;
 ...
 */
public class EditorVC extends JPanel implements KeyListener, GraphModelListener, ClipboardOwner, GraphSelectionListener, WoPeDLogger, IEditor
{
    ...
    public void updateNet()
    {
        ...
        logger.debug("Net updated. (" + (System.currentTimeMillis() - begin) + " ms)");
    }
        ...
        if (getModelProcessor().getProcessorType() == AbstractModelProcessor.MODEL_PROCESSOR_PETRINET)
        {
            ...
            if (transition != null && transition instanceof TransitionModel)
            {
                ...
                if (transition.getParent() instanceof GroupModel)
                {
                    ...
                    {
                        map.setTriggerPosition(map.getTriggerPosition().x == -1 ? transition.getX() + 10 : map.getTriggerPosition().x, map.getTriggerPosition().y == -1 ? transition.getY() - 20 : map.getTriggerPosition().y);
                    }
    ...
    public TransitionResourceModel createTransitionResource(CreationMap map)
    {
        ...
        if (transition != null && transition instanceof TransitionModel)
        {
            ...
            if (transition.getParent() instanceof GroupModel)
            {
                ...
                {
                    map.setResourcePosition(map.getResourcePosition().x == -1 ? transition.getX() - 65 : map.getResourcePosition().x, map.getResourcePosition().y == -1 ? transition.getY() - 20 : map.getResourcePosition().y);
                }
    ...
    public AbstractElementModel createElement(CreationMap map)
    {
        if (map.isValid())
        {
            ...
            {
                Point point = new Point((int) (map.getPosition().getX() / getGraph().getScale()), (int) (map.getPosition().getY() / getGraph().getScale()));
                map.setPosition((Point) getGraph().snap(point));
            ...
            if (element instanceof PetriNetModelElement)
            {
                ...
                ((PetriNetModelElement) element).getNameModel().setPosition(map.getNamePosition());
                ...
                if (map.getType() == PetriNetModelElement.TRANS_SIMPLE_TYPE || map.getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE || map.getType() == PetriNetModelElement.SUBP_TYPE)
                {
                    ...
                    if (map.getTriggerPosition() != null) ((TransitionModel) element).getToolSpecific().getTrigger().setPosition(map.getTriggerPosition());
                    if (map.getResourceOrgUnit() != null && map.getResourceRole() != null) createTransitionResource(map);
                    if (map.getResourcePosition() != null) ((TransitionModel) element).getToolSpecific().getTransResource().setPosition(map.getResourcePosition());
            ...
            } else if (element instanceof AbstractUMLElementModel)
            {
                if (element.getType() == AbstractUMLElementModel.ACTIVITY_TYPE && map.getImageIcon() != null)
                {
                    ...
                    if (map.getSize() != null)
                    {
                        ((ActivityModel) element).setSize(map.getSize());
                    } else
                    {
        ...
        {
            logger.error("Could not create Element. CreationMap is NOT VALID");
            return null;
        }
    }
    ...
    public ArcModel createArc(CreationMap map)
    {
        ...
        if (getGraph().isValidConnection(getModelProcessor().getElementContainer().getElementById(sourceId), getModelProcessor().getElementContainer().getElementById(targetId)))
        {
                    ...
                    || getModelProcessor().getElementContainer().getElementById(targetId).getType() == PetriNetModelElement.TRANS_OPERATOR_TYPE)
            {
                arc = getModelProcessor().createArc(map.getArcId(), sourceId, targetId, pointArray, true);
                getGraph().connect(arc);
                logger.debug("TODO: check this: " + sourceId + " -> " + targetId + " does already exists. DID IT ANYWAY ");
            } else
            {
                logger.debug("?");
            }
        ...
        {
            logger.warn("Requested connection (" + sourceId + " -> " + targetId + ") is not vaild, did nothing!");
        }
    ...
    public void edit(Object cell)
    {
        ...
        {
            logger.warn("No editing possible.");
        }
    }
    ...
    public void copySelection()
    {
        ...
        logger.debug("Elements copied. (" + (System.currentTimeMillis() - begin) + " ms)");
    ...
    public void pasteAtLastMousePosition()
    {
        ...
        {
            logger.warn("No last mouse position found. Elements pasted free will instead.");
            pasteAtPosition(-1, -1);
        }
    }
    ...
    public void pasteAtPosition(int x, int y)
    {
        ...
        } else
        {
            ...
            while (eleIter.hasNext())
            {
                ...
                if (leftestElement.getPosition().x > currentMap.getPosition().x) leftestElement = (CreationMap) currentMap.clone();
                else if (leftestElement.getPosition().x == currentMap.getPosition().x && leftestElement.getPosition().y < currentMap.getPosition().y) leftestElement = (CreationMap) currentMap.clone();
            }
            Point position = leftestElement.getPosition();
            deltaX = x - position.x;
            deltaY = y - position.y;
        }
        ...
        while (eleIter.hasNext())
        {
            ...
            currentMap.setPosition(currentPosition.x + deltaX, currentPosition.y + deltaY);
            // position for name
        ...
        {
            currentArcMap = (CreationMap) pasteArcs.get(arcIter.next());
            if (getModelProcessor().getElementContainer().getElementById(currentArcMap.getArcSourceId()) != null && getModelProcessor().getElementContainer().getElementById(currentArcMap.getArcTargetId()) != null)
            {
        ...
        logger.debug("Elements pasted. (" + (System.currentTimeMillis() - begin) + " ms)");
        updateNet();
    ...
    public void toggleTokenGame()
    {
        if (isTokenGameMode())
        {
            logger.debug("STOP TokenGame");
            m_tokenGameMode = false;
            m_tokenGameController.stop();
        } else
        {
            logger.debug("START TokenGame");
    ...
    public void makeScreenshot()
    {
        ...
        if (cells.length > 0)
        {
            ...
            try
            {
                ...
                logger.debug("Copied Net to Clipboard.");
    ...
    public void zoom(double factor, boolean absolute)
    {
        ...
        getGraph().scrollRectToVisible(
                new Rectangle2D.Double(newVisRect.getX() + oldVisRect.getCenterX() - newVisRect.getCenterX(), newVisRect.getY() + oldVisRect.getCenterY() - newVisRect.getCenterY(), newVisRect.getWidth(), newVisRect.getHeight())
                        .getBounds());
    }
