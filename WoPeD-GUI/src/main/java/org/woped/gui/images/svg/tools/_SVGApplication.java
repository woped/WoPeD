            ...
            if (choice == JFileChooser.APPROVE_OPTION) {
              ...
              try {
                ...
                SvgTranscoder transcoder =
                    new SvgTranscoder(f.toURI().toURL().toString(), svgClassName);
                transcoder.setJavaToImplementResizableIconInterface(true);
                transcoder.setListener(
    ...
    svgCanvas.addGVTTreeBuilderListener(
        new GVTTreeBuilderAdapter() {
          ...
          private void dumpPaint(PrintWriter pw, LinearGradientPaint paint, int offset) {
            ...
            pw.println(
                "paint = new LinearGradientPaint(new Point2D.Double("
                    + startPoint.getX()
                    + ", "
                    + startPoint.getY()
                    + "), new Point2D.Double("
          ...
          private void dumpPaint(PrintWriter pw, RadialGradientPaint paint, int offset) {
            ...
            pw.println(
                "paint = new RadialGradientPaint(new Point2D.Double("
                    + centerPoint.getX()
                    + ", "
                    + centerPoint.getY()
                    + "), "
          ...
          private void dumpCompositeShapePainter(
              PrintWriter pw, CompositeShapePainter painter, int offset) {
            // offset(offset);
          ...
          private void dumpStrokeShapePainter(
              PrintWriter pw, StrokeShapePainter painter, int offset) {
            Shape shape = painter.getShape();
            // offset(offset);
          ...
          private void dumpNode(PrintWriter pw, GraphicsNode node, String comment) {
            ...
            if (composite != null) {
              ...
              pw.println(
                  "g.setComposite(AlphaComposite.getInstance(" + rule + ", " + alpha + "f));");
            }
