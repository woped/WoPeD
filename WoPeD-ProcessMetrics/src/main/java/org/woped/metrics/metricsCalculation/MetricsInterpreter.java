...
/**
 * @author Tobias Lorentz This class handels the parsing-process from technical side, tries to
 *     handle exception or to solve problems automatically
 */
public class MetricsInterpreter {
   ...
   */
  public static double interpretString(
      String formula,
    ...
    try {
      ...
      metricsGrammarParser parser =
          new metricsGrammarParser(tokenStream, mc, stack, doNotDisplay, syntaxCheck);
      return parser.evaluator();
    } catch (RecognitionException re) {
      // This Exception will only be thrown if there is a general Error like a letter which is not
      // allowed
      return caughtRecognitionException(re, formula, mc, stack, doNotDisplay, syntaxCheck);
    } catch (FormulaVariableNotFoundException fvnfe) {
      // This Exception will be thrown if the Variable does not exist. If a math-function is false
      // spelled,
      // this error will also be thrown!!!
      return caughtFormulaVariableNotFoundException(
          fvnfe, formula, mc, stack, doNotDisplay, syntaxCheck);
    } catch (NestedCalculateFormulaException ncfe) {
      return caughtNestedCalculateFormulaException(
          ncfe, formula, mc, stack, doNotDisplay, syntaxCheck);
    }
  }
  ...
  private static String mathToLowerCase(String formula) {
    ...
    while (true) {
      ...
      while ((checkpos >= 0) && (newFormula.charAt(checkpos) != ' ')) {
        if ((newFormula.charAt(checkpos) >= 65) && (newFormula.charAt(checkpos) <= 90)) {
          char c = (char) (newFormula.charAt(checkpos) + 32);
          newFormula = newFormula.substring(0, checkpos) + c + newFormula.substring(checkpos + 1);
        }
        checkpos--;
      }
      pos++;
  ...
  private static double caughtRecognitionException(
  ...
  private static double caughtFormulaVariableNotFoundException(
      FormulaVariableNotFoundException e,
      String formula,
      MetricsCalculator mc,
      HashSet<String> stack,
      boolean doNotDisplay,
    ...
    try {
      ...
      if (formula.contains(fvnfe.getVariable())) {
        int pos = formula.indexOf(fvnfe.getVariable()) + fvnfe.getVariable().length();
        ...
        switch (formula.charAt(pos)) {
          ...
          case '(':
            ...
            throw new FormulaMathFunctionNotFoundException(fvnfe.getVariable());
          default:
      ...
      // Try to solve it
      return caughtRecognitionException(e2, formula, mc, stack, doNotDisplay, syntaxCheck);
    }
  }
  ...
  private static double caughtNestedCalculateFormulaException(
      NestedCalculateFormulaException ncfe,
      String formula,
      MetricsCalculator mc,
      HashSet<String> stack,
      boolean doNotDisplay,
    ...
    enhancedFormula =
        AlgorithmIDTranslateEnhancements(
            ncfe, enhancedFormula, mc, stack, doNotDisplay, syntaxCheck);

    enhancedFormula = mathEnhancements(ncfe, enhancedFormula, mc, stack, doNotDisplay, syntaxCheck);

    enhancedFormula = EOFEnhancements(ncfe, enhancedFormula, mc, stack, doNotDisplay, syntaxCheck);
  ...
  private static String mathEnhancements(
      ...
      throws CalculateFormulaException {
    String originalFormula = formula;
    ArrayList<FormulaVariableNotFoundException> oldList =
        ncfe.getFormulaVariableNotFoundExceptions();
    ...
    try {
      ...
      ArrayList<FormulaVariableNotFoundException> newList =
          ncfe2.getFormulaVariableNotFoundExceptions();
  ...
  private static String EOFEnhancements(
      NestedCalculateFormulaException ncfeOriginal,
      String formula,
      MetricsCalculator mc,
      HashSet<String> stack,
      boolean doNotDisplay,
    ...
    try {
      ...
      ArrayList<MismatchedTokenException> oldList = ncfe.getMismatchedTokenExceptions();
      ...
      do {
        ...
        for (MismatchedTokenException mmte : oldList) {
          if (mmte.c == Token.EOF) {
            try {
              if (AntlrException.getTokenList()[mmte.expecting].equals("')'")) {
                formula = formula + ")";
                missingChar.add(")");
              }
        ...
        metricsGrammarParser parser2 =
            new metricsGrammarParser(tokenStream2, mc, stack, doNotDisplay, syntaxCheck);
        try {
          parser2.evaluator();
          oldList.clear();
        } catch (NestedCalculateFormulaException ncfe2) {
  ...
  private static String AlgorithmIDTranslateEnhancements(
      NestedCalculateFormulaException ncfeOriginal,
      String formula,
      MetricsCalculator mc,
      HashSet<String> stack,
