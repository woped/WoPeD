...
class LinearPathDescriptor {
   ...
   *
   * <p>Calculates the number of highlighted arcs based on prior requests
   *
  ...
  Set<String> getHighlightedArcIds() {
    ...
    for (ArcModel arc : arcs)
      if (highlightedNodes.contains(arc.getSourceId())
          && highlightedNodes.contains(arc.getTargetId())) highlightedArcs.add(arc.getId());
   ...
   *
   * <p>The cyclic nodes are going to be highlighted.
   *
   ...
   *
   * <p>The method uses itself recursively with the depth-first search algorithm.
   *
   ...
   * @throws NaNException This exception occurs if the net is no workflow net and thus has no
   *     longest path by definition
   */
  public double longestPath() throws NaNException {
    highlightedNodes = new HashSet<String>();
    HashSet<String> nodes = new HashSet<String>();
    for (String key : pathMap.keySet()) nodes.add(key);

    for (String key : pathMap.keySet())
      for (String childKey : pathMap.get(key)) nodes.remove(childKey);

    if (nodes.size() != 1) throw new NaNException();

    String startNode = null;
    for (String key : nodes) startNode = key;
  ...
  private double longestPath(
      double length, String myKey, HashSet<String> previousKeys, Set<String> maxset) {
    if (pathMap.get(myKey).isEmpty()) {
      if (length > maxset.size()) {
        maxset.clear();
        for (String key : previousKeys) maxset.add(key);
        maxset.add(myKey);
      }
      return length;
    }
    ...
    for (String key : previousKeys) newPrevKeys.add(key);
   ...
   * @throws NaNException This exception occurs if the net is no workflow net and thus has no cut
  ...
  public double cutVertices() throws NaNException {
    ...
    for (String key : keySet) {
      HashMap<String, ArrayList<String>> newMap =
          (HashMap<String, ArrayList<String>>) pathMap.clone();
  ...
  private boolean isFinite() {
    ...
    for (String key : pathMap.keySet()) ids.add(key);
    for (String key : pathMap.keySet()) {
      if (pathMap.get(key).isEmpty()) hasEnd = true;
      for (String subKey : pathMap.get(key)) ids.remove(subKey);
    }
  ...
  private boolean isConnected(
    ...
    if (newMap.get(myKey).isEmpty() && (!myKey.equals(killedKey) || pathMap.get(myKey).isEmpty()))
      return true;
