...
/**
 * Please note: To ensure a strict MVC model, all requests from the UI and all information to the UI
 * uses display strings rather than the real metrics or group IDs. Therefore conversion is used
 * within this class' method wherever needed. However, real IDs may always be used - the conversion
...
public class MetricsUIRequestHandler {
  ...
  public List<UIMetricsGroup> calculateMetrics(String topGroupName) {
    ...
    List<String> groupnames =
        metricsConfig.getGroupIDsFromGroup(algorithmGroupNameToID(topGroupName));
    ...
    for (String algorithmGroupName : groupnames) {
      ...
      if (resultList.size() > 0)
        answerList.add(
            new UIMetricsGroup(
                metricsConfig.getAlgorithmGroupName(algorithmGroupName), resultList));
    }
    return answerList;
  }
  ...
  public StringPair calculateSingle(IMetricsConfiguration metricsConfig, String iid) {
    ...
    try {
      ...
      return new StringPair(
          metricsConfig.getAlgorithmName(id) != null
              ? metricsConfig.getAlgorithmName(id)
              : metricsConfig.getVariableName(id),
          round(
              value,
              metricsConfig.getVariableIDs().contains(id)
      ...
      return new StringPair(
          metricsConfig.getAlgorithmName(id) != null
              ? metricsConfig.getAlgorithmName(id)
              : metricsConfig.getVariableName(id),
          CalculateFormulaException.getShortError());
  ...
  public static String algorithmNameToID(String name) {
    ...
    for (String s : groups)
      if (ConfigurationManager.getMetricsConfiguration().getAlgorithmName(s).equals(name)) return s;
    groups = ConfigurationManager.getMetricsConfiguration().getVariableIDs();
    for (String s : groups)
      if (ConfigurationManager.getMetricsConfiguration().getVariableName(s).equals(name)) return s;
    return name;
  }
  ...
  public List<String> getMetricGroups() {
    ...
    for (String s : groups)
      groupNames.add(ConfigurationManager.getMetricsConfiguration().getAlgorithmGroupName(s));
    return groupNames;
  }
  ...
  public String getError(String ID) {
    String id = algorithmNameToID(ID);
    if (errorMessages.containsKey(id)) return errorMessages.get(id);
    return null;
  }
  ...
  public void setHighlight(String metricsID) {
    ...
    for (String nodeID : nodeIDs) mec.getElementById(nodeID).setHighlighted(true);

    Set<String> arcIDs = high.getArcIDs();
    for (String arcID : arcIDs) mec.getArcById(arcID).setHighlighted(true);
  }
  ...
  public IMetricsConfiguration.MetricThresholdState getTreshholdState(String metricsID) {
    return valueStates.containsKey(algorithmNameToID(metricsID))
        ? valueStates.get(algorithmNameToID(metricsID))
        : IMetricsConfiguration.MetricThresholdState.NONE;
  }
  ...
  public static List<UIThreshold> getUIThresholds(String metricsID) {
    ...
    for (int i = 0; i < thresholds; i++)
      thresholdList.add(
          new UIThreshold(
              metricsConfig.getMetricThresholdLowValue(id, i),
              metricsConfig.getMetricThresholdHighValue(id, i),
              metricsConfig.getMetricThresholdState(id, i)));
  ...
  public boolean exportMetricsResults(List<UIMetricsGroup> groups) {
    ...
    File defaultExportDir =
        new File(ConfigurationManager.getConfiguration().getUserdir() + "exports");
    if (!defaultExportDir.exists()) defaultExportDir.mkdirs();
    ...
    FileFilter filter =
        new LabeledFileFilter() {
          public boolean accept(File file) {
            if (file.isDirectory() || file.getAbsolutePath().endsWith(".txt")) return true;
            return false;
          }
    ...
    filter =
        new LabeledFileFilter() {
          public boolean accept(File file) {
            if (file.isDirectory() || file.getAbsolutePath().endsWith(".csv")) return true;
            return false;
          }
    ...
    try {
      ...
      for (UIMetricsGroup group : groups) {
        ...
        for (StringPair sp : group.getValues()) {
          ...
          if (fileChooser.getSelectedFile().getName().endsWith("csv")) bw.write(";");
          bw.write("\r\n");
        }
      }
      bw.close();
