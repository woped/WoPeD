...
public class MetricsHighlighter {
  ...
  public void addHighlight(
      String metricsKey, Set<String> nodeids, Set<String> arcids, boolean ignoreCall) {
    if (ignoreCall) return;
    MetricHighlighting highlight = highlightMap.get(metricsKey);
    if (highlight == null) highlight = new MetricHighlighting();
    if (!inverted) {
  ...
  public void mergeHighlights(Set<String> stack, String copySource, boolean ignorecall) {
    ...
    for (String key : stack) addHighlight(key, source.getNodeIDs(), source.getArcIDs(), ignorecall);
  }

  public void addHighlight(
      Set<String> stack, Set<String> nodeids, Set<String> arcids, boolean ignoreCall) {
    for (String key : stack) addHighlight(key, nodeids, arcids, ignoreCall);
  }
