...
/**
 * @author Tobias Lorentz This Class should store all the Formula enhancement which had been found
 *     by the parser This Class implements the Singleton-Pattern
 */
public class FormulaEnhancementList {
  /** The Enhancements are Stored as a HashMap Key is the formulaID */
  ...
  /**
   * This list stores all UIs which are binded to this data-container It stores instances of
   * event-listeners
   */
   ...
   * @param enhancedFormula This is the new Formula which the parser had produced If there is
   *     already a Entry for this FormulaID, it will be overwritten
   */
  ...
  /**
   * @return a Array for displaying the EnhancementList as a Table First-Column: FormulaID
   *     Second-Column: Old Formula Third-Column: Enhanced Formula
   */
  ...
  public void acceptEnhancement(String formulaID) {
    ...
    ConfigurationManager.getMetricsConfiguration()
        .setAlgorithmFormula(formulaID, this.enhancementList.get(formulaID));
    // Flush the Queue
  ...
  /**
   * This Methods adds a new Listener-Implemention, which will be called if the enhancementList
   * changes
   *
   * @param listener A Class that implements the
   *     FormulaEnhancementListChangedEventListener-Interface Should be a FormulaEnhancementUI
   */
  ...
  /**
   * This Methods removes a new Listener-Implemention, which will be called if the enhancementList
   * changes
   *
   * @param listener A Class that implements the
   *     FormulaEnhancementListChangedEventListener-Interface Should be a FormulaEnhancementUI
  ...
  protected synchronized void notifyFormulaEnhancementListChangedEvent(
    ...
    if (listeners != null) {
      for (FormulaEnhancementListChangedEventListener l :
          listeners.getListeners(FormulaEnhancementListChangedEventListener.class)) {
