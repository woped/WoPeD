 ...
 */
public class MetricsBuilderListener
    implements ActionListener, MouseListener, ItemListener, DocumentListener, ChangeListener {

  /** Indicates whether there are unsaved changes */
  private static boolean dirty = false;

  /**
   * Indicates that we are right now processing a double click on a list element, Needed to prevent
   * those action from changing the "dirty" variable.
   */
  ...
  /**
   * Index of the tab that we were on. This variable will get changed in the ChangeListener, but
   * only after we stored the changed data on the "old" tab first.
   */
    ...
    if (btnName != null) {
      if (btnName.equals(MetricsBuilderPanel.NEW_BTN)) {
        ...
        if (metricsConfig.isMetricIDInUse(newMetricID))
          ...
          FileFilter filter =
              new LabeledFileFilter() {
                public boolean accept(File file) {
                  if (file.getAbsolutePath().endsWith(".xml")) return true;
                  return false;
                }
          ...
          fileChooser.setCurrentDirectory(
              new File(metricsConfig.getCustomMetricsDir() + File.separatorChar));
          File file = null;
          ...
          if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (!path.substring(path.length() - 4, path.length()).equals(".xml")) path += ".xml";

            file = new File(path);

            // User selected a new file for the new algorithm, so
            // add it
            if (metricsConfig.findFileIDToFileName(file.getAbsolutePath()) == -1)
              metricsConfig.addNewMetricFile(file.getAbsolutePath());

            metricsConfig.addNewAlgorithm(
                newMetricID, metricsConfig.findFileIDToFileName(file.getAbsolutePath()));
            update();
      ...
      } else if (btnName.equals(MetricsBuilderPanel.IMPORT_BTN)) importMetricsFile();
      else if (btnName.equals(MetricsBuilderPanel.EXPORT_BTN)) exportMetricsFile();
        ...
        try {
          MetricsCalculator.checkFormula(MetricsBuilderPanel.jMetricsFormulaTextField.getText());
          JOptionPane.showMessageDialog(
              null,
              Messages.getString("Metrics.Builder.ConsistencyCheck.NoIssuesFound"), // $NON-NLS-1$
              Messages.getString(
                  "Metrics.Builder.ConsistencyCheck.ConsistencyChecker"), //$NON-NLS-1$
            ...
            if (ncfe.hasEnhancementExceptions()) {
              ...
              int answer =
                  JOptionPane.showConfirmDialog(
                      null,
                      Messages.getString(
                              "Metrics.Builder.ConsistencyCheck.ErrorsCanBeReducedByUsingTheFormula")
                          + newLine
                          + //$NON-NLS-1$
                          "\""
                          + ncfe.getEnhancementExceptions().get(0).getEnhancedFormula()
                          + "\""
                          + newLine
                          + Messages.getString(
                              "Metrics.Builder.ConsistencyCheck.DoYouWantToUseThisFormula"), //$NON-NLS-1$
                      Messages.getString(
                          "Metrics.Builder.ConsistencyCheck.EnhancementAvailable"), //$NON-NLS-1$
              ...
              if (answer == JOptionPane.YES_OPTION) {
                ...
                try {
                  // Run the checker
                  MetricsCalculator.checkFormula(
                      ncfe.getEnhancementExceptions().get(0).getEnhancedFormula());
                  ...
                  JOptionPane.showMessageDialog(
                      null,
                      Messages.getString(
                          "Metrics.Builder.ConsistencyCheck.NoIssuesFound"), //$NON-NLS-1$
                      Messages.getString(
                          "Metrics.Builder.ConsistencyCheck.ConsistencyChecker"), //$NON-NLS-1$
                      JOptionPane.INFORMATION_MESSAGE);
        ...
        MetricsBuilderPanel.jTableValues.editingCanceled(
            new ChangeEvent(MetricsBuilderPanel.jTableValues));
        ...
        textAddition = ((JButton) evt.getSource()).getText().toLowerCase() + "(";
    } else textAddition = ((JButton) evt.getSource()).getText();
    if (textAddition != "") {
      int index = MetricsBuilderPanel.jMetricsFormulaTextField.getCaretPosition();

      String text = MetricsBuilderPanel.jMetricsFormulaTextField.getText();
      String firstPart = text.substring(0, index);
      String secondPart = "";

      if (index < text.length()) secondPart = text.substring(index, text.length());

      if (secondPart.substring(0, 1).equals(" "))
        secondPart = secondPart.substring(1, secondPart.length());

      MetricsBuilderPanel.jMetricsFormulaTextField.setText(
          firstPart + " " + textAddition + " " + secondPart);
      MetricsBuilderPanel.jMetricsFormulaTextField.grabFocus();
      MetricsBuilderPanel.jMetricsFormulaTextField.setCaretPosition(
          index + textAddition.length() + 1);

      // If first character is a blank, remove it
      if (MetricsBuilderPanel.jMetricsFormulaTextField.getText().length() > 0
          && MetricsBuilderPanel.jMetricsFormulaTextField.getText().substring(0, 1).equals(" "))
        MetricsBuilderPanel.jMetricsFormulaTextField.setText(
            MetricsBuilderPanel.jMetricsFormulaTextField.getText().substring(1));
    }
  }
  ...
  /** Import a metrics XML file (= add it to the user metrics folder and load it) */
  private void importMetricsFile() {
    ...
    if (returnVal == JFileChooser.APPROVE_OPTION) {
      importFromFile = new File(fileChooser.getSelectedFile().getAbsolutePath());
      IMetricsConfiguration metricsConfig = ConfigurationManager.getMetricsConfiguration();
      File importToFile =
          new File(
              metricsConfig.getCustomMetricsDir()
                  + File.separatorChar
      ...
      if (importToFile.exists())
        ...
        update();
        JOptionPane.showMessageDialog(null, "Import finished succesfully.");
      }
    }
  }

  /** Exports the selected entries from the variables and algorithm lists to a metrics XML file */
  private void exportMetricsFile() {
    ...
    while (!valid_selection && !cancel) {
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        if (!exportFile
            .getName()
            .endsWith(((LabeledFileFilter) fileChooser.getFileFilter()).getExtension()))
          exportFile =
              new File(
                  exportFile.getAbsolutePath()
                      + ((LabeledFileFilter) fileChooser.getFileFilter()).getExtension());
        ...
        if (exportFile.exists()) {
          ...
          if (option == JOptionPane.YES_OPTION) valid_selection = true;
    ...
    for (int i : MetricsBuilderPanel.jAlgorithmListTab1.getSelectedIndices())
      list.add((String) MetricsBuilderPanel.jAlgorithmListTab1.getModel().getElementAt(i));

    for (int i : MetricsBuilderPanel.jVariableListTab3.getSelectedIndices())
      list.add((String) MetricsBuilderPanel.jVariableListTab3.getModel().getElementAt(i));

    IMetricsConfiguration metricsConfig = ConfigurationManager.getMetricsConfiguration();
    if (metricsConfig.save(list, exportFile))
      JOptionPane.showMessageDialog(null, "Export finished succesfully.");
    else JOptionPane.showMessageDialog(null, "Error while exporting.");
  }

  public void itemStateChanged(ItemEvent e) {

    if (e.getItem().equals("red")) MetricsBuilderPanel.jComboBoxColor.setSelectedIndex(0);
    else if (e.getItem().equals("yellow")) MetricsBuilderPanel.jComboBoxColor.setSelectedIndex(1);
    else MetricsBuilderPanel.jComboBoxColor.setSelectedIndex(2);
  }
        ...
        if (oldTabIndex == 0) {
          if (listName != null) {
            if (listName.equals(MetricsBuilderPanel.ALGORITHM_LIST)) {
              NameIDListEntry listEntry =
                  (NameIDListEntry)
                      MetricsBuilderPanel.jAlgorithmListTab1.getModel().getElementAt(index);
              metricsID = listEntry.getMetricID();
              MetricsBuilderPanel.clear();
              MetricsBuilderPanel.fillTable(metricsID);

            } else if (listName.equals(MetricsBuilderPanel.VARIABLE_LIST)) {
              NameIDListEntry listEntry =
                  (NameIDListEntry)
                      MetricsBuilderPanel.jVariableListTab1.getModel().getElementAt(index);
              metricsID = listEntry.getMetricID();
              MetricsBuilderPanel.clear();
              MetricsBuilderPanel.fillTable(metricsID);
            } else if (listName.equals(MetricsBuilderPanel.ALGORITHM_LIST_TAB2)) {
              NameIDListEntry listEntry =
                  (NameIDListEntry)
                      MetricsBuilderPanel.jAlgorithmListTab2.getModel().getElementAt(index);
              metricsID = listEntry.getMetricID();
              MetricsBuilderPanel.clear();
              MetricsBuilderPanel.fillTable(metricsID);
            } else if (listName.equals(MetricsBuilderPanel.VARIABLE_LIST_TAB2)) {
              NameIDListEntry listEntry =
                  (NameIDListEntry)
                      MetricsBuilderPanel.jVariableListTab2.getModel().getElementAt(index);
              MetricsBuilderPanel.clear();
              metricsID = listEntry.getMetricID();
              MetricsBuilderPanel.fillTable(metricsID);
            }
  ...
  public void changedUpdate(DocumentEvent evt) {
    if (!whileMetricFilling) dirty = true;
  }
  ...
  /**
   * Called on tab change. Will trigger the actual changing of internal metric data as represented
   * in the GUI (change != save).
   */
  ...
  /**
   * Will modify the internal metric representation with data from the GUI. Note: This method will
   * not save the data yet!
   */
  private boolean updateMetricWithGUIData() {
    if (currentMetricID.isEmpty()) return false;
    ...
    switch (oldTabIndex) {
      case 0:
        ...
        if (!MetricsBuilderPanel.jMetricsNameTextField.isEnabled()) return true;

        if (MetricsBuilderPanel.jMetricsFormulaTextField.getText().isEmpty()) {
          JOptionPane.showMessageDialog(
              null,
        ...
        metricsConfig.setAlgorithmName(
            currentMetricID, MetricsBuilderPanel.jMetricsNameTextField.getText());
        metricsConfig.setAlgorithmFormula(
            currentMetricID, MetricsBuilderPanel.jMetricsFormulaTextField.getText());
        metricsConfig.setAlgorithmDescription(
            currentMetricID, MetricsBuilderPanel.jMetricsDescriptionTextField.getText());
        break;
