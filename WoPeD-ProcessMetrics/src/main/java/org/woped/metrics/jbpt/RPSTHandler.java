...
public class RPSTHandler {
  ...
  /**
   * An RPST handler is used for conversion and calculation of RPST models based on the library of
   * Artem Polyvyanyy
   *
   * @param mec
   */
  ...
  private double getNodesInRigid(RPSTNode<DirectedEdge, Vertex> tn, boolean instack) {
    ...
    for (RPSTNode<DirectedEdge, Vertex> c : subs) elements += getNodesInRigid(c, instack);
    return elements;
  }
}
