...
public class MetricGroup extends JPanel {
  ...
  public MetricGroup(
      MetricsUIRequestHandler uiHandler,
      IEditor editor,
      UIMetricsGroup metricGroup,
      SideBar parent) {
    ...
    for (StringPair item : values) {
      ...
      MetricItem metricItem =
          new MetricItem(item.getKey(), item.getValue(), uiHandler, editor, parent);
    ...
    caption.setFont(
        new Font(caption.getFont().getFamily(), Font.BOLD, caption.getFont().getSize()));
