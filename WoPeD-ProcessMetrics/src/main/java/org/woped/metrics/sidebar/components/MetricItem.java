...
/**
 * @author Mathias Gruschinske a metric item is a single entry in the metric group and shows some
 *     information about the metric and displays the result of them
 */
public class MetricItem extends JPanel {
   ...
   *     <p>sets the instance variable for the metric item
   */
  public MetricItem(
      String key, String value, MetricsUIRequestHandler uiHandler, IEditor editor, SideBar parent) {
  ...
  /**
   * Method to create the summary panel, which is displayed in a group. A click on the panel sets
   * the detail panel to visible.
   */
  private void createSummaryPanel() {
    ...
    // TreshholdState
    JLabel treshholdState = getTrafficLight(uiHandler.getTreshholdState(key));
  ...
  /**
   * Method to create the detail panel, which is displayed under the summary panel. Contains a
   * description, errors and the threshold states. Additional it contains two actions, which use the
   * metrics builder.
   */
  private void createDetailPanel() {
    ...
    descriptionLabel.setFont(
        new Font(
            descriptionLabel.getFont().getFamily(),
            Font.BOLD,
    ...
    if (uiHandler.getError(key) != null) {
      ...
      errorLabel.setFont(
          new Font(
              errorLabel.getFont().getFamily(), Font.BOLD, descriptionLabel.getFont().getSize()));
    ...
    List<UIThreshold> thresholds = MetricsUIRequestHandler.getUIThresholds(key.toString());
    ...
    for (UIThreshold uiThreshold : thresholds) {
      ...
      thresholdsPanel.add(getTrafficLight(uiThreshold.getThreshold()), constraintsDetails);
    ...
    if (MetricsUIRequestHandler.canBeEdited(key)) actions = new JPanel(new GridLayout(0, 2));
    else actions = new JPanel(new GridLayout(0, 1));
   ...
   */
  private JLabel getTrafficLight(IMetricsConfiguration.MetricThresholdState color) {
    ...
    switch (color) {
      ...
      case GRAY:
        trafficLight = new JLabel(Messages.getImageIcon("Metrics.SideBar.Incorrect"));
        break;
  ...
  /**
   * @author Mathias Gruschinske internal class, which contains the mouse listener for the expander
   *     button
   */
  ...
  public void setDetailsVisiblity(boolean isVisible) {
    ...
    if (detailPanel.isVisible()) {
      ...
      expandIcon.setIcon(Messages.getImageIcon("PetriNet.Resources.Expand"));
  ...
  public void setKeyLabelWidth(double width) {
    ...
    keyLabel.setBorder(
        BorderFactory.createEmptyBorder(0, 0, 0, (int) (width - keyLabel.getSize().getWidth())));
  }

  /**
   * @author Mathias Gruschinske A config label with mouse listener, which open the metrics builder
   *     and select the metric to edit (only custom metrics)
   */
  ...
  /**
   * @author Mathias Gruschinske A delete label with mouse listener, which delete the metric (only
   *     custom metrics)
   */
  ...
  /**
   * @author Mathias Gruschinske A info label with mouse listener, which open the metrics builder
