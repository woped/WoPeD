...
public class ApromoreExportFrame extends JDialog {
	...
	private WopedButton getUpdateButton() {
		...
		if (updateButton == null) {
			...
			exportButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					updateAction(table.getSelectedRow());
				}
			});
		}
		...
		try {
			if (getIDFilterText().getText().equals(""))
				tabModel.setDataVector(aproAccess.filterProcessList(
						getNameFilterText().getText(), 0, getOwnerFilterText()
								.getText(), getTypeFilterText().getText()),
						columnNames);
			else
				tabModel.setDataVector(aproAccess.filterProcessList(
						getNameFilterText().getText(), Integer
								.parseInt(getIDFilterText().getText()),
	...
	private void exportAction() {
		try {
			...
			PNMLExport pExport = new PNMLExport(iSB);
			pExport.saveToStream((EditorVC) mediator.getUi().getEditorFocus(),
					os);
			InputStream is = new ByteArrayInputStream(os.toByteArray());
			aproAccess.exportProcess(getProcessOwnerText().getText(),
					getProcessNameText().getText(), is, "", true);
	...
	private void updateAction(int ind) {
		try {
			...
			pExport.saveToStream((EditorVC) mediator.getUi().getEditorFocus(),
					os);
			InputStream is = new ByteArrayInputStream(os.toByteArray());
			aproAccess.updateProcess(ind, is);
