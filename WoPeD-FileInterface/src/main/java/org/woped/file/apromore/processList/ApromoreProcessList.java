...
public class ApromoreProcessList {
  ...
  public ApromoreProcessList(
      ApromoreAccess aproAccess, JDialog frame, AbstractApplicationMediator mediator) {
  ...
  public boolean importAction(boolean beautify, boolean edgesToPlaces, boolean tasksToTransitions) {
    ...
    try {
      if (ind != -1) {
        String processName = (String) table.getModel().getValueAt(ind + 1, 0);
        ...
        if (pLoader.run(is, processName + ".pnml")) {
          LoggerManager.info(
              Constants.APROMORE_LOGGER, "Model description successfully loaded from Apromore");
  ...
  public void exportAction(String owner, String folder, String name, String version) {
    try {
  ...
  public void updateAction(
      Integer id, String username, String nativeType, String processName, String newVersionNumber)
      throws Exception {
          ...
          public boolean include(Entry entry) {
            ...
            for (int i = 0; i < filterFolder.length; i++) {
              ...
              if (entry.getValue(0) != null && entry.getValue(6) != null) {
                ...
                if (entry.getValue(6).toString().equals(filterFolder[i])
                    || filterFolder[i].equalsIgnoreCase(ApromoreFolderTree.TOP_NODE_NAME)) {
                  found = true;
                  break;
                } else {
                  found = false;
