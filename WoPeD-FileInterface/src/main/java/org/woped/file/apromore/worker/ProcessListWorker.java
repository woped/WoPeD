...
public class ProcessListWorker extends SwingWorker<ReturnElement, Void> {
  ...
  protected ReturnElement doInBackground() throws Exception {
    ...
    ApromoreServer[] servers = ConfigurationManager.getConfiguration().getApromoreServers();
    parent.getAproAccess().setCurrentServer(parent.getServerDropdown().getSelectedIndex());

    ConfigurationManager.getConfiguration()
        .setApromoreUsername(
            servers[parent.getServerDropdown().getSelectedIndex()].getApromoreUserName());

    final boolean serverAviable = parent.getAproAccess().checkIfServerAvailable();

    final List<FolderType> folders = parent.getAproAccess().getFoldersForCurrentUser();
            ...
            if (ele != null) {
              ...
              for (FolderType folder : ele.getFolders()) {
                ...
                if (parent.getProcessFolderBox() != null) {
                  parent.getProcessFolderBox().addItem(folder.getFolderName());
                }
