...
public class ApromoreExportFrame extends AbstractApromoreFrame {
            ...
            new MouseAdapter() {
              ...
              public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() == 2) {
                  String version = checkCurrentVersion(getProcessVersionText().getText());
                  if (version != null) {
  ...
  /** Aktuallisiert die Prozess-Datenfelder mit dem ausgew�hlten Tabellenelement */
  ...
  private void initDataFields() {
    getProcessNameText().setText(mediator.getUi().getEditorFocus().getName().replace(".pnml", ""));
    getProcessIDText().setText("");
    getProcessOwnerText().setText(ConfigurationManager.getConfiguration().getApromoreUsername());
      ...
      exportButton.addActionListener(
          new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
              String version = checkCurrentVersion(processVersionText.getText());
              if (version != null) {
  ...
  private JPanel getDataPanel() {
    ...
    if (processDataPanel == null) {
      ...
      processDataPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(Messages.getString("Apromore.UI.ProcessData")),
      ...
      // -------------------------label
      c = getConstraits(c, 0, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessIDLabel(), c);

      c = getConstraits(c, 1, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessNameLabel(), c);
      c = getConstraits(c, 2, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessOwnerLabel(), c);

      c = getConstraits(c, 3, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessTypeLabel(), c);

      c = getConstraits(c, 4, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessFolderLabel(), c);

      c = getConstraits(c, 5, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessVersionLabel(), c);

      c = getConstraits(c, 6, 0, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessPrivacyLabel(), c);

      // -------------------------Text
      c = getConstraits(c, 0, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessIDText(), c);

      c = getConstraits(c, 1, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessNameText(), c);

      c = getConstraits(c, 2, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessOwnerText(), c);

      c = getConstraits(c, 3, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessTypeText(), c);

      c = getConstraits(c, 4, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessFolderBox(), c);

      c = getConstraits(c, 5, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
      processDataPanel.add(getProcessVersionText(), c);

      c = getConstraits(c, 6, 1, 1, 1, 1, 1, inset, GridBagConstraints.HORIZONTAL);
  ...
  private GridBagConstraints getConstraits(
  ...
  private JLabel getProcessFolderLabel() {
    if (processFolderLabel == null) {
      processFolderLabel = new JLabel(Messages.getString("Apromore.UI.FolderName"));
    }
  ...
  private JLabel getProcessPrivacyLabel() {
    if (processPrivacyLabel == null) {
      processPrivacyLabel = new JLabel(Messages.getString("Apromore.UI.Privacy"));
    }
    ...
    Thread queryThread =
        new Thread() {
          ...
          public void run() {
            ...
            setButtons(false);
            exportWorker = new ExportWorker(ApromoreExportFrame.this, version, update);
