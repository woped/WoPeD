...
public abstract class AbstractApromoreFrame extends JDialog {
  ...
  protected JPanel getCreateFolderPanel() {
    if (createFolderPanel == null) {
      ...
      createFolderButton.addActionListener(
          new ActionListener() {
  ...
  protected JPanel getProcessListPanel(boolean showPnmlOnly) {
    if (processListPanel == null) {
      ...
      processListPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(Messages.getString("Apromore.UI.ProcessList")),
      ...
      c.weighty = 2;
      c.gridheight = 2;
      processListPanel.add(processList.getScrollableProcessTable(showPnmlOnly), c);
    }
  ...
  protected JPanel getServerSelectionPane() {
    if (serverSelectionPane == null) {
      ...
      serverSelectionPane.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("Apromore.UI.ServerSelectionTitel")),
              BorderFactory.createEmptyBorder(2, 0, 0, 2)));

      serverSelectionPane.add(new JLabel(Messages.getString("Apromore.UI.CurrentServer")));

      serverDropdown = new JComboBox<String>();
      serverDropdown.setBorder(BorderFactory.createEmptyBorder(2, 0, 0, 2));
          ...
          public void run() {
            ...
            SwingUtilities.invokeLater(
                new Runnable() {
                  @Override
                  public void run() {
                    AbstractApromoreFrame.this.getWopedPorgressBar().setIndeterminate(true);
            ...
            progressBarWorker = new ProcessListWorker(AbstractApromoreFrame.this);
