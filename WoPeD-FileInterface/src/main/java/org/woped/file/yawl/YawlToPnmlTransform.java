...
public class YawlToPnmlTransform {
  ...
  private void transformXorSplits() {
    ...
    for (WfNetNode node : wfNet.getNodes()) {
      ...
      String operatorCode =
          Transition.getWoPedOperatorCode(t_old.getJoinType(), t_old.getSplitType());
  ...
  private void transformAndSplits() {
    ...
    for (Transition node : nodes_to_modify) {
      ...
      node.setWopedOperatorCode(
          Transition.getWoPedOperatorCode(JoinSplitType.None, JoinSplitType.And));
  ...
  private void transformAndJoins() {
    ...
    for (Transition node : nodes_to_modify) {
      ...
      node.setWopedOperatorId(node.getId());
      node.setWopedOperatorCode(
          Transition.getWoPedOperatorCode(JoinSplitType.And, JoinSplitType.None));
  ...
  private void transformAndJoinAndSplits() {
    ...
    for (Transition node : nodes_to_modify) {
      ...
      node.setWopedOperatorId(node.getId());
      node.setWopedOperatorCode(
          Transition.getWoPedOperatorCode(JoinSplitType.And, JoinSplitType.And));
      node.setWopedOrientation("3");
    ...
    for (WfNetNode node : wfNet.getNodes()) {
      ...
      for (int pcount = 1; pcount <= output_count; pcount++) {
        ...
        do {
          ...
          for (int ip = 0; ip < pointers.length; ip++) {
            t_new.addOutputNode(t_old.getOutputNode(pointers[ip]));
          }
    ...
    for (WfNetNode node : wfNet.getNodes()) {
      ...
      for (int pcount = 1; pcount <= input_count; pcount++) {
        ...
        do {
          ...
          for (int ip = 0; ip < pointers.length; ip++) {
            t_new.addInputNode(t_old.getInputNode(pointers[ip]));
          }
  ...
  /**
   * Updates an array of indexes (or pointers) into another (target) array or list that is equal or
   * larger in size (see 'target_size' parameter). Initially, the pointer array must be initialized
   * with pointers[i] = i. Subsequent calls update the pointer array so that all combinations of
   * pointers into the target list/array are being set. When no additional combinations are
   * available, the function returns 'false'.
   *
   * @param pointers
   * @param target_size
   * @return
