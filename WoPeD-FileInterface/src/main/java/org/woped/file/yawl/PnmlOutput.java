...
public class PnmlOutput {
  ...
  public String getPnmlOutput() {
    ...
    netElement.addAttribute(
        new XmlElement.XmlAttribute("type", "http://www.informatik.hu-berlin.de/top/pntd/ptNetb"));
    ...
    for (WfNetNode node : this.wfNet.getNodes()) {
      ...
      ts.addAndGetChild("orientation").setContents(transition.getWopedOrientation());
    }
    ...
    for (WfNetNode node : this.wfNet.getNodes()) {
      ...
      for (WfNetNode output_node : node.getOutputNodes()) {
        ...
        xml_arc.addAndGetChild("inscription").addAndGetChild("text").setContents("1");
    ...
    bounds.addAndGetChild("position", false).addAttribute("x", "2").addAttribute("y", "2");
    bounds.addAndGetChild("dimension", false).addAttribute("x", "1024").addAttribute("y", "768");
  ...
  private void setIdAndNameAndLayoutData(XmlElement xml_node, WfNetNode node) {
    ...
    nameElem
        .addAndGetChild("graphics") // <name><graphics>
