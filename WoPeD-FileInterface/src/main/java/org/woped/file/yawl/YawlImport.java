...
public class YawlImport {
      ...
      for (LayoutNetFactsType layout : spec.getNet()) {
        ...
        for (JAXBElement<?> boundsOrFrameOrViewport : layout.getBoundsOrFrameOrViewport()) {
          ...
          if (boundsOrFrameOrViewport.getValue() instanceof LayoutFrameType) {
            ...
            node.getLabelPosition().offsetBy(0, node.getShapePosition().height + 5);

          } else if (boundsOrFrameOrViewport.getValue() instanceof LayoutContainerFactsType) {

            // layout data for conditions and tasks
            LayoutContainerFactsType lcft =
                (LayoutContainerFactsType) boundsOrFrameOrViewport.getValue();
            ...
            for (Object vld : lcft.getVertexOrLabelOrDecorator()) {
              ...
              if (vld instanceof LayoutVertexFactsType) {
                ...
                for (JAXBElement<?> sbb :
                    lbl.getAttributes().getAutosizeOrBackgroundColorOrBendable()) {
              ...
              if (vld instanceof LayoutDecoratorFactsType && node instanceof Transition) {
  ...
  /**
   * Set join- and split-types using the YAWL-XML decorator type
   *
   * @param dectype
   */
  ...
  public void importYawlLabelLayout(
      WfNetNode node, LayoutRectangleType rect, NumberFormat numberFormat) {
  ...
  /**
   * import a YAWL &lt;vertex>...&lt;/vertex> element (position and dimensions) and set values to
   * the Node object
   *
   ...
   */
  private void importYawlVertexFactsType(
      NumberFormat numberFormat, WfNetNode node, LayoutVertexFactsType v) {

    // import a YAWL <vertex>...</vertex> element (position and
    // dimensions) and set values to the Node object

    for (JAXBElement<?> sizeOrBackgroundOrBendable :
        v.getAttributes().getAutosizeOrBackgroundColorOrBendable()) {
  ...
  private void importYawlConditionsAndTasks(SpecificationSetFactsType yawlRootElement) {
  ...
  private Place createPlaceFromYawlOutputCondition(OutputConditionFactsType cond) {
  ...
  public Place createPlaceFromYawlCondition(ExternalConditionFactsType cond) {
    ...
    for (FlowsIntoType fit : cond.getFlowsInto()) {
      ...
      this.yawlArcs.add(new YawlArc(yawlId, fit.getNextElementRef().getId()));
    }
  ...
  private void connectNodes() {
    for (YawlArc arc : yawlArcs) {
      ...
      if (nodeFrom == null || nodeTo == null) {
        Logger.getLogger("YawlImport")
            .log(Level.WARNING, "Node specified by YawlArc not found in WfNet");
        continue;
      }
