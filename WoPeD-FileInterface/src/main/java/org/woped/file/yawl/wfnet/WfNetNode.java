...
public abstract class WfNetNode {
  ...
  /**
   * Insert a new Node between this one and the specified existing output node (i.e. create the
   * following flow: this -> newOutputNode -> existingOutputNode)
   *
   * @param existingOutputNode
   * @param newOutputNode
   */
  public void insertBetween(WfNetNode existingOutputNode, WfNetNode newOutputNode) {
  ...
  public void copyAttributesTo(WfNetNode node, boolean copy_arcs) {
    node.name = this.name;
