...
public class PNMLExport {
  ...
  public boolean saveToStream(EditorVC editor, ByteArrayOutputStream os) {
    ...
    try {
      ...
      LoggerManager.error(
          Constants.FILE_LOGGER, "Could not write to the Stream. " + e.getMessage());
      return false;
    } finally {
      LoggerManager.debug(
          Constants.FILE_LOGGER,
  ...
  private void createJavaBeansInstances(EditorVC editor) {
    ...
    // (blackfox) - Could extend statusbars for simulations

    for (int i = 0; i < statusBars.length; i++)
      statusBars[i].startProgress(
          "Save to File", resources + roles + resourcesMapping + orgaUnits + rootElements + arcs);
  ...
  private void saveModelElementContainer(NetType iNet, ModelElementContainer elementContainer) {
    ...
    while (root2Iter.hasNext()) {
      ...
      /* ##### TRANSITION ##### */ {
        ...
        LoggerManager.debug(
            Constants.FILE_LOGGER,
            "   ... Setting InnerTransitions for Operator (ID:" + currentModel.getId() + ")");
  ...
  private PlaceType initPlace(PlaceType iPlace, PlaceModel currentModel) {
    ...
    LoggerManager.debug(
        Constants.FILE_LOGGER, "   ... Place (ID:" + currentModel.getId() + ") set");
  ...
  private TransitionType initTransition(
    ...
    LoggerManager.debug(
        Constants.FILE_LOGGER, "   ... Transition (ID:" + currentModel.getId() + ") set");
  ...
  private TransitionToolspecificType initToolspecific(
    ...
    if (Assign.class.isInstance(currentModel.getBpelData())) {
      ...
      if (org.oasisOpen.docs.wsbpel.x20.process.executable.TFrom.class.isInstance(
          (((Assign) currentModel.getBpelData()).getActivity()).getCopyArray(0).getFrom()))
        iCopy
            .addNewFrom()
            .setVariable(
                (((Assign) currentModel.getBpelData()).getActivity())
                    .getCopyArray(0)
                    .getFrom()
                    .getVariable());
      if (org.oasisOpen.docs.wsbpel.x20.process.executable.TTo.class.isInstance(
          (((Assign) currentModel.getBpelData()).getActivity()).getCopyArray(0).getTo()))
        iCopy
            .addNewTo()
            .setVariable(
                (((Assign) currentModel.getBpelData()).getActivity())
    ...
    if (currentModel.getToolSpecific().getOperatorId() != null) {
      ...
      LoggerManager.debug(
          Constants.FILE_LOGGER,
          "   ... Operator for Transition (ID:" + currentModel.getId() + ") set");
    }
    if (currentModel.getToolSpecific().getTrigger() != null) {
      ...
      LoggerManager.debug(
          Constants.FILE_LOGGER,
          "   ... Trigger for Transition (ID:" + currentModel.getId() + ") set");
    }
    if (currentModel.getToolSpecific().isSubprocess()) {
      iToolspecific.setSubprocess(true);
      LoggerManager.debug(
          Constants.FILE_LOGGER, "   ... Subprocess (ID:" + currentModel.getId() + ") set");
    }
    if (currentModel.getToolSpecific().getTransResource() != null) {
      ...
      LoggerManager.debug(
          Constants.FILE_LOGGER,
          "   ... Resource for Transition (ID:" + currentModel.getId() + ") set");
    }
  ...
  ArcType initArc(ArcType arcBean, ArcModel outerArc, ArcModel innerArc) {
    ...
    if (ConfigurationManager.getConfiguration().isExportToolspecific()) {
      ...
      LoggerManager.debug(
          Constants.FILE_LOGGER,
          "... Arc label position: X = " + probPos.getX() + " / Y = " + probPos.getY());
