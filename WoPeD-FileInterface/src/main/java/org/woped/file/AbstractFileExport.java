...
/**
 * @author lai
 *     <p>Folgendes auswählen, um die Schablone für den erstellten Typenkommentar zu Ändern: Fenster
...
public abstract class AbstractFileExport {
  ...
  public AbstractFileExport(
      Object Data, String absoluteFilePath, String fileName, int FileFilterType) {
    ...
    if (type == FileFilterImpl.JPGFilter) {
      ...
      if (fileName.length() >= 5) {
        if (!(fileName.substring(fileName.length() - 4, fileName.length()).equals(".xml"))
            && !(fileName.substring(fileName.length() - 5, fileName.length()).equals(".pnml"))) {
