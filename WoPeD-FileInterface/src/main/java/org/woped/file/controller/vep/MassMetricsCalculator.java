...
public class MassMetricsCalculator {
  ...
  /**
   * Used for mass calculation of WoPeD metrics. Contained in this project due to the need to open
   * and load files
   *
   * <p>Mass calculation logic (Increasing performance >90%) has been added directly to the IEditor
   * and loader classes
   */
  ...
  public void prepareMetrics(File[] files) {
    ...
    for (File f : files) {
      if (!f.isFile() || !f.getName().endsWith(".pnml")) continue;
      filesToCheck.add(f);
    }
  }

  /**
   * Runs the actual calculation, using a path to save the file and the mediator required to open
   * files
   *
   * @param saveFile
   * @param medi
  ...
  public void calculateMetrics(File saveFile, ApplicationMediator medi) {
    if (filesToCheck == null || filesToCheck.size() < 1) return;
  ...
  /**
   * This Thread is used for the actual metrics calculation. It should not intervene with working
   * with other nets or any UI-related functions
   */
  private class MetricsCalculation extends Thread {
    ...
    /** Performs the task of opening the nets, calculating the metrics and storing the results */
    @Override
    public void run() {
      index = 0;
      ...
      try {
        ...
        for (String s : metricsList) write.write("," + s);
        write.write("\r\n");

        // Loop over all selected files to analyze them
        for (File f : filesToCheck) {
          write.write(f.getName());
          if (pr.run(new FileInputStream(f), "", false)) edit = pr.getEditor()[0];
          /*
          ...
          for (String metric : metricsList) {
            StringPair result;
            if (metric != null) result = ui.calculateSingle(metricsConfig, metric);
            else result = new StringPair("", "");
