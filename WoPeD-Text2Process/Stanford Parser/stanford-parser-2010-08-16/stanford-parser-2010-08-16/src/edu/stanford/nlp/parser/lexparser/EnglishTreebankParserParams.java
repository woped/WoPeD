...
public class EnglishTreebankParserParams extends AbstractTreebankParserParams {
  ...
  protected class EnglishSubcategoryStripper implements TreeTransformer {
    ...
    public Tree transformTree(Tree tree) {
      ...
      if (tree.isLeaf()) {
        ...
        if (EnglishTreebankParserParams.EnglishTrain.splitBaseNP == 2 &&
            s.equals("NP")) {
        ...
        if (EnglishTreebankParserParams.EnglishTrain.splitPoss == 2 &&
            s.equals("POSSP")) {
          Tree[] kids = tree.children();
  ...
  public TreeReaderFactory treeReaderFactory() {
    return new TreeReaderFactory() {
      public TreeReader newTreeReader(Reader in) {
        return new PennTreeReader(in, new LabeledScoredTreeFactory(new StringLabelFactory()), new NPTmpRetainingTreeNormalizer(EnglishTrain.splitTMP, EnglishTrain.splitSGapped == 5, Train.leaveItAll, EnglishTrain.splitNPADV >= 1, headFinder()));
      }
    };
  }
  ...
  public MemoryTreebank testMemoryTreebank() {
    return new MemoryTreebank(new TreeReaderFactory() {
      public TreeReader newTreeReader(Reader in) {
        return new PennTreeReader(in, new LabeledScoredTreeFactory(new StringLabelFactory()), new BobChrisTreeNormalizer(tlp));
      }
    });
  }
  ...
  public TreeTransformer collinizer() {
    return new TreeCollinizer(tlp, true, EnglishTrain.splitBaseNP == 2);
  }
  ...
  @Override
  public Lexicon lex(LexOptions op) {
    if(op.uwModel == null) {
      //use default unknown word model for English
      op.uwModel = "edu.stanford.nlp.parser.lexparser.EnglishUnknownWordModel";
    }
  ...
  public static class EnglishTest {
    ...
    private EnglishTest() {}  // not instantiable
  ...
  public static class EnglishTrain {
    ...
    private EnglishTrain() {}  // not instantiable
     ...
     */
    public static boolean unaryDT = false;//true;
    /**
     * Mark "Intransitive" RB.  Good.
     */
    public static boolean unaryRB = false;//true;
    /**
     ...
     * Differenes are small. You get about 0.3 F1 by doing something; the best
     ...
     */
    public static int splitSGapped = 0;
     ...
     */
    public static int splitTMP = NPTmpRetainingTreeNormalizer.TEMPORAL_NONE;
  ...
  private static TreeFactory categoryWordTagTreeFactory =
  ...
  public Tree transformTree(Tree t, Tree root) {
    ...
    if (t.isPreTerminal()) {
      ...
      if (EnglishTrain.makePPTOintoIN && baseCat.equals("TO") &&
          baseParentStr.equals("PP")) {
        cat = changeBaseCat(cat, "IN");
      }
  ...
  public int setOptionFlag(String[] args, int i) {
    ...
    if (args[i].equalsIgnoreCase("-splitIN")) {
      ...
      // At present need to do this manually at train _and_ test time;
      // it's not serialized
      try {
