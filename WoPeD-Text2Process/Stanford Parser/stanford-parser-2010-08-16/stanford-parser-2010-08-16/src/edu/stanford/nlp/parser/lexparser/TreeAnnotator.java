...
public class TreeAnnotator implements TreeTransformer {
  ...
  private Tree transformTreeHelper(Tree t, Tree root) {
	...
	tlpParams.pw(System.err).println("\n\nTreeAnnotator ERROR: Ill-formed tree around empty node \"" + headChild.label() + "\" in tree\n" + t);
      throw cce;
    } catch (NullPointerException npe) {
      tlpParams.pw(System.err).println("TreeAnnotator: null head found for tree:\n" + t);
      throw npe;
    }
    ...
    if (Train.sisterAnnotate && !Train.smoothing && baseParentStr.length() > 0) {
      ...
      for (Iterator j = rightAnn.iterator(); j.hasNext();) {
    ...
    if (Train.xOverX && xOverX(t, baseCat)) {
      cat = cat + "-X";
    }
    if (Train.rightRec && rightRec(t, baseCat)) {
      cat = cat + "-R";
    }
    if (Train.leftRec && leftRec(t, baseCat)) {
      cat = cat + "-L";
    }
    ...
    t.setLabel(label);
    // System.out.println("internal transformTree: " +
    //                         baseCat + "^" + baseParentStr + "~" +
    //                         baseGrandParentStr + " to " +
    //                         label.value());
    return tlpParams.transformTree(t, root);
  }
  ...
  private static boolean xOverX(Tree t, String baseCat) {
    for (Tree s : t.subTreeList()) {
      ...
      if (s.label().value().startsWith(baseCat)) {
        return true;
      }
    }
    return false;
