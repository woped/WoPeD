...
public class TreeBinarizer implements TreeTransformer {
   ...
   * where the consituent after the @ is the passive that we are building
   * and  the consituent in brackets is the head
  ...
  private Label makeSyntheticLabel1(Tree t, int left, int right, int headLoc, int markovOrder) {
    ...
    String word = ((CategoryWordTag) t.label()).word();
    String tag = ((CategoryWordTag) t.label()).tag();
