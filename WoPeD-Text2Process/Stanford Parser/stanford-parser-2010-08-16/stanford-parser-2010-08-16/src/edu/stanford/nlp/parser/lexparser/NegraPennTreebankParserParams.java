...
public class NegraPennTreebankParserParams extends AbstractTreebankParserParams {
  ...
  private static boolean defaultLeaveGF = false;
  private static char defaultGFCharacter = '-';
  ...
  public NegraPennTreebankParserParams() {
    this(defaultLeaveGF, defaultGFCharacter);
  }

  public NegraPennTreebankParserParams(boolean leaveGF, char gfCharacter) {
    super(new NegraPennLanguagePack(leaveGF, gfCharacter));
 // override output encoding: make it UTF-8
  ...
  public Lexicon lex(Options.LexOptions op) {
    if (op.uwModel == null) {
      op.uwModel = "edu.stanford.nlp.parser.lexparser.GermanUnknownWordModel";
    }
  ...
  /**
   * Returns a <code>NegraPennLanguagePack</code>
   */
//  public TreebankLanguagePack treebankLanguagePack(boolean leaveGF, char gfChar) {
//    return new NegraPennLanguagePack(leaveGF, gfChar);
//  }
  ...
  public int setOptionFlag(String[] args, int i) {
    ...
    if (args[i].equalsIgnoreCase("-nodeCleanup")) {
      ...
      ((NegraPennLanguagePack) treebankLanguagePack()).setLimitedGF(Boolean.parseBoolean(args[i + 1]));
      i+=2;
    }
    return i;
  }
