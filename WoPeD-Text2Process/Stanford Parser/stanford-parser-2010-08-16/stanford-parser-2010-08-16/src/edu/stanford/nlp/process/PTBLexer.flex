...
SPACESNLS = {SPACENL}+
SENTEND = {SPACESNLS}({SPACENL}|([A-Z]|{SGML}))
DIGIT = [:digit:]|[\u07C0-\u07C9]
...
/* Tokens you want but already okay: C'mon 'n' '[2-9]0s '[eE]m 'till?
   ...
   final words. */
APOWORD = {APOS}n{APOS}?|[lLdDjJ]'|Dunkin{APOS}|somethin{APOS}|ol{APOS}|{APOS}em|C{APOSETCETERA}mon|{APOS}[2-9]0s|{APOS}till?|o{APOSETCETERA}clock|[A-Za-z][a-z]*[aeiou]{APOSETCETERA}[aeiou][a-z]*|N'[a-z]+|{APOS}cause
FULLURL = https?:\/\/[^ \t\n\f\r\"<>|()]+[^ \t\n\f\r\"<>|.!?(){},-]
...
ABPTIT = Jr|Sr|Bros|Ph\.D|Blvd|Rd|Esq
...
/* ABBREV1 abbreviations are normally followed by lower case words.  If
   ...
   sentence boundary */
ABBREV3 = {ABMONTH}|{ABDAYS}|{ABSTATE}|{ABCOMP}|{ABNUM}|{ABPTIT}|etc|al|seq
ABBREV1 = {ABBREV3}\.
...
{WORD}/{REDAUX}		{ String tmp = removeSoftHyphens(yytext());
                          if (americanize) {
...
{WORD}			{ String tmp = removeSoftHyphens(yytext());
                          if (americanize) {
                            tmp = Americanize.americanize(tmp);
                          }
                          return getNext(tmp, yytext());
...
/* U.S. is the single big source of errors, so add special rule -- it's not sentence end except when followed by this list of pronouns, determiners, and prepositions, and then it is....  Character classes make it case sensitive!  Needed!! Now any acronym can be treated as word final if followed by this list of words. */
