 ...
 */
public abstract class Tree extends AbstractCollection<Tree> implements Label, Labeled, Scored, Serializable, Iterable<Tree> {
  ...
  private int constituents(Set<Constituent> constituentsSet, int left, ConstituentFactory cf, boolean charLevel) {
    ...
    if(isPreTerminal())
      return left + ((charLevel) ? firstChild().label().value().length() : 1);
  ...
  public StringBuilder toStringBuilder(StringBuilder sb) {
    return toStringBuilder(sb, false);
  }
  ...
  public StringBuilder toStringBuilder(StringBuilder sb, boolean printOnlyLabelValue) {
    sb.append('(');
    if (label() != null) {
      if(printOnlyLabelValue) sb.append(label().value());
