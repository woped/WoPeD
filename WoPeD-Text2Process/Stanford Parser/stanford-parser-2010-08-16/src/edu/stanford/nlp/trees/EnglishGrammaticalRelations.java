...
public class EnglishGrammaticalRelations {
  ...
  private static final String timeWordRegex =
    "/(?i)^Mondays?|Tuesdays?|Wednesdays?|Thursdays?|Fridays?|Saturdays?|Sundays?|years?|months?|weeks?|days?|mornings?|evenings?|nights?|January|February|March|April|May|June|July|August|September|October|November|December|today|yesterday|tomorrow|spring|summer|fall|autumn|winter$/";
  private static final String timeWordLotRegex =
    "/(?i)^Mondays?|Tuesdays?|Wednesdays?|Thursdays?|Fridays?|Saturdays?|Sundays?|years?|months?|weeks?|days?|mornings?|evenings?|nights?|January|February|March|April|May|June|July|August|September|October|November|December|today|yesterday|tomorrow|spring|summer|fall|autumn|winter|lot$/";
  ...
  private static final String passiveAuxWordRegex =
    "/^(?i:am|is|are|be|being|'s|'re|'m|was|were|been|s|ai|seem|seems|seemed|seeming|appear|appears|appeared|become|becomes|became|becoming|get|got|getting|gets|gotten)$/";
  ...
  public static final GrammaticalRelation AUX_PASSIVE_MODIFIER =
    new GrammaticalRelation(Language.English, "auxpass", "passive auxiliary",
        ...
        new String[] {
          "VP < (/^(?:VB|AUX)/=target < " + passiveAuxWordRegex + " ) < (VP|ADJP [ < VBN|VBD | < (VP|ADJP < VBN|VBD) < CC ] )",
          "SQ < (/^(?:VB|AUX)/=target < " + beAuxiliaryRegex + " $++ (VP < /^VB[DN]$/))"
        });
  ...
  public static final GrammaticalRelation COPULA =
    new GrammaticalRelation(Language.English, "cop", "copula",
        ...
        new String[] {
          "VP < (/^(?:VB|AUX)/=target < " + copularWordRegex + " [ $++ (ADJP|NP !< VBN) | $++ (S <: (ADJP < JJ)) ] )",
          "SQ <, (/^(?:VB|AUX)/=target < " + beAuxiliaryRegex + ") !$ /^WH*/"
        });
   ...
   */
  // todo: maybe add other categories to allow conjunction like SINV. They are rare but validly occur.
  public static final GrammaticalRelation CONJUNCT =
    new GrammaticalRelation(Language.English, "conj", "conjunct",
        ConjunctGRAnnotation.class, DEPENDENT, "VP|NP|ADJP|PP|QP|ADVP|UCP|S|NX|SBAR|SBARQ",
        new String[] { // remember conjunction can be left or right headed....
          ...
          "VP|ADJP|PP|QP|NP|ADVP|UCP|S|NX|SBAR|SBARQ < (CC|CONJP $-- !/^(``|-LRB-|PRN)$/ $+ !/^(?:PRN|``|''|-[LR]RB-|,|:|\\.)$/=target) !<, CC",
          // non-parenthetical or comma in suitable phrase with conj then adverb to left
          "VP|ADJP|PP|NP|ADVP|UCP|S|NX|SBAR|SBARQ < (CC|CONJP $-- !/^(``|-LRB-|PRN)$/ $+ (ADVP $+ !/^(?:PRN|``|''|-[LR]RB-|,|:|\\.)$/=target))",
          // content phrase to the right of a comma or a parenthetical
          "VP|ADJP|PP|NP|ADVP|UCP|S|NX|SBAR|SBARQ < (CC|CONJP $-- !/^(``|-LRB-|PRN)$/) < (/^(?:PRN|``|''|-[LR]RB-|,|:|\\.)$/ $+ /^S$|^(?:A|N|V|PP|PRP|J|W|R)/=target)",
          // content phrase to the left of a comma for at least NX
          "NX < (CC|CONJP $- __) < (/^,$/ $- /^(A|N|V|PP|PRP|J|W|R|S)/=target)",
          // to take the conjunct in a preconjunct structure "either X or Y"
          "VP|ADJP|PP|QP|NP|ADVP|UCP|S|NX|SBAR|SBARQ < (CC $++ (CC|CONJP $+ !/^(?:PRN|``|''|-[LR]RB-|,|:|\\.)$/=target))"
        });
  ...
  public static final GrammaticalRelation COORDINATION =
    new GrammaticalRelation(Language.English, "cc", "coordination",
        CoordinationGRAnnotation.class, DEPENDENT, "S|VP|NP|QP|ADJP|PP|ADVP|UCP|NX|SBAR|SBARQ",
        new String[] {
          "S|VP|NP|QP|ADJP|PP|ADVP|UCP|NX|SBAR|SBARQ [ < (CC=target !< /^(?i:either|neither|both)$/) | < (CONJP=target !< (RB < /^(?i:not)$/ $+ (RB|JJ < /^(?i:only|just|merely)$/))) ]"
        });
  ...
  public static final GrammaticalRelation NOMINAL_SUBJECT =
    new GrammaticalRelation(Language.English, "nsubj", "nominal subject",
        ...
        new String[] {
          ...
          "SINV < (/^NP|WHNP$/=target [ $- VP|VBZ|VBD|VBP|VB|MD|AUX | $- (@RB|ADVP $- VP|VBZ|VBD|VBP|VB|MD|AUX) ] )",
  ...
  public static final GrammaticalRelation DIRECT_OBJECT =
    new GrammaticalRelation(Language.English, "dobj", "direct object",
        ...
        new String[] {
          ...
          "VP < (NP $+ (/^(NP|WHNP)$/=target !< (/^NN/ < " + timeWordLotRegex + "))) !<(/^VB|AUXG?/ < " + copularWordRegex + ")",  // this time one also included "lot"
          // match "give it next week"
          "VP < (NP < (NP $+ (/^(NP|WHNP)$/=target !< (/^NN/ < " + timeWordLotRegex + "))))!< (/^VB|AUXG?/ < " + copularWordRegex + ")",  // this time one also included "lot"
          // basic direct object cases
          "VP !<(/^VB|AUXG?/ < " + copularWordRegex + ") < (/^(NP|WHNP)$/=target [ [ !< (/^NN/ < " + timeWordRegex + ") !$+ NP ] | $+ NP-TMP | $+ (NP < (/^NN/ < " + timeWordRegex + ")) ] )",
          ...
          "SBARQ < (WHNP=target !< WRB) <+(SQ|SINV|S|VP) (VP !< (S < (VP < TO)) !< (/^VB|AUXG?/ < " + copularWordRegex + " !$++ (VP < VBG)) !<- PRT !<- (PP <: IN) $-- (NP !< /^-NONE-$/))",  // this pattern used to assume no empty NPs. Corrected.
   ...
   * phrase being matched as a pobj.)
   */
  public static final GrammaticalRelation PREPOSITIONAL_OBJECT =
    new GrammaticalRelation(Language.English, "pobj", "prepositional object",
        PrepositionalObjectGRAnnotation.class, OBJECT, "PP(?:-TMP)?|WHPP|PRT",
        new String[] {
          "/^PP(?:-TMP)?|WHPP$/ < (IN|VBG|TO|FW $++ (/^NP(?:-TMP)?|WHNP|ADJP$/=target !$- @NP))",
          "/^PP(?:-TMP)?$/ < (/^IN|VBG|TO/ $+ (ADVP=target [ < (RB < /^(here|there)$/) | < (ADVP < /^NP(?:-TMP)?$/) ] ))",
          ...
          "(PP <: IN) >- (VP !< (S < (VP < TO)) >+(SQ|SINV|S|VP) (SBARQ <, (WHNP=target !< WRB)) $-- (NP !< /^-NONE-$/))",
          "(PP <: IN) $- (NP $-- (VBZ|VBD) !$++ VP) >+(SQ) (SBARQ <, (WHNP=target !< WRB)) $-- (NP !< /^-NONE-$/)",

         // to deal with preposition stranding in relative clause (works well but doesn't preserve the tree structure!)
         "(PP <- IN) >+(@VP|S|SINV|SBAR) (SBAR !< (WHPP|WHNP) < (S < (NP $+ (VP !<(/^VB|AUXG?/ < " + copularWordRegex + " !$+ VP) !<+(VP) NP !< SBAR ))) $-- NP > NP=target)",
  ...
  public static final GrammaticalRelation ATTRIBUTIVE =
    new GrammaticalRelation(Language.English, "attr", "attributive",
        ...
        new String[] {
          "VP !$ (NP < EX) < NP=target <(/^VB|AUXG?/ < " + copularWordRegex + ")",
          // "What is that?"
          "SBARQ < (WHNP|WHADJP=target $+ (SQ < (/^VB|AUX?/ < " + copularWordRegex + " !$++ VP) !< (VP <- (PP <:IN)) !<- (PP <: IN)))",
          "SBARQ <, (WHNP|WHADJP=target !< WRB) <+(SQ|SINV|S|VP) (VP !< (S < (VP < TO)) < (/^VB|AUX?/ < " + copularWordRegex + " !$++ (VP < VBG)) !<- PRT !<- (PP <: IN) $-- (NP !< /^-NONE-$/))",
          //"Is he the man?"
  ...
  public static final GrammaticalRelation CLAUSAL_COMPLEMENT =
    new GrammaticalRelation(Language.English, "ccomp", "clausal complement",
        ClausalComplementGRAnnotation.class, COMPLEMENT, "VP|SINV|S|ADJP|NP",
  ...
  public static final GrammaticalRelation COMPLEMENTIZER =
    new GrammaticalRelation(Language.English, "complm", "complementizer",
        ...
        new String[] {
          "SBAR <, (IN|DT=target < /^(?:that|whether)$/) [ $-- /^VB|AUXG?/ | $- NP|NN|NNS | > ADJP|PP | > (@NP|UCP|SBAR < CC|CONJP $-- /^VB|AUXG?/) ]",
  ...
  public static final GrammaticalRelation ADV_CLAUSE_MODIFIER =
    new GrammaticalRelation(Language.English, "advcl", "adverbial clause modifier",
        ...
        new String[] {   // !$+ (NN < /^order$/) has been added so that "in order to" is not marked as an advcl
          "VP < (@SBAR=target [ <, (IN !< /^([Tt]hat|[Ww]hether)$/ !$+ (NN < /^order$/)) | <: (SINV <1 /^(?:VB|MD)/) ] )", // second disjunct matches inverted "had he investigated" cases
          "S|SQ <, (/^SBAR(?:-TMP)?$/=target <, (IN !< /^([Tt]hat|[Ww]hether)$/ !$+ (NN < /^order$/)) !$+ VP)",
          // to get "rather than"
          "S|SQ <, (/^SBAR(?:-TMP)?$/=target <2 (IN !< /^([Tt]hat|[Ww]hether)$/ !$+ (NN < /^order$/)))",
          "S|SQ < (/^SBAR(?:-TMP)?$/=target <, (IN !< /^([Tt]hat|[Ww]hether)$/ !$+ (NN < /^order$/)) !$+ @VP $+ /^,$/ $++ @NP)", // this one might just be better, but at any rate license one with quotation marks or a conjunction beforehand
  ...
  public static final GrammaticalRelation TEMPORAL_MODIFIER =
    new GrammaticalRelation(Language.English, "tmod", "temporal modifier",
        TemporalModifierGRAnnotation.class, MODIFIER, "VP|S|ADJP|PP|SBAR",
        new String[] {
          ...
          "SBAR < (@WHADVP < (WRB < when)) < (S < (NP $+ (VP !< (/^VB|AUXG?/ < " + copularWordRegex + " !$+ VP) ))) !$-- CC $-- NP > NP=target",
          });
  ...
  public static final GrammaticalRelation RELATIVE_CLAUSE_MODIFIER =
    new GrammaticalRelation(Language.English, "rcmod", "relative clause modifier",
        RelativeClauseModifierGRAnnotation.class, MODIFIER, "(?:WH)?NP",
        new String[] {
          "NP|WHNP $++ (SBAR=target <+(SBAR) WHPP|WHNP) > @NP|WHNP",
          "NP|WHNP $++ (SBAR=target <: (S !<, (VP <, TO))) > @NP|WHNP",
  ...
  public static final GrammaticalRelation NUMERIC_MODIFIER =
    new GrammaticalRelation(Language.English, "num", "numeric modifier",
        ...
        new String[] {
          "/^(?:WH)?NP(?:-TMP|-ADV)?$/ < (CD|QP=target !$- CC)",
          "/^(?:WH)?NP(?:-TMP|-ADV)?$/ < (ADJP=target <: QP)"
        });
  ...
  public static final GrammaticalRelation NOUN_COMPOUND_MODIFIER =
    new GrammaticalRelation(Language.English, "nn", "nn modifier",
        NounCompoundModifierGRAnnotation.class, MODIFIER, "(?:WH)?NP(?:-TMP|-ADV)?",
        new String[] {
          "/^(?:WH)?NP(?:-TMP|-ADV)?$/ < (NP|NN|NNS|NNP|NNPS|FW=target $++ NN|NNS|NNP|NNPS|FW|CD !<- POS !$- /^,$/ )",
          "/^(?:WH)?NP(?:-TMP|-ADV)?$/ < (NP|NN|NNS|NNP|NNPS|FW=target !<- POS $+ JJ|JJR|JJS) <# NN|NNS|NNP|NNPS !<- POS"
        });
  ...
  public static final GrammaticalRelation ABBREVIATION_MODIFIER =
    new GrammaticalRelation(Language.English, "abbrev", "abbreviation modifier",
        ...
        new String[] {
          "/^(?:WH)?NP(?:-TMP|-ADV)?$/ < (PRN=target <, /^-LRB-$/ <- /^-RRB-$/ !<< /^(?:POS|PRP\\$|[,$#]|CC|RB|CD)$/ <+(NP) (NNP|NN < /^(?:[A-Z]\\.?){2,}/) )"
        });
  ...
  public static final GrammaticalRelation PARTICIPIAL_MODIFIER =
    new GrammaticalRelation(Language.English, "partmod", "participial modifier",
        ParticipialModifierGRAnnotation.class, MODIFIER, "(?:WH)?NP(?:-TMP|-ADV)?|VP|S",
        new String[] {
  ...
  public static final GrammaticalRelation ADVERBIAL_MODIFIER =
    new GrammaticalRelation(Language.English, "advmod", "adverbial modifier",
        ...
        "VP|ADJP|WHADJP|ADVP|WHADVP|S|SBAR|SINV|SQ|SBARQ|XS|(?:WH)?NP(?:-TMP|-ADV)?|RRC",
        new String[] {
          "/^(?:VP|ADJP|WHADJP|S|SBAR|SINV|SQ|XS|NP(?:-TMP|-ADV)?|RRC|(?:WH)?NP(?:-TMP|-ADV)?)$/ < (RB|RBR|RBS|WRB|ADVP|WHADVP=target !< " + NOT_PAT + ")",
  ...
  public static final GrammaticalRelation PRECONJUNCT =
    new GrammaticalRelation(Language.English, "preconj", "preconjunct",
        ...
        new String[] {
          // "/^NP(?:-TMP|-ADV)?|NX$/ < (PDT|CC=target < /^(?i:both|neither|either)$/ [ $++ /^N[NX]/ | $++ (/^JJ/ !$+ /^NN/) ] ) $++ CC",  // cdm Jun 2010: This pattern was matching nothing, since it was looking for the second CC as sister of the top NP not of the preconjunct.  But if you move the close parenthesis right, you'd just get a more restricted form of the second pattern....
          // This is the normal/correct NP-internal preconjunct
          "/^NP(?:-TMP|-ADV)?|NX$/ < (PDT|CC|DT=target < /^(?i:both|neither|either)$/ $++ CC)",
          "/^NP(?:-TMP|-ADV)?|NX$/ < (CONJP=target < (RB < /^(?i:not)$/) < (RB|JJ < /^(?i:only|merely|just)$/) $++ CC|CONJP)",
          // This matches weird/wrong NP-internal preconjuncts where you get (NP PDT (NP NP CC NP)) or similar
          "/^NP(?:-TMP|-ADV)?|NX$/ < (PDT|CC|DT=target < /^(?i:both|neither|either)$/) < (NP < CC)",
          "S|VP|ADJP|PP|ADVP|UCP|NX|SBAR < (PDT|DT|CC=target < /^(?i:both|neither|either)$/ $++ CC)",
          "S|VP|ADJP|PP|ADVP|UCP|NX|SBAR < (CONJP=target < (RB < /^(?i:not)$/) < (RB|JJ < /^(?i:only|merely|just)$/) $++ CC|CONJP)"
        });
  ...
  public static final GrammaticalRelation PREPOSITIONAL_MODIFIER =
    new GrammaticalRelation(Language.English, "prep", "prepositional modifier",
        PrepositionalModifierGRAnnotation.class, MODIFIER, "(?:WH)?NP(?:-TMP|-ADV)?|VP|S|SINV|SQ|ADJP|SBARQ|NAC",
        new String[] {
          "/^(?:WH)?NP(?:-TMP|-ADV)?|VP|ADJP|NAC|SQ$/ < /^(?:WH)?PP(?:-TMP)?$/=target",
