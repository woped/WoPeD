...
public class TreePrint {
  ...
  private boolean nonCollapsedDependencies;
  private boolean nonCollapsedDependenciesSeparated;
  ...
  public TreePrint(String formats, String options, TreebankLanguagePack tlp) {
    this(formats, options, tlp, tlp.headFinder());
  }
   ...
   *                conll2008.  The last two are both tab-separated values
   *                formats.  The latter has a lot mroe columns filled with
  ...
  private void printTreeInternal(final Tree t, final PrintWriter pw, final boolean inXml) {
    ...
    if (propertyToBoolean(options, "xml")) {
      ...
      if (formats.containsKey("conll2008") || formats.containsKey("conllStyleDependencies")) {
        System.err.println("The \"conll2008\" and \"conllStyleDependencies\" formats are ignored in xml.");
      }
      ...
      if (formats.containsKey("conll2008")) {
        Tree it = outputTree.deeperCopy
           (outputTree.treeFactory(), CoreLabel.factory());
        it.indexLeaves();
        ...
        for (int wi = 0, sz = tagged.size(); wi < sz; ++wi) {
