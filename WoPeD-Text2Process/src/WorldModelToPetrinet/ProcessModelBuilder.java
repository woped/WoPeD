...
private WordNetFunctionality wordnet= new WordNetFunctionality();
        ...
        private final boolean REMOVE_LOW_ENTROPY_NODES = true;//"1".equals(f_config.getProperty(Constants.CONF_GENERATE_REMOVE_LOW_ENT_NODES));
        private final boolean HIGHLIGHT_LOW_ENTROPY = true;//"1".equals(f_config.getProperty(Constants.CONF_GENERATE_HIGHLIGHT_META_ACTIONS));
        //Labeling
        private final boolean ADD_UNKNOWN_PHRASETYPES =true;// "1".equals(f_config.getProperty(Constants.CONF_GENERATE_ADD_UNKNOWN_PT));
        private final int MAX_NAME_DEPTH = 3;
        //Model in General
        private final boolean BUILD_BLACK_BOX_POOL_COMMUNICATION = true;//"1".equals(f_config.getProperty(Constants.CONF_GENERATE_BB_POOLS));
        private final boolean BUILD_DATA_OBJECTS = true;//"1".equals(f_config.getProperty(Constants.CONF_GENERATE_DATA_OBJECTS));
...
/*        private void buildBlackBoxPools(WorldModel world) {
      ...
      /*  private void checkForBBPools(Action a) {
            //candidate
            Specifier _sender = containedSender(a.getSpecifiers(Specifier.SpecifierType.PP));
            Specifier _receiver = containedReceiver(a.getSpecifiers(Specifier.SpecifierType.PP));
            ...
            if(_obj != null && !(_obj instanceof Actor)) {
                //checking genetive
                for(Specifier spec:_obj.getSpecifiers(Specifier.SpecifierType.PP)) {
                    if(spec.getPhraseType().equals(FrameNetFunctionality.PhraseType.GENITIVE)) {
