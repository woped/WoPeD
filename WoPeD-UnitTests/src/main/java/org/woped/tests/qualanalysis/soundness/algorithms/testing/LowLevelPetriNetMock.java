...
public class LowLevelPetriNetMock implements ILowLevelPetriNet {
  ...
  public LowLevelPetriNetMock() {
    ...
    //		Create transitions
    transitions[0] =
        new TransitionNode("t3", "t3", "t3", OperatorTransitionModel.TRANS_SIMPLE_TYPE);
    transitions[1] =
        new TransitionNode("t2", "t2", "t2", OperatorTransitionModel.TRANS_SIMPLE_TYPE);
    transitions[2] =
    ...
    for (int i = 0; i < transitions.length; i++) this.transitions.add(transitions[i]);

    for (int i = 0; i < places.length; i++) this.places.add(places[i]);
  }

  @Override
  public Set<AbstractNode> getAllContainedNodes() {
    Set<AbstractNode> set = new HashSet<AbstractNode>();
    for (TransitionNode node : transitions) set.add(node);
    for (PlaceNode node : places) set.add(node);
    return set;
  }
