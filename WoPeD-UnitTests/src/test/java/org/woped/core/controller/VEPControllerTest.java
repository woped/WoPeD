...
public class VEPControllerTest {
  ...
  public void getViewEventProcessorForType_NoneRegistered_ReturnsEmptyList() throws Exception {
    Collection<AbstractEventProcessor> eventProcessors =
        cut.getViewEventProcessorsForType(TEST_VIEW_EVENT_TYPE);
  ...
  @Test
  public void getViewEventProcessorForType_TwoRegisteredDifferentEventTypes_ReturnsOne()
      throws Exception {
    ...
    Collection<AbstractEventProcessor> eventProcessors =
        cut.getViewEventProcessorsForType(TEST_VIEW_EVENT_TYPE);
    assertEquals(
        "There should be only one registered processor for this type", eventProcessors.size(), 1);
    assertTrue(
        "The registered processor for this type is not contained",
        eventProcessors.contains(fakeProcessor1));
  }

  @Test
  public void getViewEventProcessorForType_TwoRegisteredSameEventType_ReturnsBoth()
      throws Exception {
    AbstractEventProcessor fakeProcessor1 = mock(AbstractEventProcessor.class);
    AbstractEventProcessor fakeProcessor2 = mock(AbstractEventProcessor.class);
    cut.register(TEST_VIEW_EVENT_TYPE, fakeProcessor1);
    ...
    Collection<AbstractEventProcessor> eventProcessors =
        cut.getViewEventProcessorsForType(TEST_VIEW_EVENT_TYPE);
    assertEquals(
        "There should be two registered processors for this type", eventProcessors.size(), 2);
  }

  @Test
  public void getViewEventProcessorForType_oneRegisteredAndUnregistered_returnsEmptyList()
      throws Exception {
    AbstractEventProcessor fakeProcessor = mock(AbstractEventProcessor.class);
  ...
  public void unregister_processorNotRegistered_logsWarning() throws Exception {
    ...
    verify(fakeLogger)
        .warn(
            "Tried to unregister a AbstractViewEventProcessor which has not registered for this"
                + " type.");
  }
  ...
  public void unregister_unregisterRegisteredProcessorTwice_logsWarning() throws Exception {
    ...
    verify(fakeLogger)
        .warn(
            "Tried to unregister a AbstractViewEventProcessor which has not registered for "
                + "this type.");
  }
}
