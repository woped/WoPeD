...
public class MarkingNetTest {
  ...
  public void getActivatedTransitions_demoNet_resultHasSizeOne() throws Exception {
    ...
    assertTrue(
        "The activated transition should be t1", activatedTransitions[0].getId().equals("t1"));
  }

  @Test
  public void getActivatedTransitions_DemoNetP2IsUnlimited_transitionIsActivated()
      throws Exception {
    LowLevelPetriNet net = createDemoLowLevelNet();
    MarkingNet cut = new MarkingNet(net);
    ...
    assertTrue(
        "The first activated transition should be t1",
        activatedTransitions[0].getId().equals("t1"));
    assertTrue(
        "The second activated transition should be t2",
        activatedTransitions[1].getId().equals("t2"));
  }
  ...
  public void calculateSucceedingMarking_demoNet_producesMarking010() throws Exception {
    ...
    TransitionNode t1 =
        net.getTransitionNode(
            new TransitionNode("t1", "t1", "t1", AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE));
   ...
   *
   * <p>Creates net: p1 -> t1 -> p2 -> t2 -> p3 with marking ( 1 0 0 )
   *
  ...
  private LowLevelPetriNet createDemoLowLevelNet() {
    ...
    TransitionNode t1 =
        new TransitionNode("t1", "t1", "t1", AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE);
    TransitionNode t2 =
        new TransitionNode("t2", "t2", "t2", AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE);
