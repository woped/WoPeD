...
public class MpNodeTest {
  ...
  public void setup() {
    marking =
        new MarkingGenerator()
            .createDemoMarking(new int[] {1, 0, 0}, new boolean[] {false, false, false});
  ...
  public void setState_newState_notifiesListeners() throws Exception {
    ...
    ArgumentCaptor<NodeChangedEvent> argumentCaptor =
        ArgumentCaptor.forClass(NodeChangedEvent.class);
    ...
    assertEquals(
        "Previous state should be " + previousState, previousState, event.getPreviousState());
