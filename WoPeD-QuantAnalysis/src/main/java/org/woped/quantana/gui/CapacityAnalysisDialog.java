...
public class CapacityAnalysisDialog extends JDialog {
      ...
      jp.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("QuantAna.CapacityPlanning.Heading0")),
  ...
  private JPanel getValuePanel() {
    if (valuePanel == null) {
      ...
      lblThreshold = new JLabel(Messages.getString("QuantAna.Config.TerminationThreshold"));
  ...
  private JPanel getTasksPanel() {
    if (tasksPanel == null) {
      ...
      tasksPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("QuantAna.CapacityPlanning.Heading1")),
              BorderFactory.createEmptyBorder(5, 5, 0, 5)));
  ...
  private JPanel getResourcesPanel() {
    if (resourcesPanel == null) {
      ...
      resourcesPanel.setBorder(
          BorderFactory.createCompoundBorder(
              BorderFactory.createTitledBorder(
                  Messages.getString("QuantAna.CapacityPlanning.Heading2")),
              BorderFactory.createEmptyBorder(5, 5, 0, 5)));
  ...
  private JPanel getResourceUtilPanel() {
    if (resourceUtilPanel == null) {
      ...
      constraints.gridx = 0;
      constraints.gridy = 0;
      lblResourceUtil = new JLabel(Messages.getString("QuantAna.CapacityPlanning.Utilization"));
  ...
  private JTable getTasksTable() {
    ...
    if (tasksTable == null) {
      ...
      for (int i = 0; i < j; i++) {
        ...
        tableTasksMatrix[i][4] = String.format("%15." + currPrec + "f", runs[i]);
        tableTasksMatrix[i][1] = String.format("%15." + currPrec + "f", times[i]);
      }

      tasksMatrix[j][2] = 0.0;
      tasksMatrix[j][4] = 0.0;

      tableTasksMatrix[j][0] = Messages.getString("QuantAna.CapacityPlanning.Aggregate");

      tmTasks = new TasksTableModel(colTasks, tableTasksMatrix);
      tmTasks.addTableModelListener(
      ...
      tasksTable.setDefaultRenderer(Object.class, new MyTableCellRenderer());
  ...
  private JTable getResourcesTable() {
    if (resourcesTable == null) {
      ...
      resourcesTable.setDefaultRenderer(Object.class, new MyTableCellRenderer());
  ...
  private void unfoldNet(WorkflowNetGraph g, double l, double e) {
    ...
    for (Node n : g.getNodeArray()) {
      if (n.isJoinReached()) n.setJoinReached(false);
      ...
      for (Arc a : np.getFirst().getSuccessor()) {
        ...
        if (!m.isJoinReached()) {
          ...
          if (val >= epsilon) {
            ...
            if (!(containsElement(q, m))) q.addLast(new NodePair(m, y));
          }
        }
      }
  ...
  private LinkedList<TransitionModel> getTransModels() {
    ...
    for (int i = 0; i < nodes.length; i++)
      if (graph.isTransition(nodes[i].getId())) ids.add(nodes[i].getId());
  ...
  static class MyTableCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (column == 0 || column == 6) setHorizontalAlignment(LEFT);
  ...
  public void updParameters() {
    ...
    setPeriod(tm.cv(cboPeriodUnit.getSelectedIndex(), Double.parseDouble(txtPeriod.getText())));
    setPeriodIntervall(convTIndexToString(tm.getStdUnit()));
    lblTimeUnit.setText(
        Messages.getString("QuantAna.CapacityPlanning.TimeUnit")
            + " "
  ...
  public void updContents() {
    ...
    tb[numTransGT0][3] =
        String.format("%15." + currPrec + "f", sumCase); // Double.valueOf(sumCase);
    tmTasks.fireTableCellUpdated(numTransGT0, 3);
    tb[numTransGT0][5] =
        String.format("%15." + currPrec + "f", sumPeriod); // Double.valueOf(sumPeriod);
    ...
    lblUnfolding.setText("The unfolded net has " + unfoldedNet.size() + " nodes.");
  ...
  private void updPrecision() {
    ...
    for (int i = 0; i < numTransGT0; i++) {
      for (int j = 0; j < numCols1 - 2; j++) {
        tmTasks.setValueAt(String.format("%15." + currPrec + "f", tasksMatrix[i][j]), i, j + 1);
      }
    }

    tmTasks.setValueAt(
        String.format("%15." + currPrec + "f", tasksMatrix[numTransGT0][2]), numTransGT0, 3);
    tmTasks.setValueAt(
        String.format("%15." + currPrec + "f", tasksMatrix[numTransGT0][4]), numTransGT0, 5);

    for (int i = 0; i < numResCls; i++) {
      tmRes.setValueAt(String.format("%15." + currPrec + "f", resMatrix[i][0]), i, 1);
      tmRes.setValueAt(String.format("%15." + currPrec + "f", resMatrix[i][1]), i, 2);
    }
  }
