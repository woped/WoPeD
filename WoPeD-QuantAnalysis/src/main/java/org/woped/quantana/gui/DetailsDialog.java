...
public class DetailsDialog extends JDialog {
  ...
  static JFreeChart createDistributionChart(
      XYDataset dataset, String title, String xtitle, String ytitle) {
    JFreeChart chart =
  ...
  XYDataset createDataset(int[] values) {
    ...
    for (int i = left; i < right; i++) series.add(i - 100, values[i]);
  ...
  private String produceID(String key) {
    if (key.equals(Messages.getString("QuantAna.Simulation.Process"))) return key;
