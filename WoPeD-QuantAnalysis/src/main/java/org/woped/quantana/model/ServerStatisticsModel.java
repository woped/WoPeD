...
public class ServerStatisticsModel {
  ...
  private HashMap<String, HashMap<String, ArrayList<ActivityPanel>>> sraList =
      new HashMap<String, HashMap<String, ArrayList<ActivityPanel>>>();
  private int rowNum = 0;
  private int numTasks = 0;
  private String[] sortedTasks;
  private ArrayList<ArrayList<ArrayList<ActivityPanel>>> sortedMatrix;

  public ServerStatisticsModel(TasksResourcesAllocation tra, ArrayList<ActivityPanel> ap) {
    this.tra = tra;
    this.actList = ap;
  ...
  private void makeSRAList() {
    for (String s : tra.getTasks()) {
      ...
      HashMap<String, ArrayList<ActivityPanel>> hl =
          new HashMap<String, ArrayList<ActivityPanel>>();
  ...
  public ArrayList<ArrayList<ArrayList<ActivityPanel>>> getSortedMatrix() {
    if (sortedMatrix == null) {
      ...
      if (sortedTasks == null) makeSortedTasks();
  ...
  private void makeSortedTasks() {
    ...
    for (int i = 0; i < o.length; i++) sortedTasks[i] = (String) o[i];
  ...
  public ArrayList<ActivityPanel> getActivityPanelsList(String server, String resource) {
    return (sraList.get(server)).get(resource);
  }

  public String getServer(int row) {
    if (sortedMatrix == null) getSortedMatrix();
