...
public class SimulationStorageDataProcess extends SimulationObject {
  ...
  private double DeliveryAdherence =
      0.0; // Summe fertiggestellter Ergebnisse ohne Verzug / Summe aller fertiggestellten
  // Ergebnisse
  ...
  private void setAvgDeliveryAdherence() {
    if (this.AvgFinishedCases != 0) {
      // LoggerManager.debug(Constants.DASHBOARDSTORE_LOGGER, "setAvgDeliveryAdherence: " +
      // UIDCreater.round((AvgThroughPut)) + "/" + UIDCreater.round((AvgFinishedCases)) + " = " +
      // UIDCreater.round((AvgThroughPut / AvgFinishedCases)));
  ...
  private void setDeliveryAdherence() {
    if (this.FinishedCases != 0) {
      // LoggerManager.debug(Constants.DASHBOARDSTORE_LOGGER, "setDeliveryAdherence: " +
      // UIDCreater.round((ThroughPut)) + "/" + UIDCreater.round((FinishedCases)) + " = " +
      // UIDCreater.round((ThroughPut / FinishedCases)));
