...
public class WorkflowNetGraph {
  ...
  private void initNodeArray() {
    Node dummy = new Node("", "");
    for (int i = 0; i < nodeArray.length; i++) nodeArray[i] = dummy;
  }

  public int getNodeIdx(String id) {
    int f = nodeArray.length;
    for (int i = 0; i < f; i++) {
      if (nodeArray[i].getId().equals(id)) f = i;
    }
  ...
  private void buildGraph(Node n) {
    Iterator<AbstractPetriNetElementModel> postNodes =
        mec.getTargetElements(n.getId()).values().iterator();
    Iterator<AbstractPetriNetElementModel> preNodes =
        mec.getSourceElements(n.getId()).values().iterator();

    if (mec.getTargetElements(n.getId()).size() > 1) n.setFork(true);
        ...
        if (id.equals(sinkPlace.getId())) {
          ...
          switch (currentPlace.getType()) {
            ...
            case AbstractPetriNetElementModel.TRANS_OPERATOR_TYPE:
              ...
              postNode.setTimeUnit(
                  getTimeUnitConst(
                      ((TransitionModel) currentPlace).getToolSpecific().getTimeUnit()));
              ...
              if ((type == OperatorTransitionModel.XOR_JOIN_TYPE)
  ...
  public int getNumTransitions() {
    int num = 0;
    for (int i = 0; i < nodeArray.length; i++) {
      if (isTransition(nodeArray[i].getId())) num++;
    }
  ...
  public int getNumTransitionsGT0() {
    int num = 0;
    for (int i = 0; i < nodeArray.length; i++) {
      if (isTransitionGT0(nodeArray[i].getId())) num++;
    }
  ...
  public ArrayList<Node> getInputNodes(Node n) {
    ...
    for (int i = 0; i < nodeArray.length; i++) {
      for (Arc a : nodeArray[i].getSuccessor()) {
        if (n.equals(a.target)) list.add(nodeArray[i]);
      }
    }
