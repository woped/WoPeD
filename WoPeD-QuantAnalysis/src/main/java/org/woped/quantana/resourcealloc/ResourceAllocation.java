...
public class ResourceAllocation {
  ...
  public ResourceAllocation(
      ...
      Iterator<TransitionModel> tasks,
      PetriNetModelProcessor pmp) {
    ...
    while (tasks.hasNext()) {
      ...
      for (int i = 0; i < res.size(); i++) {
        ...
        for (int j = 0; j < classes.size(); j++) {
          ...
          if (resClsType.get(cls).equals("role")) resObj.getRoles().add(cls);
          else resObj.getGroups().add(cls);
        }
      }
      ...
      while (iter.hasNext()) {
        ...
        for (AllocationTableItem t : taskAlloc2.getTable().values()) {
          if (t.getRoles().contains(rcta.getResClass())
              || t.getGroups().contains(rcta.getResClass())) rcta.getTasks().add(t.getTask());
        }
  ...
  public ArrayList<Resource> getListPerRole(String role) {
    ...
    if (roles.contains(role)) {
      for (Resource r : resources.values()) {
        if (r.getRoles().contains(role)) list.add(r);
      }
    }
  ...
  public ArrayList<Resource> getListPerGroup(String group) {
    ...
    if (roles.contains(group)) {
      for (Resource r : resources.values()) {
        if (r.getGroups().contains(group)) list.add(r);
      }
    }
  ...
  public ArrayList<String> getTasksForRole(String role) {
    ...
    for (int i = 0; i < table.size(); i++) {
      if (table.get(i).getRoles().contains(role)) list.add(table.get(i).getTask());
    }
  ...
  public ArrayList<String> getTasksForGroup(String group) {
    ...
    for (int i = 0; i < table.size(); i++) {
      if (table.get(i).getGroups().contains(group)) list.add(table.get(i).getTask());
    }
      ...
      for (String r : roles) {
        for (String g : groups) {
          for (Resource res : resources.values()) {
            if (res.getRoles().contains(r) && res.getGroups().contains(g)) list.add(res);
          }
        }
      }
    }
