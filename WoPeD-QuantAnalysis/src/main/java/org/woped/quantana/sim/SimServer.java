...
public class SimServer {
  ...
  public SimServer(
      SimRunner sim, String id, String name, String role, String group, SimDistribution dist) {
    this.id = id;
    this.sim = sim;
    this.role = role;
    this.group = group;
    this.dist = dist;
    this.name = name;
    if (dist != null) distLogger = new SimDistributionLogger(this.dist.getMean());
    rand = new MRG32k3a();
  }
  ...
  public void updateRunStats(double time, int type) {
    ...
    if (type > 0) {
      if (num >= max) {
        maxParallel = num + 1;
        while (rProbs.size() < maxParallel) rProbs.add(new Double(0));
      }
      numCaseParallel++;
    } else {
      numCaseParallel--;
    ...
    switch (num) {
      ...
      default:
        if (this instanceof SimSplitServer) {
          for (SimOutServer s : out) list.add(s.getserver());
          ...
          for (int i = 0; i < num; i++) {
            probs[i][0] = i;
            probs[i][SimRunner.MIN_VALUE] =
                (Double.valueOf(out.get(i).getprobability() * 100)).intValue();
            if (i > 0) {
              probs[i][SimRunner.MAX_VALUE] =
                  probs[i][SimRunner.MIN_VALUE] + probs[i - 1][SimRunner.MAX_VALUE];
            } else {
          ...
          for (int i = 0; i < num; i++) {
            if (i == 0) {
              ...
              if ((rnd >= probs[i - 1][SimRunner.MAX_VALUE])
                  && (rnd < probs[i][SimRunner.MAX_VALUE])) {
                idx = i;
              }
            }
          }
          if (out.get(idx).getserver() == null) list = null;
  ...
  SimCase getCaseFromQueue() {
    return (sim.getQueueDiscipline() == SimRunner.Q_FIFO)
        ? queue.removeFirst()
        : queue.removeLast();
  }
  ...
  public void handleQueue(double time) {
    ...
    if (queue.isEmpty()) sim.getQueueingServers().remove(this);
  }
