...
public class SimRunner implements Runnable {
  ...
  public SimRunner(SimGraph graph, ResourceUtilization resUtil, SimParameters sp) {
    this.graph = graph;
  ...
  private void createServerForBirth() {
    ...
    if (out.size() == 1) {
      ...
      for (int i = 0; i < out.size(); i++) {
        ...
        fstProb[i][IND] = i;
        fstProb[i][MIN_VALUE] =
            Double.valueOf(((SimArc) out.get(i)).getProbability() * 100).intValue();
        fstProb[i][MAX_VALUE] =
            (i > 0) ? fstProb[i][MIN_VALUE] + fstProb[i - 1][MAX_VALUE] : fstProb[i][MIN_VALUE];
      }
    }
  }
  ...
  public SimServer getFirstServer() {
    ...
    for (int i = 0; i < fstProb.length; i++) {
      if (i == 0) {
        if (rnd < fstProb[0][MAX_VALUE]) index = i;
      } else {
        if ((rnd >= fstProb[i - 1][MAX_VALUE]) && (rnd < fstProb[i][MAX_VALUE])) index = i;
      }
    }
    return serverList.get(graph.getSource().getarcOut().get(index).getTarget().getid());
  }
  ...
  private boolean hasToStop() {
    switch (params.getStop()) {
      case STOP_BOTH:
        return (cntFinished >= params.getLambda()) || (runClock >= params.getPeriod());
      case STOP_CASE:
  ...
  private void initRun(int numRun) {
    ...
    // reset all Servers
    for (SimServer s : serverList.values()) s.reset();
    ...
    for (Resource r : resAlloc.getResources().values()) r.reset();
    caseMaker.reset();
    SimDistribution dist =
        new SimDistribution(
            params.getDistCases(), params.getPeriod() / params.getLambda(), params.getCParam());
    ...
    for (SimServer s : serverList.values()) {
      ...
      for (SimArc a : n.getarcOut()) {
        ...
        for (SimArc b : m.getarcOut()) {
          SimOutServer os =
              new SimOutServer(
                  serverList.get(b.getTarget().getid()), a.getProbability() * b.getProbability());
    ...
    for (SimNode n : graph.getNodes().values()) {
      if (n.isTransition()) {
        ...
        if (n.isAndJoin() && !n.isAndSplit()) {
          s = new SimJoinServer(this, n.getid(), n.getname(), n.getrole(), n.getgroup(), sd);
        } else if (n.isAndSplit() && !n.isAndJoin()) {
          s = new SimSplitServer(this, n.getid(), n.getname(), n.getrole(), n.getgroup(), sd);
        } else if (n.isAndSplit() && n.isAndJoin()) {
          s = new SimJoinSplitServer(this, n.getid(), n.getname(), n.getrole(), n.getgroup(), sd);
        } else {
          s = new SimServer(this, n.getid(), n.getname(), n.getrole(), n.getgroup(), sd);
        }
  ...
  public String printEventList() {
    ...
    if (l > 0)
      s +=
          ((SimulatorEvent) list[l - 1]).getName()
              + " : "
              + ((SimulatorEvent) list[l - 1]).getTime();
  ...
  public void addLog(int id, String name, double time) {
    if (params.getWriteLog())
      ...
      log.add(
          Messages.getString("QuantAna.Simulation.Log.CaseArrival")
              + " CASE_"
              + id
              + " "
              + Messages.getString("QuantAna.Simulation.Log.At")
              + " TASK '"
              + name
              + "' "
              + Messages.getString("QuantAna.Simulation.Log.AtTime")
  ...
  public void addLog(
      String res, String task, int caseid, double svctime, double wttime, double runtime) {
    if (params.getWriteLog())
      // Joerg Evermann, Feb 18 2016
      log.add(
          Messages.getString("QuantAna.Simulation.Log.ResourceBinding")
              + " RES "
              + res
              + " "
              + Messages.getString("QuantAna.Simulation.Log.To")
              + " TASK '"
              + task
              + "' "
              + Messages.getString("QuantAna.Simulation.Log.AtTime")
              ...
              + caseid
              + " "
              + Messages.getString("QuantAna.Simulation.Log.ServiceTime")
              + String.format(" %.2f", svctime)
              + " "
              + Messages.getString("QuantAna.Simulation.Log.WaitTime")
