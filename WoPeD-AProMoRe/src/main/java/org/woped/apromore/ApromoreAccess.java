...
public class ApromoreAccess {
  ...
  private static final SimpleDateFormat apromoreTimeFormat =
  ...
  public void test(String server, String port, String managerpath, String username) {
    ...
    try {
      ...
      showDialog(
          Messages.getString("Apromore.UI.Validation.Error.Port"),
    ...
    try {
      ...
      showDialog(
          Messages.getString("Apromore.UI.Validation.Error.Url"),
          Messages.getString("Apromore.UI.Error.Title"),
          JOptionPane.ERROR_MESSAGE);
      return;
      ...
      if (e.getCause() instanceof ConnectTimeoutException) {
        showDialog(
            Messages.getString("Apromore.UI.Validation.Error.Timeout"),
            Messages.getString("Apromore.UI.Error.Title"),
            JOptionPane.ERROR_MESSAGE);
      } else if (e.getCause() instanceof UnknownHostException) {
        showDialog(
            Messages.getString("Apromore.UI.Validation.Error.UnknownHost"),
            Messages.getString("Apromore.UI.Error.Title"),
            JOptionPane.ERROR_MESSAGE);
      } else {
        showDialog(
    ...
    if (user == null) {
      showDialog(
          Messages.getString("Apromore.UI.Validation.Error.User"),
          Messages.getString("Apromore.UI.Error.Title"),
          JOptionPane.ERROR_MESSAGE);
      return;
    }
    showDialog(
        Messages.getString("Apromore.UI.Validation.ConnectionOk"),
        Messages.getString("Apromore.UI.TextBand.Title"),
        JOptionPane.INFORMATION_MESSAGE);
  }

  public String[][] getProcessList() throws Exception {
    UserType user =
        managerService.readUserByUsername(
  ...
  public String[][] filterProcessList(
      String name, String id, String owner, String type, String domain) {
      ...
      for (int i = 0; i < processList.size() - 1; i++) {
        if ((processList.get(i).getName().toLowerCase().contains(name.toLowerCase())
            ...
            && (processList.get(i).getDomain().toLowerCase().contains(domain.toLowerCase())
                || domain.equalsIgnoreCase(""))
            && ((id == null) || id.equals("" + processList.get(i).getId()))) j++;
      }
    }
  ...
  public ByteArrayInputStream importProcess(
      int id, boolean edgesToPlaces, boolean tasksToTransitions) throws Exception {
    ...
    RequestParameterType<?> rptTasks =
        new RequestParameterType<>("isCpfTaskPnmlTransition", tasksToTransitions);
    // RequestParameterType<?> rptEdges = new RequestParameterType<>("isCpfTaskPnmlTransition",
    // edgesToPlaces);
  ...
  public void setCurrentServer(int selectedIndex) {
    ...
    ConfigurationManager.getConfiguration()
        .setApromoreManagerPath(servers[selectedIndex].getApromoreManagerPath());
    ConfigurationManager.getConfiguration()
        .setApromoreServerName(servers[selectedIndex].getApromoreServerName());
    ConfigurationManager.getConfiguration()
        .setApromoreServerPort(servers[selectedIndex].getApromoreServerPort());

    ConfigurationManager.getConfiguration()
        .setApromoreServerURL(servers[selectedIndex].getApromoreServerURL());
