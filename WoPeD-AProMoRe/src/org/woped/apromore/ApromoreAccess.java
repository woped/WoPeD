...
public class ApromoreAccess {
	...
	public InputStream importProcess(int id) throws Exception {
		...
		exf = managerService.exportFormat(p.getId(), p.getName(), "MAIN",
				p.getLastVersion(), "PNML 1.3.2", p.getLastVersion(), true, p.getOwner(),
				noCanoniserParameters);
		InputStream is = exf.getNative().getInputStream();
		String inputString = getStringFromInputStream(is);
		inputString = inputString.replaceAll("pnml.apromore.org",
				"pnml.woped.org");
		InputStream manipulateIs = new ByteArrayInputStream(
	...
	public ImportProcessResultType export(String userName, String domain,
			String processName, String version, boolean makePublic,
		...
		return managerService.importProcess(userName, 0, "PNML 1.3.2",
				processName, version, fis, "", "", sdf.format(new Date()), "",
				makePublic, noCanoniserParameters);
	}
