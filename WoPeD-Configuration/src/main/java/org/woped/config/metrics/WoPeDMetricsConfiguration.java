...
/**
 * Class that provides access to the metrics configuration settings. Access to it at runtime is to
 * be gained through ConfigurationManager.
 *
 * @see ConfigurationManager
 * @author Philip Allgaier
 */
public class WoPeDMetricsConfiguration extends WoPeDConfiguration implements IMetricsConfiguration {

  private ResourceBundle rb = PropertyResourceBundle.getBundle("Messages");
  ...
  private HashMap<Integer, ConfigurationDocument> confMap =
      new HashMap<Integer, ConfigurationDocument>();
  private HashMap<Integer, ConfigurationDocument> confMapBackup =
      new HashMap<Integer, ConfigurationDocument>();
  ...
  private HashMap<String, AlgorithmGroup> algorithmGroupsMap =
      new HashMap<String, AlgorithmGroup>();
  ...
  private final String ALL_METRICS_GROUP_NAME =
      rb.getString("Metrics.AlgorthimGroup.Name.AllMetrics");
  private final String ALL_CUSTOM_GROUP_NAME =
      rb.getString("Metrics.AlgorthimGroup.Name.AllCustom");
  ...
  public boolean readConfig() {
    ...
    if (new File(metricsFilePath).exists()) {
      ...
      if (confOk) confMap.put(currentFileID, confDoc);
    } else {
  ...
  public boolean readConfig(InputStream is) {
    try {
      confDoc = org.woped.config.metrics.ConfigurationDocument.Factory.parse(is);
      return readConfig(confDoc);
    } catch (XmlException e) {
      LoggerManager.error(Constants.CONFIG_LOGGER, rb.getString("Init.Config.ParsingError"));
      return false;
    } catch (IOException e) {
  ...
  public boolean readConfig(File file) {
    if (file.exists()) {
      try {
        ...
        // schema
        confDoc = org.woped.config.metrics.ConfigurationDocument.Factory.parse(file);
        return readConfig(confDoc);
      } catch (XmlException e) {
        LoggerManager.error(Constants.CONFIG_LOGGER, rb.getString("Init.Config.ParsingError"));
        return false;
      ...
      LoggerManager.error(
          Constants.CONFIG_LOGGER,
          rb.getString("Init.Config.FileNotFound") + ": " + file.getName() + ".");
      return false;
    }
  }

  /**
   * Read the actual metrics configuration and retrieve the variables, algorithms and
   * algorithmGroups as well as "manually" creating the three internal algorithmGroups.
   *
  ...
  public boolean readConfig(XmlObject configDoc) {
    ...
    org.woped.config.metrics.ConfigurationDocument confDoc =
        (org.woped.config.metrics.ConfigurationDocument) configDoc;
    ...
    if (confDoc != null && (config = confDoc.getConfiguration()) != null) {
      ...
      if (config.isSetAlgorithmGroups()) {
        ...
        for (int i = 0; i < algorithmGroups.length; i++) {
          ...
          localAlgorithmGroupIDOrder.put(
              algorithmGroups[i].getGroupOrder(), algorithmGroups[i].getID());
      ...
      LoggerManager.info(Constants.CONFIG_LOGGER, rb.getString("Init.Config.LoadingSuccess") + ".");
      return true;
    } else {
      LoggerManager.info(Constants.CONFIG_LOGGER, rb.getString("Init.Config.LoadingError") + ".");
      return false;
    }
  }

  /**
   * Create the two groups "ALL_METRICS" and "ALL_CUSTOM". Those will always be shown as the two
   * first in the metrics sidebar dropdown box.
   */
  private void createManualGroups() {
    ...
    for (String key : ungroupedMetrics) dummyAlgorithmGroup.addNewAlgorithmID().setStringValue(key);
    ...
    for (String key : algorithmGroupsMap.keySet()) {
      if (isCustomMetric(key)) manualAlgorithmGroup.addNewAlgorithmID().setStringValue(key);
    }
    ...
    for (int i : fileMap.keySet()) {
      ...
      for (String key : ungroupedCustomMetrics) {
        ...
        dummyAlgorithmGroup
            .getAlgorithmIDArray(dummyAlgorithmGroup.getAlgorithmIDArray().length - 1)
            .setOrder(dummyAlgorithmGroup.getAlgorithmIDArray().length);
      }
    }
  ...
  /**
   * DO NOT CALL! ONLY IMPLEMENTED HERE TO PREVENT THE EXECUTION OF THE BEHAVIOR OF PARENT CLASS!
   *
   * @return always false
   */
  ...
  public String getVariableName(String variableID) {
    if (!variablesMap.containsKey(variableID)) return null;
    ...
    if (variablesMap.get(variableID).isSetName()) {
      ...
      else return Messages.getString(variablesMap.get(variableID).getName());
  ...
  public void setVariableName(String variableID, String variableName) {
    if (variablesMap.containsKey(variableID)) {
      Variable[] vars =
          confMap
              .get(variableOrigin.get(variableID))
              .getConfiguration()
              .getVariables()
              .getVariableArray();
  ...
  public boolean hasVariableFormula(String variableID) {
    if (!variablesMap.containsKey(variableID)) return false;

    if (variablesMap.get(variableID).isSetFormula()
        && !variablesMap.get(variableID).getFormula().isEmpty()) return true;
    else return false;
  }
  ...
  public boolean hasVariableMethod(String variableID) {
    ...
    if (variablesMap.get(variableID).isSetMethod()
        && !variablesMap.get(variableID).getMethod().isEmpty()) return true;
    else return false;
  }
  ...
  public String getVariableDescription(String variableID) {
    ...
    if (variablesMap.get(variableID).isSetDescription())
      ...
      else return Messages.getString(variablesMap.get(variableID).getDescription());
    else return "";
  }
  ...
  public String getVariableImplementation(String variableID) {
    ...
    if (hasVariableMethod(variableID)) return getVariableMethod(variableID);
    else if (hasVariableFormula(variableID)) return getVariableFormula(variableID);
    else return null;
  }
  ...
  public String getAlgorithmName(String algorithmID) {
    if (!algorithmsMap.containsKey(algorithmID)) return null;
    ...
    if (algorithmsMap.get(algorithmID).isSetName())
      ...
      else return Messages.getString(algorithmsMap.get(algorithmID).getName());
    else return algorithmID;
  }
  ...
  public boolean hasAlgorithmFormula(String algorithmID) {
    if (!algorithmsMap.containsKey(algorithmID)) return false;

    if (algorithmsMap.get(algorithmID).isSetFormula()
        && !algorithmsMap.get(algorithmID).getFormula().isEmpty()) return true;
    else return false;
  }
  ...
  public boolean hasAlgorithmMethod(String algorithmID) {
    ...
    if (algorithmsMap.get(algorithmID).isSetMethod()
        && !algorithmsMap.get(algorithmID).getMethod().isEmpty()) return true;
    else return false;
  }
  ...
  public String getAlgorithmDescription(String algorithmID) {
    ...
    if (algorithmsMap.get(algorithmID).isSetDescription())
      ...
      else return Messages.getString(algorithmsMap.get(algorithmID).getDescription());
    else return "";
  }
  ...
  public String getAlgorithmsImplementation(String algorithmID) {
    ...
    if (hasAlgorithmMethod(algorithmID)) return getAlgorithmMethod(algorithmID);
    else if (hasAlgorithmFormula(algorithmID)) return getAlgorithmFormula(algorithmID);
    else return null;
  }
    ...
    if (getMetricThresholdCount(metricID) > thresholdID) {
      if (algorithmsMap.containsKey(metricID)) {
        if (algorithmsMap
                ...
                .getState()
                .intValue()
            == ThresholdState.Enum.INT_GREEN) return MetricThresholdState.GREEN;
        else if (algorithmsMap
                .get(metricID)
                .getThresholds()
                .getThresholdArray()[thresholdID]
                .getState()
                .intValue()
            == ThresholdState.Enum.INT_YELLOW) return MetricThresholdState.YELLOW;
        else if (algorithmsMap
                .get(metricID)
                .getThresholds()
                .getThresholdArray()[thresholdID]
                .getState()
                .intValue()
            == ThresholdState.Enum.INT_RED) return MetricThresholdState.RED;
      } else if (variablesMap.containsKey(metricID)) {
        if (variablesMap
                .get(metricID)
                .getThresholds()
            ...
            == ThresholdState.Enum.INT_YELLOW) return MetricThresholdState.YELLOW;
                ...
                .intValue()
            == ThresholdState.Enum.INT_RED) return MetricThresholdState.RED;
      } else return MetricThresholdState.NONE;
    }
    return null;
  }
  ...
  public boolean deleteAlgorithm(String algorithmID) {
    if (!isCustomMetric(algorithmID)) {
      JOptionPane.showMessageDialog(
          null,
          Messages.getString("Metrics.General.CannotDeleteInternal"),
          Messages.getString("Metrics.General.CannotDeleteInternal"),
          JOptionPane.ERROR_MESSAGE);
      return false;
    } else {
      int res =
          JOptionPane.showConfirmDialog(
              null,
              Messages.getString("Metrics.General.ReallyDeleteAlgo"),
              Messages.getString("Metrics.General.ReallyDeleteAlgo"),
  ...
  public List<String> getAlgorithmGroupIDs() {
    ...
    for (int i : sortedset) list.add(algorithmGroupIDOrder.get(i));
  ...
  public String getAlgorithmGroupName(String algorithmGroupID) {
    ...
    if (algorithmGroupID.equals(ALL_METRICS_GROUP_NAME)
        || algorithmGroupID.equals(ALL_CUSTOM_GROUP_NAME)) return algorithmGroupID;

    if (!algorithmGroupsMap.containsKey(algorithmGroupID)) return null;
    ...
    if (algorithmGroupsMap.get(algorithmGroupID).isSetName())
      ...
      else return Messages.getString(algorithmGroupsMap.get(algorithmGroupID).getName());
    else return algorithmGroupID;
  }
  ...
  public void setAlgorithmGroupName(String algorithmGroupID, String algorithmGroupName) {
    if (algorithmGroupsMap.containsKey(algorithmGroupID)) {
      AlgorithmGroup[] groups =
          confMap
              .get(algorithmGroupOrigin.get(algorithmGroupID))
              .getConfiguration()
              .getAlgorithmGroups()
              .getAlgorithmGroupArray();
  ...
  public List<String> getGroupIDsFromGroup(String algorithmGroupID) {
    ...
    if (algorithmGroupID.equals(ALL_METRICS_GROUP_NAME)) {
      ...
      for (String key : algorithmGroupOrigin.keySet()) {
        if (algorithmGroupOrigin.get(key) != 0) list.add(key);
      }
    else list.add(algorithmGroupID);
  ...
  private List<String> getUngroupedMetricsFromFile(int fileID) {
    ...
    for (String key : variableOrigin.keySet()) {
      if (variableOrigin.get(key) == fileID) list.add(key);
    }

    for (String key : algorithmOrigin.keySet()) {
      if (algorithmOrigin.get(key) == fileID) list.add(key);
    }

    for (String key : algorithmGroupsMap.keySet()) {
      // Ignore those two
      if (key.equals(ALL_METRICS_GROUP_NAME) || key.equals(ALL_CUSTOM_GROUP_NAME)) continue;
      if (algorithmGroupOrigin.get(key) == fileID) {
        List<String> children = getAlgorithmIDsFromGroup(key);
        for (String childKey : children) list.remove(childKey);
      }
    }
  ...
  public List<String> getAlgorithmIDsFromGroup(String algorithmGroupID) {
    ...
    for (int i : sortedset) list.add(algorithmGroupMemberIDOrder.get(i));

    for (int i = 0; i < ids.length; i++) {
      if (!ids[i].isSetOrder()) list.add(ids[i].getStringValue());
    }
  ...
  public String getAlgorithmGroupDescription(String algorithmGroupID) {
    ...
    else return Messages.getString(algorithmGroupsMap.get(algorithmGroupID).getDescription());
  }

  public void setAlgorithmGroupDescription(
      String algorithmGroupID, String algorithmGroupDescription) {
    if (algorithmGroupsMap.containsKey(algorithmGroupID)) {
      AlgorithmGroup[] groups =
          confMap
              .get(algorithmGroupOrigin.get(algorithmGroupID))
  ...
  public boolean isCustomMetric(String metricsID) {
    if (variablesMap.containsKey(metricsID) == true)
      ...
      if (variableOrigin.containsKey(metricsID) && variableOrigin.get(metricsID) != 0) return true;
      else return false;
  ...
  public void setMetricThreholdState(
      String metricID, int thresholdID, java.lang.Enum<?> metricState) {
    if (isAlgorithm(metricID) && algorithmsMap.containsKey(metricID))
      algorithmsMap
          .get(metricID)
          .getThresholds()
          .getThresholdArray(thresholdID)
          .xsetState((ThresholdState) metricState);
  }
  ...
  public boolean isMetricIDInUse(String metricID) {
    if (variablesMap.containsKey(metricID) || algorithmsMap.containsKey(metricID)) return true;
    else return false;
  }
  ...
  public boolean isVariable(String metricID) {
    if (variablesMap.get(metricID) != null) return true;
    else return false;
  }
  ...
  public boolean isAlgorithm(String metricID) {
    if (algorithmsMap.get(metricID) != null) return true;
    else return false;
  }
  ...
  public String getMetricOrigin(String metricID) {
    if (isVariable(metricID)) return fileMap.get(variableOrigin.get(metricID));
    else if (isAlgorithm(metricID)) return fileMap.get(algorithmOrigin.get(metricID));
    else return "";
  }

  /**
   * Starts a new edit session = create an internal copy of the current XML state so that we can go
   * back if editing is canceled. Once the editing changes are acknowledged, the internal backup
   * state gets overwritten and the new state is saved.
   */
  ...
  /**
   * Ends the current edit session = either drop the changes or keep them, depending on the passed
   * parameter
   *
  ...
  public int findFileIDToFileName(String fileName) {
    for (int i : fileMap.keySet()) {
      if (fileMap.get(i).equals(fileName)) return i;
    }
    return -1;
  }
  ...
  public void addNewMetricFile(String filePath) {
    if (!fileMap.containsValue(filePath)) fileMap.put(fileMap.size(), filePath);
