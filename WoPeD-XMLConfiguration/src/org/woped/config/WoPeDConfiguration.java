 ...
 * 10.10.2003
 */
public class WoPeDConfiguration implements IConfiguration, WoPeDLogger
{
    ...
    public boolean readConfig()
    {
        ...
        {
            logger.warn("User-Configuration not found. Try using Backup Configuration.");
    ...
    public boolean readConfig(InputStream is)
    {
        ...
        {
            logger.error("ERROR during parsing file.");
            return false;
        } catch (IOException e)
        {
    ...
    public boolean readConfig(ConfigurationDocument confDoc)
    {
        ...
        if (confDoc != null && (config = confDoc.getConfiguration()) != null)
        {
            ...
            {
                if (getDocument().getConfiguration().getGeneral().getRecentFilesArray()[i].getName() != null) runtimeRecentFiles.addElement(new WoPeDRecentFile(getDocument().getConfiguration().getGeneral()
                        .getRecentFilesArray()[i].getName(), getDocument().getConfiguration().getGeneral().getRecentFilesArray()[i].getPath()));
            }
            ...
            if (config.getGeneral().getHomedir() == null || config.getGeneral().getHomedir().equals(""))
            {
                if (!new File(DEFAULT_HOME).mkdir())
                {
                    ...
                    logger.error("Could not create standard homedir");
                } else
                {
                    logger.info("standard homedir created");
                }
            } else
            {
                setHomedir(config.getGeneral().getHomedir());
                // logger.info(config.getGeneral().getHomedir());
            }
    ...
    public void addRecentFile(String name, String path)
    {
        try
        {
            ...
            {
                if (runtimeRecentFiles.size() == RECENTFILES_SIZE) runtimeRecentFiles.remove(RECENTFILES_SIZE - 1);
                runtimeRecentFiles.add(0, recent);
            }
        } catch (NullPointerException ne)
        {
            logger.error("Could not store Recentfile data.");
        }
    ...
    {
        if (isHomedirSet()) return getDocument().getConfiguration().getGeneral().getHomedir();
    ...
    {
        if (getDocument().getConfiguration().getEditor().isSetArrowWidth()) return getDocument().getConfiguration().getEditor().getArrowWidth();
    ...
    {
        if (getDocument().getConfiguration().getEditor().isSetArrowheadSize()) return getDocument().getConfiguration().getEditor().getArrowheadSize();
    ...
    {
        if (getDocument().getConfiguration().getEditor().isSetArrowFilledHead()) return getDocument().getConfiguration().getEditor().getArrowFilledHead();
    ...
    {
        if (getDocument().getConfiguration().getEditor().isSetRoundRouting()) return getDocument().getConfiguration().getEditor().getRoundRouting();
        else return false;
    }
