 ...
 *     <br>
 *     <p>Represents a Petri-Net. It is the main access point to the Model. Contains the
 *     ModelElementFactory and the ModelElementContainer <br>
 *     <br>
...
public class PetriNetModelProcessor implements Serializable {
   ...
   *
   * <p>The new instance is going to have the id "noID".
   */
        ...
        if (map.getTriggerType() != -1) {
          ...
          if (map.getTriggerPosition() != null) {
            ((TransitionModel) anElement)
                .getToolSpecific()
                .getTrigger()
                .setPosition(map.getTriggerPosition().x, map.getTriggerPosition().y);
          }
        }
        if (map.getResourceOrgUnit() != null && map.getResourceRole() != null) {
          ...
          if (map.getResourcePosition() != null) {
            ((TransitionModel) anElement)
                .getToolSpecific()
                .getTransResource()
                .setPosition(map.getResourcePosition().x, map.getResourcePosition().y);
          }
        }
   ...
   *
   * <p>This Method is called, when Elements get connected. If the source or the target is an
   * Operator, the internal transformation to the classic petrinet will be done.
   *
   ...
   *
   * <p>This Method is called, when Elements get connected. It only checks the connection for
   * AalstElements and guarantees the building of a classical Petrinet in The Model, if
   * transformOperators is true.
   *
   ...
   * @param transformOperators indicator if the inner connection of an operator type should be
   *     processed
   * @return ArcModel the new arc or null, if one of the parameters is invalid
   */
  public ArcModel createArc(
      String id, Object sourceId, Object targetId, Point2D[] points, boolean transformOperators) {
    ...
    ArcModel displayedArc =
        ModelElementFactory.createArcModel(
            id, (DefaultPort) sourceModel.getChildAt(0), (DefaultPort) targetModel.getChildAt(0));
  ...
  private void resolveInnerConnections(
      AbstractPetriNetElementModel sourceModel, AbstractPetriNetElementModel targetModel) {
    ...
    /* IF OPERATOR IS TARGET */
      ...
      operatorModel =
          (OperatorTransitionModel) getElementContainer().getElementById(targetModel.getId());
  ...
  public void removeArc(Object arcId) {
    ...
    AbstractPetriNetElementModel sourceElement =
        getElementContainer().getElementById(arcToDelete.getSourceId());
    AbstractPetriNetElementModel targetElement =
        getElementContainer().getElementById(arcToDelete.getTargetId());
   ...
   *
   * <p>The fingerprint is represented as a String with the following components:
   *
   * <ul>
   *   <li>number of places
   *   <li>place details
   ...
   * </ul>
   *
   * <p>The place details consist of the place id and the amount of tokens for each place. For
   * example place p0 with 2 tokens: "p02".
   *
   * <p>The arc details consists of the source id and the target id of each arc. The arc id is not
   * appropriate for this purpose, because it is possible they could be different after
   * saving/loading a net. Each arc is preceded with a single {@code *}. For example, an arc form p1
   * to t1 is listed as "*p1t1"
   *
   * <p>This is an example of an whole fingerprint: <code>2p10p20#2*t1p2*p2t1</code>
   *
  ...
  public String getLogicalFingerprint() {
    ...
    // Add places
    Collection<AbstractPetriNetElementModel> places =
        getElementContainer().getElementsByType(AbstractPetriNetElementModel.PLACE_TYPE).values();
    ...
    for (ArcModel arc : arcs) {
      String weight =
          arc.getInscriptionValue() == 1 ? "" : String.valueOf(arc.getInscriptionValue());
   ...
   *
   * <p>The difference to the {@code String.equals()}-method is, that fingerprints are considered
   * equal if the have a different order of the contained arcs. But the id's of the places has to be
   * consistent.
   *
