...
public class ArcModel extends DefaultEdge implements Serializable {
  ...
  public void initAttributes() {
    ...
    GraphConstants.setLineStyle(
        map,
        ConfigurationManager.getConfiguration().isRoundRouting()
            ? GraphConstants.STYLE_BEZIER
            : GraphConstants.STYLE_ORTHOGONAL);
    GraphConstants.setLineEnd(map, GraphConstants.ARROW_CLASSIC);
    GraphConstants.setLineWidth(
        map,
        ConfigurationManager.getConfiguration().getArrowWidth() == 0
    ...
    GraphConstants.setEndSize(
        map,
        ConfigurationManager.getConfiguration().getArrowheadSize() == 0
    ...
    Point2D.Double weightLabelPosition =
        new Point2D.Double(defaultPos.getX(), defaultPos.getY() - 12);
    ...
    Point2D.Double probabilityLabelPosition =
        new Point2D.Double(defaultPos.getX(), defaultPos.getY() + 12);
  ...
  public boolean isXORsplit(PetriNetModelProcessor mp) {
    ...
    if (cell instanceof TransitionModel) {
      ...
      if ((opType == OperatorTransitionModel.XOR_SPLIT_TYPE)
          || (opType == OperatorTransitionModel.XORJOIN_XORSPLIT_TYPE)
    ...
    if (cell instanceof PlaceModel) {
      ...
      int num = mp.getElementContainer().getOutgoingArcs(place.getId()).size();
   ...
   *
   * <p>The inscription value represents the amount of tokens that are transferred from the source
   * to the target of the arc on a single execution. It is also known as the weight of the arc.
   *
   ...
   * to the target of the arc on a single execution. It is also known as the weight of the arc.
   *
   * <p>The valid range is form {@code 1} to {@link Integer#MAX_VALUE}.
   *
  ...
  public void setInscriptionValue(int inscriptionValue) {
    ...
    if (inscriptionValue <= 0)
      throw new IllegalArgumentException("The inscription value has to be positive");
  ...
  public CreationMap getCreationMap() {
    ...
    for (int i = 1; i < points.size() - 1; i++) {
      newPoints.add(
          new IntPair(
              (int) ((Point2D) points.get(i)).getX(), (int) ((Point2D) points.get(i)).getY()));
    }
    map.setArcPoints(newPoints);
    map.setArcProbability(getProbability());
    map.setArcDisplayProbability(displayProbability());
    map.setArcLabelPosition(
        (int) this.getProbabilityLabelPosition().getX(),
        (int) this.getProbabilityLabelPosition().getY());
