...
public class XORJoinOperatorTransitionModel extends OperatorTransitionModel {
  ...
  public void registerIncomingConnection(
     ...
     */
    if (outerContainer.getSourceElements(getId()).size() == 1
      ...
      // of the Operator
      Iterator<String> targetIter = outerContainer.getTargetElements(getId()).keySet().iterator();
      while (targetIter.hasNext()) {
        addReference(
            getNextFreeArcID(processor),
            (DefaultPort) simpleTrans.getChildAt(0),
            (DefaultPort) outerContainer.getElementById(targetIter.next()).getChildAt(0));
      }
    }
  }
  ...
  public void registerOutgoingConnection(
    ...
    Iterator<AbstractPetriNetElementModel> simpleRootIter =
        getSimpleTransContainer().getRootElements().iterator();
    while (simpleRootIter.hasNext()) {
      AbstractPetriNetElementModel pec = simpleRootIter.next();
      if (pec.getType() == AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE)
        addReference(getNextFreeArcID(processor), pec.getPort(), targetModel.getPort());
    }
  }
