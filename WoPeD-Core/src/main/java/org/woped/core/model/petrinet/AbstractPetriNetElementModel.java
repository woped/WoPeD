 ...
 *     <br>
 *     Note: (from JGraph) <br>
 *     When combining the attributes from a GraphCell with the attributes from the CellView, the
 *     graph cell's attributes have precedence over the view's attributes. The special value
 *     attribute is in sync with the cell's user object.
 *     <p>09.10.2003
 */
public abstract class AbstractPetriNetElementModel extends DefaultGraphCell
  ...
  // ! For better understandability a new color from the config colorset will highlight this
  // element.
  ...
  public AbstractPetriNetElementModel(CreationMap creationMap, Object userObject) {
    ...
    if (creationMap.getSize() == null) {
      ...
      GraphConstants.setSize(
          map, new Dimension(creationMap.getSize().getX1(), creationMap.getSize().getX2()));
    }
    ...
    if (creationMap.getId() != null) {
      ...
      if (creationMap.getNamePosition() != null) {
        nameModel.setPosition(
            new Point(creationMap.getNamePosition().x, creationMap.getNamePosition().y));
      } else if (getPosition() != null) {
        nameModel.setPosition(
            getPosition().x + ((this.getWidth() - getNameModel().getWidth()) / 2),
  ...
  public int getHierarchyLevel() {
    ...
    if (rootOwningContainer != null) {
      ...
      if (owningElement != null) result = owningElement.getHierarchyLevel() + 1;
    }
    return result;
  }
  ...
  public void addOwningContainer(ModelElementContainer owningContainer) {
    ...
    int nNewHiearchyLevel =
        ((newOwningElement != null) ? (newOwningElement.getHierarchyLevel() + 1) : 0);
    ...
    if (owningContainer == rootOwningContainer) {
      ...
      if (i.hasNext()) {
        ...
        while (i.hasNext()) {
          ...
          int candidateHierarchy =
              ((owningElement != null) ? owningElement.getHierarchyLevel() + 1 : 0);
          if (candidateHierarchy < newLowest) {
            rootOwningContainer = candidate;
            newLowest = candidateHierarchy;
          }
        }
      } else rootOwningContainer = null;
    }
  }
   ...
   *
   * @param x the value to set the model and the surrounded Objects for the x coordinate
   * @param y the value to set the model and the surrounded Objects for the y coordinate
   */
  public void setPositionGroup(int x, int y) {
    setPosition(x, y);
    nameModel = getNameModel();
    if (this.getId().startsWith("t")) nameModel.setPosition(x - 10, y + 35);
    else nameModel.setPosition(x, y + 35);
    if (this instanceof TransitionModel) {
      ...
      if (transitionModel.hasResource()) {
        TransitionResourceModel transitionResourceModel =
            transitionModel.getToolSpecific().getTransResource();
  ...
  public void setColor(Color c) {
    ...
    int alpha = 180;
    this.understandabilityColor = new Color(c.getRed(), c.getGreen(), c.getBlue(), alpha);
  }
  ...
  public int getHeight() {
    return (int)
        (GraphConstants.getSize(getAttributes()) == null
            ? -1
  ...
  public boolean isSubprocessElement() {
    for (ModelElementContainer container : getOwningContainers())
      if (container.getOwningElement() instanceof SubProcessModel) return true;
    return false;
  }

  /**
   * Determine whether an element is a sink node (no outgoing connections in any of the owning
   * containers)
   *
  ...
  public boolean isSink() {
    ...
    while (ownerIterator.hasNext()) {
      ...
      Map<String, AbstractPetriNetElementModel> targetElements =
          currentContainer.getTargetElements(getId());
      if (targetElements.size() > 0) result = false;
    }
    return result;
  }
  ...
  public boolean isRoot() {
    ...
    while (ownerIterator.hasNext()) {
      ...
      Map<String, AbstractPetriNetElementModel> sourceElements =
          currentContainer.getSourceElements(getId());
      if (sourceElements.size() > 0) result = false;
    }
    return result;
  }

  //	/**
  //	 * Determine whether an element is a source node (no incoming connections in any of the owning
  // containers)
  ...
  /**
   * While the actual implementation is node-type specific, we have a common definition of what
   * active means for all node types: Being active means that the node can fire when triggered to do
   * so. This common notion is used to share some of the view and drawing logic around the notion of
   * being active
   *
