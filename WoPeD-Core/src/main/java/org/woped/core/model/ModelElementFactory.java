 ...
 *     <br>
 *     The <code>ModelElementFactory</code> could create <code>PetriNetModelElement</code>. It
 *     creates initial id's for the Elements. 29.04.2003
 */
    ...
    if (map.getId() != null) {
      try {
        ...
        if (map.getType() == AbstractPetriNetElementModel.PLACE_TYPE) {
          ...
          // Tokens
          if (map.getTokens() > 0) ((PlaceModel) modElement).setTokens(map.getTokens());

        } else if (map.getType() == AbstractPetriNetElementModel.SUBP_TYPE) {
          /* CREATING A SUBPROCESS */
          modElement = new SubProcessModel(map);
        } else if (map.getType() == AbstractPetriNetElementModel.TRANS_SIMPLE_TYPE
