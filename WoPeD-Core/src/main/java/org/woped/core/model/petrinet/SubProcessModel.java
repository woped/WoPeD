...
public class SubProcessModel extends TransitionModel implements InnerElementContainer {
  ...
  public SubProcessModel(CreationMap map) {
    ...
    if (map.getSubElementContainer() != null) {
      ModelElementContainer newContainer = copySubElementContainer(map.getSubElementContainer());
  ...
  private ModelElementContainer copySubElementContainer(ModelElementContainer container) {
    ...
    while (keyIterator.hasNext()) {
      ...
      AbstractPetriNetElementModel currentElementSub = container.getElementById(keyIterator.next());

      if (!currentElementSub.isReadOnly()) {
        CreationMap newMapSub = (CreationMap) currentElementSub.getCreationMap().clone();
        // newMap.setId("copyof_" + newMap.getId());
        String[] splitName = newMapSub.getId().split(SUBELEMENT_SEPERATOR);
        if (splitName.length > 1)
          newMapSub.setId(super.getId() + SUBELEMENT_SEPERATOR + splitName[splitName.length - 1]);
        newMapSub.setName(newMapSub.getId());
        newElementSub = ModelElementFactory.createModelElement(newMapSub);

        if (currentElementSub instanceof TransitionModel) {
          TransitionModel newTransitionSub = (TransitionModel) newElementSub;
          newTransitionSub.setToolSpecific(((TransitionModel) currentElementSub).getToolSpecific());

          newContainer.addElement(newTransitionSub);

        } else newContainer.addElement(newElementSub);
      }
    }
    ...
    while (arcIterSub.hasNext()) {
      currentArcMapSub = container.getArcMap().get((arcIterSub.next())).getCreationMap();
      originalElementId = currentArcMapSub.getName();
      splitedSourceId = currentArcMapSub.getArcSourceId().split(SUBELEMENT_SEPERATOR);
      splitedTargetId = currentArcMapSub.getArcTargetId().split(SUBELEMENT_SEPERATOR);
      ...
      if ((splitedSourceId.length > 1) && (splitedTargetId.length > 1)) {
        newTargetName =
            super.getId() + SUBELEMENT_SEPERATOR + splitedTargetId[splitedTargetId.length - 1];
        newSourceName =
            super.getId() + SUBELEMENT_SEPERATOR + splitedSourceId[splitedSourceId.length - 1];
        cMapSub.setArcSourceId(newSourceName);
        cMapSub.setArcTargetId(newTargetName);
        processor.createArc(newSourceName, newTargetName);
        ...
        processor.createArc(originalElementId, cMapSub.getArcTargetId());
      } else if ((splitedSourceId.length > 1) && (splitedTargetId.length <= 1)) {
        newSourceName =
            super.getId() + SUBELEMENT_SEPERATOR + splitedSourceId[splitedSourceId.length - 1];
        cMapSub.setArcSourceId(newSourceName);
  ...
  public boolean getAllowOutgoingConnections() {
    ...
    for (Iterator<?> i = getPort().edges(); i.hasNext(); ) {
      ...
      if (o instanceof Edge) {
        Edge e = (Edge) o;
        if (e.getSource() == getPort()) ++nNumOutgoing;
      }
    }
