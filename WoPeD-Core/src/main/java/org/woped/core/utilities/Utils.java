...
public class Utils {
  ...
  public static Point getCenterPoint(Rectangle ownerBounds, Dimension componentDimension) {
    ...
    return new Point(
        middleX - (int) (componentDimension.getWidth() * 0.5),
        middleY - (int) (componentDimension.getHeight() * 0.5));
  }

  /**
   * Checks a filename for its extension and adds if necessary the standard extension. <br>
   * (= The first extension in the String Array)
   *
   * @param fileName
   * @param extensions
   * @return
  ...
  public static String getQualifiedFileName(String fileName, Vector<?> extensions) {
    ...
    if (fileName.length() >= 5) {
      ...
      for (int i = 0; i < extensions.size(); i++) {
        if (fileName
            .substring(fileName.length() - extensions.get(i).toString().length(), fileName.length())
            .equals(extensions.get(i).toString())) {
          extensionFound = true;
    ...
    if (!extensionFound && extensions.get(0).toString() != null)
      fileName += "." + extensions.get(0).toString();
    return fileName;
  }
