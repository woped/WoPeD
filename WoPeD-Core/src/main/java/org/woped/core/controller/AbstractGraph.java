...
public abstract class AbstractGraph extends JGraph implements Printable {
  ...
  public abstract void drawNet(PetriNetModelProcessor processor);

  /**
   * Groups an <code>NameModel</code> ans all additional cells to their <code>PetriNetModelElement
   * </code>.
   *
   * @param mainElement PetriNetModelElement
   * @param name NameModel
   * @param additional other cells to group
   * @param ungroupable flag for ungrouping
   * @return
   */
  public GroupModel groupName(
      AbstractPetriNetElementModel mainElement,
      NameModel name,
      Object[] additional,
      boolean ungroupable) {
  ...
  /**
   * Groups an <code>NameModel</code> to its <code>PetriNetModelElement</code>.
   *
   * @param mainElement
   * @param name
   * @return
  ...
  public boolean isValidConnection(Port source, Port target) {
    AbstractPetriNetElementModel sourceCell =
        (AbstractPetriNetElementModel) ((DefaultPort) source).getParent();
    AbstractPetriNetElementModel targetCell =
        (AbstractPetriNetElementModel) ((DefaultPort) target).getParent();
  ...
  public Vector<Object> getAllCellsForLocation(double x, double y) {
    ...
    for (cell = getNextCellForLocation(cell, x, y);
        cell != topMostCell;
  ...
  public void setEnabled(boolean enabled) {
    ...
    while ((parent != null) && (!(parent instanceof IEditor))) parent = parent.getParent();
    if (parent != null) {
      if (((IEditor) parent).isShowingTStar()) super.setEnabled(false);
