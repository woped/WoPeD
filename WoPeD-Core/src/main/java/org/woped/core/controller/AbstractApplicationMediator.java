...
public abstract class AbstractApplicationMediator implements IViewListener {
  ...
  public AbstractApplicationMediator(IUserInterface ui, IGeneralConfiguration conf) {
    ...
    try {
      ...
      if (ConfigurationManager.getConfiguration().getLookAndFeel() == null) {
        ConfigurationManager.getConfiguration().setLookAndFeel(Platform.getSystemLookAndFeel());
      }
  ...
  public abstract IEditor createSubprocessEditor(
  ...
  /**
   * Returns the reference to the controller of ViewEventProcessors (VEP), which holds a map of
   * VEPs, where the key is the major code of the event to be handled.
   *
   * @return
   */
  ...
  public void processViewEvent(AbstractViewEvent viewevent) {
    Collection<AbstractEventProcessor> eventProcessors =
        getVepController().getViewEventProcessorsForType(viewevent.getType());
