...
public abstract class AbstractMarqueeHandler extends BasicMarqueeHandler {
  ...
  /**
   * The isForceMarqueeEvent method is used to fetch the subsequent mousePressed, mouseDragged and
   * mouseReleased events.Thus, the marquee handler may be used to gain control over the mouse. The
   * argument to the method is the event that triggered the call, namely the mousePressed event.
   * (The graph�s portsVisible flag is used to toggle the connect mode.)
   *
  ...
  /**
   * checks if there is a cell under the mouse pointer, and if one is found, it returns its default
   * port(first port)
   */
  ...
  /**
   * The paintConnector method displays a preview of the edge to be inserted. (The paintPort method
   * is not shown.)
   */
  public void paintConnector(Color fg, Color bg, Graphics g) {
    ...
    if (firstPort != null && start != null && current != null) {
      ...
      g.drawLine(
          (int) start.getX(), (int) start.getY(), (int) current.getX(), (int) current.getY());
    }
  }
