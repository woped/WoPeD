...
public class DatabaseBackedDictionary extends AbstractCachingDictionary {
    ...
    public void install(Map params, String contextPath) throws JWNLException {
        ...
        MorphologicalProcessor morph =
                (param == null) ? null : (MorphologicalProcessor) param.create(contextPath);

        param = (Param) params.get(DICTIONARY_ELEMENT_FACTORY);
        DatabaseDictionaryElementFactory factory =
                (param == null) ? null : (DatabaseDictionaryElementFactory) param.create(contextPath);

        param = (Param) params.get(DATABASE_MANAGER);
        DatabaseManager manager = (param == null) ? null : (DatabaseManager) param.create(contextPath);
