...
public final class JWNL {
	...
	public static void initialize(InputStream propertiesStream, String contextPath) throws JWNLException {
		...
		createElementFromNode(dictionaryNodeList.item(0)).install(contextPath);
	...
	private static Param[] getParams(NodeList list) throws JWNLException {
		...
		for (int i = 0; i < list.getLength(); i++) {
			...
			if (n.getNodeType() == Node.ELEMENT_NODE && n.getNodeName().equals(PARAM_TAG)) {
				...
				if (name != null && name.equals("dictionary_path")) {
					ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
					URL resource = classLoader.getResource(value);
					if (!_contextPath.equals("")) {
						value = _contextPath + resource.getPath().substring(resource.getPath().indexOf(".war/")+4);
					} else {
						value = resource.getPath();
					}
//					System.out.println(name + " - " + _contextPath + " - " + value);
				}
