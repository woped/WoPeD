package org.woped.p2t.contentDetermination.preprocessing;
...
public class FormatConverter {
	...
	public ProcessModel transformFromRigidFormat(Process p) {
		...
		for (Task t: p.getTasks()) {
			...
			if (converterMap.containsKey(id)) {
				...
				if (elem.getClass().toString().endsWith("Activity")) {
					org.woped.p2t.dataModel.process.Activity a = (org.woped.p2t.dataModel.process.Activity) elem;
					pm.addActivity(a);
					idMap.put(t.getId(), a.getId());
					elemMap.put(a.getId(), a);
				}
				if (elem.getClass().toString().endsWith("Event")) {
					org.woped.p2t.dataModel.process.Event e = (org.woped.p2t.dataModel.process.Event) elem;
					pm.addEvent(e);
		...
		for (Gateway g: p.getGateways()) {
			if (!g.getName().equals("") && converterMap.containsKey(Integer.valueOf(g.getName()))) {
				...
				org.woped.p2t.dataModel.process.Gateway gw = (org.woped.p2t.dataModel.process.Gateway) converterMap.get(id);
				pm.addGateway(gw);
			} else {
				if (g.getGatewayType() == GatewayType.XOR) {
					org.woped.p2t.dataModel.process.Gateway gw = new org.woped.p2t.dataModel.process.Gateway(getId(), "", null, null, org.woped.p2t.dataModel.process.GatewayType.XOR);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.OR) {
					org.woped.p2t.dataModel.process.Gateway gw = new org.woped.p2t.dataModel.process.Gateway(getId(), "", null, null, org.woped.p2t.dataModel.process.GatewayType.OR);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.AND) {
					org.woped.p2t.dataModel.process.Gateway gw = new org.woped.p2t.dataModel.process.Gateway(getId(), "", null, null, org.woped.p2t.dataModel.process.GatewayType.AND);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.EVENT) {
					org.woped.p2t.dataModel.process.Gateway gw = new org.woped.p2t.dataModel.process.Gateway(getId(), "", null, null, org.woped.p2t.dataModel.process.GatewayType.EVENT);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
		...
		for (ControlFlow f: p.getControlFlow()) {
			...
			org.woped.p2t.dataModel.process.Arc arc = new Arc(getId(), f.getName(), source , target);
			pm.addArc(arc);
		}
		return pm;
	}
	 ...
	 */
	public Process transformToRigidFormat(org.woped.p2t.dataModel.process.ProcessModel pm) {
		...
		// Transform events
		for (org.woped.p2t.dataModel.process.Event e: pm.getEvents().values()) {
		...
		// Transform gateway
		for (org.woped.p2t.dataModel.process.Gateway g: pm.getGateways().values()) {
			if (g.getType() == org.woped.p2t.dataModel.process.GatewayType.XOR) {
				Gateway gt = new Gateway(GatewayType.XOR,Integer.toString(g.getId()));
				elementMap.put(g.getId(), gt);

			}
			if (g.getType() == org.woped.p2t.dataModel.process.GatewayType.OR) {
				Gateway gt = new Gateway(GatewayType.OR,Integer.toString(g.getId()));
				elementMap.put(g.getId(), gt);
			}
			if (g.getType() == org.woped.p2t.dataModel.process.GatewayType.AND) {
				Gateway gt = new Gateway(GatewayType.AND,Integer.toString(g.getId()));
				elementMap.put(g.getId(), gt);
			}
			if (g.getType() == org.woped.p2t.dataModel.process.GatewayType.EVENT) {
		...
		// Transform arcs
		for (org.woped.p2t.dataModel.process.Arc arc: pm.getArcs().values()) {
	 ...
	 */
	public Process transformToRPSTFormat(org.woped.p2t.dataModel.process.ProcessModel pm) {
		Process p = new Process();
		HashMap <Integer, Node> elementMap = new HashMap<>();
